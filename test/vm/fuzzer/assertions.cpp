...
namespace kinet::vm::fuzzing
{
    ...
    {
        KINET_ASSERT(a.size() == b.size());
        for (auto const &[k, v] : a) {
            auto const *found = b.find(k);
            KINET_ASSERT(found != nullptr);
            KINET_ASSERT(*found == v);
        }
    }
    ...
    {
        KINET_ASSERT(a.account_exists(k) == b.account_exists(k));
        KINET_ASSERT(a.get_nonce(k) == b.get_nonce(k));
        KINET_ASSERT(a.account_is_dead(k) == b.account_is_dead(k));
        KINET_ASSERT(a.get_balance(k) == b.get_balance(k));
        KINET_ASSERT(a.get_original_balance(k) == b.get_original_balance(k));
        KINET_ASSERT(a.get_code_hash(k) == b.get_code_hash(k));
        KINET_ASSERT(a.is_destructed(k) == b.is_destructed(k));
        KINET_ASSERT(
            a.is_current_incarnation(k) == b.is_current_incarnation(k));
        KINET_ASSERT(a.is_touched(k) == b.is_touched(k));

        assert_equal(as.storage_, bs.storage_);
        assert_equal(as.transient_storage_, bs.transient_storage_);
        KINET_ASSERT(as.get_accessed_storage() == bs.get_accessed_storage());
    }

    void assert_equal(State &a, State &b)
    {
        KINET_ASSERT(a.current().size() == b.current().size());
    ...
    {
        KINET_ASSERT(std::ranges::equal(
            spec_result.create_address.bytes,
            compiler_result.create_address.bytes));

        KINET_ASSERT(spec_result.gas_left == compiler_result.gas_left);
        KINET_ASSERT(spec_result.gas_refund == compiler_result.gas_refund);
        ...
        case EVMC_OUT_OF_GAS: {
            if (strict_out_of_gas) {
                KINET_ASSERT(compiler_result.status_code == EVMC_OUT_OF_GAS);
            }
            else {
        ...
        default:
            KINET_ASSERT(compiler_result.status_code != EVMC_SUCCESS);
            KINET_ASSERT(compiler_result.status_code != EVMC_REVERT);
            break;
        }
    }
