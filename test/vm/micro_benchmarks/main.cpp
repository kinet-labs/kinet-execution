...
using traits = EvmTraits<KINET_ETH_OSAKA>;
...
using BenchTestStateRef = kinet::test::TestStateRef<true>;
...
{
    auto const json_state = kinet::test::JsonState{};
    auto const test_state = json_state.make_test_state<true>();

    vm::VM kinet_vm;
    kinet_vm.debug_set_execute_override(
        [](auto const *const,
           auto *const,
           auto const,
           auto const *msg,
    ...
    BlockState block_state{test_state->trie_db, kinet_vm};
    kinet::State state{block_state, Incarnation{2, 1}};

    kinet::test::TestBlockHashBuffer block_hash_buffer{};
    Transaction tx{};
    ...
    auto test_host = kinet::test::TestHost<traits>{
        block_hash_buffer,
        state,
        tx,
        tx_sender,
    ...
    for (size_t i = 0; i < bench.subject_seqs.size(); ++i) {
        ...
        auto const calldata = [&] {
            if (auto const &ss = bench.effect_free_subject_seqs) {
                KINET_ASSERT(ss->size() == bench.subject_seqs.size());
...
BenchmarkBuilder &BenchmarkBuilder::run_latency_benchmark()
{
    ...
    KINET_ASSERT(calldata_.size());
    KINET_ASSERT(has_output_);
    KINET_ASSERT(num_inputs_ >= 1);
