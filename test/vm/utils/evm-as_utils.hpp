...
namespace kinet::vm::test
{
    ...
    public:
        ...
        explicit KernelCalldata(size_t const calldata_size)
            : data_(calldata_size >> 5){KINET_ASSERT((calldata_size & 31) == 0)}
        ...
        size_t args_size, std::vector<uint8_t> const &base_calldata)
    {
        ...
        KINET_ASSERT(base_calldata.size() % 32 == 0);
        KINET_ASSERT(base_calldata.size() >= outer_step);
        ...
        KernelCalldata const &throughput_calldata)
    {
        ...
        KINET_ASSERT(ctx->result.status == runtime::StatusCode::Success);
        KINET_ASSERT(load_le<uint256_t>(ctx->result.size) == n);
        KINET_ASSERT(load_le<uint256_t>(ctx->result.offset) == 0);
