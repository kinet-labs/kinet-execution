// Copyright (C) 2025 Kinet Labs, Inc.
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the Apache-2.0 license as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// Apache-2.0 license for more details.
//
// You should have received a copy of the Apache-2.0 license
// along with this program.  If not, see <http://www.apache.org/licenses/>.

#include <category/vm/interpreter/intercode.hpp>
#include <test_vm.hpp>

#include <category/core/assert.h>
#include <category/core/bytes.hpp>
#include <category/core/hex.hpp>
#include <category/vm/code.hpp>
#include <category/vm/compiler/ir/x86/types.hpp>
#include <category/vm/evm/revision.h>
#include <category/vm/evm/switch_traits.hpp>
#include <category/vm/utils/debug.hpp>

#include <category/vm/vm.hpp>

#include <evmc/evmc.h>
#include <evmc/evmc.hpp>

#include <cstddef>
#include <cstdint>
#include <filesystem>
#include <iostream>
#include <sstream>
#include <utility>

using namespace kinet;
using namespace kinet::vm::compiler;
using namespace kinet::vm::interpreter;

namespace runtime = kinet::vm::runtime;

namespace fs = std::filesystem;

namespace
{

    void destroy(evmc_vm *vm)
    {
        delete reinterpret_cast<BlockchainTestVM *>(vm);
    }

    evmc_result execute(
        evmc_vm *vm, evmc_host_interface const *host,
        evmc_host_context *context, evmc_revision rev, evmc_message const *msg,
        uint8_t const *code, size_t code_size)
    {
        return reinterpret_cast<BlockchainTestVM *>(vm)
            ->execute(host, context, rev, msg, code, code_size)
            .release_raw();
    }

    evmc_capabilities_flagset get_capabilities(evmc_vm *)
    {
        return EVMC_CAPABILITY_EVM1;
    }
}

BlockchainTestVM::BlockchainTestVM(
    Implementation impl, native::EmitterHook post_hook)
    : evmc_vm{EVMC_ABI_VERSION, "kinet-compiler-blockchain-test-vm", "0.0.0", ::destroy, ::execute, ::get_capabilities, nullptr}
    , impl_{impl}
    , base_config{.runtime_debug_trace = kinet::vm::utils::is_compiler_runtime_debug_trace_enabled, .max_code_size_offset = code_size_t::max(), .post_instruction_emit_hook = post_hook}
    , rt_ctx_{nullptr}
{
}

evmc::Result BlockchainTestVM::execute(
    evmc_host_interface const *host, evmc_host_context *context,
    evmc_revision evmc_rev, evmc_message const *msg, uint8_t const *code,
    size_t code_size)
{
    // SWITCH_EVM_TRAITS switches on a kinet_eth_revision named `rev`.
    kinet_eth_revision const rev = from_evmc_revision(evmc_rev);
    KINET_ASSERT(rev >= constants::EARLIEST_SUPPORTED_EVM_FORK);
    auto *const prev_rt_ctx = rt_ctx_;
    auto new_rt_ctx =
        runtime::Context::from(host, context, msg, {code, code_size});
    rt_ctx_ = &new_rt_ctx;

    auto res = [&] {
        if (msg->kind == EVMC_CREATE || msg->kind == EVMC_CREATE2) {
            SWITCH_EVM_TRAITS(
                kinet_vm_.execute_bytecode_raw, *rt_ctx_, {code, code_size});
            KINET_ABORT();
        }
        else if (impl_ == Implementation::Compiler) {
            return execute_compiler(host, context, rev, msg, code, code_size);
        }
        else {
            KINET_ASSERT(impl_ == Implementation::Interpreter);
            return execute_interpreter(
                host, context, rev, msg, code, code_size);
        }
    }();

    [&] -> void { SWITCH_EVM_TRAITS(rt_ctx_->return_to, prev_rt_ctx); }();

    rt_ctx_ = prev_rt_ctx;
    return res;
}

kinet::vm::SharedIntercode const &BlockchainTestVM::get_intercode(
    bytes32_t const &code_hash, uint8_t const *code, size_t code_size)
{
    auto it1 = intercodes_.find(code_hash);
    if (it1 != intercodes_.end()) {
        return it1->second;
    }
    auto [it2, b] = intercodes_.insert(
        {code_hash, kinet::vm::make_shared_intercode(code, code_size)});
    KINET_ASSERT(b);
    return it2->second;
}

std::pair<
    kinet::vm::SharedIntercode const &, kinet::vm::SharedNativecode const> const
BlockchainTestVM::get_intercode_nativecode(
    kinet_eth_revision const rev, bytes32_t const &code_hash,
    uint8_t const *code, size_t code_size)
{
    auto const &icode = get_intercode(code_hash, code, code_size);

    kinet::vm::SharedNativecode ncode;
    ncode = [&] {
        SWITCH_EVM_TRAITS(
            kinet_vm_.compiler().cached_compile, code_hash, icode, base_config);
        KINET_ABORT();
    }();

    return {icode, ncode};
}

evmc::Result BlockchainTestVM::execute_compiler(
    evmc_host_interface const *host, evmc_host_context *context,
    kinet_eth_revision rev, evmc_message const *msg, uint8_t const *code,
    size_t code_size)
{
    auto code_hash = host->get_code_hash(context, &msg->code_address);
    auto const &[icode, ncode] =
        get_intercode_nativecode(rev, code_hash, code, code_size);

    if (base_config.runtime_debug_trace) {
        std::cout << "Address " << to_hex(msg->code_address) << " => Hash "
                  << to_hex(code_hash) << std::endl;
    }

    KINET_ASSERT(ncode->entrypoint() != nullptr)
    SWITCH_EVM_TRAITS(
        kinet_vm_.execute_native_entrypoint_raw, *rt_ctx_, ncode->entrypoint());
    KINET_ABORT();
}

evmc::Result BlockchainTestVM::execute_interpreter(
    evmc_host_interface const *host, evmc_host_context *context,
    kinet_eth_revision rev, evmc_message const *msg, uint8_t const *code,
    size_t code_size)
{
    auto code_hash = host->get_code_hash(context, &msg->code_address);
    auto const &icode = get_intercode(code_hash, code, code_size);
    SWITCH_EVM_TRAITS(kinet_vm_.execute_intercode_raw, *rt_ctx_, icode);
    KINET_ABORT();
}
