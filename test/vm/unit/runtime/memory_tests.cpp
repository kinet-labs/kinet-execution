...
namespace
{
    ...
    class MemoryTestMachine
    {
        ...
        kinet::vm::test::MockedHost &host_;

    public:
        MemoryTestMachine(kinet::vm::test::MockedHost &host)
            : prev_rt_ctx_{}
            , host_{host}
        {
        }
        ...
        void call(std::function<void(Context &)> const continuation)
        {
            ...
            if (prev_rt_ctx_) {
                ...
                KINET_ASSERT(m.size <= m.capacity);
            }
            else {
    ...
    {
        KINET_ASSERT(config.pre_calls.size() == config.depth);
        KINET_ASSERT(config.post_calls.size() == config.depth);
        if (config.depth == 0) {
            return;
        }
    ...
    void run_memory_test_machine(
        kinet::vm::test::MockedHost &host, MemoryTestMachineConfig config)
    {
...
void test_expand_memory_impl(Context &ctx, std::function<void(Bin<29>)> expand)
{
    ...
    if (memory_version == Memory::Version::MIP3) {
        ...
        kinet_vm_runtime_increase_memory_mip3(
...
TEST_F(RuntimeTest, RuntimeIncreaseMemoryV1)
{
    ...
    test_expand_memory_impl<memory_version>(
        this->ctx_, [this](Bin<29> const s) {
            kinet_vm_runtime_increase_memory_v1(s, &ctx_);
        });
}
...
TEST_F(RuntimeTest, RuntimeIncreaseMemoryMIP3)
{
    ...
    test_expand_memory_impl<memory_version>(
        this->ctx_, [this](Bin<29> const s) {
            kinet_vm_runtime_increase_memory_mip3(s, &ctx_);
        });
}

TEST_F(RuntimeTest, MemoryTestMachineV1)
{
    using traits = KinetTraits<KINET_EIGHT>;
...
{
    using traits = KinetTraits<KINET_NINE>;
    for (auto const &config : memory_test_machine_configs()) {
        run_memory_test_machine<traits>(RuntimeTestBase::host_, config);
    }
}
