...
using kinet::EvmTraits;
using kinet::load_be_unsafe;
...
// https://github.com/category-labs/kinet-compiler/issues/138
...
// https://github.com/category-labs/kinet-compiler/issues/190
...
// https://github.com/category-labs/kinet-compiler/issues/192
...
{
    static_assert(TestFixture::Trait::evm_rev() >= KINET_ETH_CONSTANTINOPLE);
...
//   `kinet-integration/tests/test_contract_interaction/example.sol`
//
...
// the client to use the Kinet VM before out of gas reporting was re-enabled.
...
namespace
{
    std::vector<std::variant<kinet_eth_revision, kinet_revision>>
    kinet_evm_revisions()
    {
        std::vector<std::variant<kinet_eth_revision, kinet_revision>> result;
        ...
        for (auto evm_rev = 0;
             evm_rev <= kinet::constants::LATEST_SUPPORTED_EVM_FORK;
             ++evm_rev) {
            result.push_back(static_cast<kinet_eth_revision>(evm_rev));
        }
        for (auto kinet_rev = 0; kinet_rev <= KINET_NEXT; ++kinet_rev) {
            result.push_back(static_cast<kinet_revision>(kinet_rev));
        }
        return result;
    }
    ...
    static auto regression_tests =
        std::views::cartesian_product(
            fs::directory_iterator(kinet::test_resource::regression_tests_dir),
            kinet_evm_revisions()) |
        std::ranges::to<std::vector>();

    std::string kinet_evm_revision_name(
        std::variant<kinet_eth_revision, kinet_revision> rev)
    {
        std::string name;
        if (std::holds_alternative<kinet_revision>(rev)) {
            name = kinet_revision_to_string(std::get<kinet_revision>(rev));
        }
        else {
            name =
                kinet_eth_revision_to_string(std::get<kinet_eth_revision>(rev));
        }
...
INSTANTIATE_TEST_SUITE_P(
    , VMFileTest, ::testing::ValuesIn(regression_tests), ([](auto const &info) {
        ...
        return kinet_evm_revision_name(rev) + "_" + f.path().stem().string();
    }));
