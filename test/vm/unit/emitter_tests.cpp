...
namespace
{
    ...
    std::string new_emitter_asm_log_path()
    {
        ...
        return std::string(std::format(
            "/tmp/kinet_vm_test_logs/emitter_test_{}.s", test_emitter_ix));
    }
    ...
    struct TestEmitter : Emitter
    {
        ...
        {
            if (!c.asm_log_path &&
                kinet::vm::compiler::test::params.dump_asm_on_failure) {
        ...
        entrypoint_t finish_contract(asmjit::JitRuntime &rt)
        {
            ...
            flush_debug_logger();
            if (kinet::vm::compiler::test::params.dump_asm_on_failure) {
...
TEST(Emitter, exp)
{
...

    {
        ...
        for (int i = 0; i < rep_count; ++i) {
            ...
            emit.exp<EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>(
    ...
    for (auto const &b : bases) {
        for (auto const &e : exps) {
            pure_bin_instr_test(
                ...
                EXP,
                [&](Emitter &em) {
                    (em.exp<EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>(
                        std::numeric_limits<int32_t>::max()));
                },
                {b},
                {e},
            ...
            dynamic_gas_test(
                ...
                runtime::exp_dynamic_gas_cost_multiplier<
                    EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>() *
...
TEST(Emitter, clz)
{
    ...
    // Test zero case
    pure_una_instr_test<EvmTraits<KINET_ETH_OSAKA>>(
...
TEST(Emitter, runtime_exit)
{
    ...
    emit.push(10);
    emit.call_runtime(
        9, true, runtime::exp<EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>);
    emit.return_();
