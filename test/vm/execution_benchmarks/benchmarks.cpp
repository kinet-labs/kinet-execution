...
using namespace kinet::test_resource;
...
using kinet::vm::test::TestMemory;
...
namespace
{
    auto vm_performance_dir = kinet::test_resource::ethereum_tests_dir /
    ...
    {
        KINET_DEBUG_ASSERT(fs::is_directory(path));

        auto const contract_path = path / "contract";
        KINET_DEBUG_ASSERT(fs::is_regular_file(contract_path));

        auto const calldata_path = path / "calldata";
        KINET_DEBUG_ASSERT(fs::is_regular_file(calldata_path));
    ...
    void precompile_contract(
        BlockchainTestVM *vm_ptr, kinet_eth_revision rev,
        ...
        std::vector<uint8_t> const &code)
    {
        ...
        auto const test_state = json_state.make_test_state();
        vm::VM kinet_vm;
        kinet_vm.debug_set_execute_override(to_execute_override(vm));
    ...
    void touch_init_state(JsonState const &json_state, kinet::State &state)
    {
        for (auto const &addr : json_state.initial_accounts()) {
            state.touch(addr);
            state.get_code(addr);
