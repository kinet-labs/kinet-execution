...
namespace
{
    ...
    // clang-format on
    {
        ...
        KINET_ASSERT(
            program.size() <= *kinet::vm::interpreter::code_size_t::max());
        auto rt = asmjit::JitRuntime{};

        for (auto _ : state) {
            auto fn = kinet::vm::compiler::native::compile<
                kinet::EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>(
                rt,
                program.data(),
                kinet::vm::interpreter::code_size_t::unsafe_from(
    ...
    void run_benchmark(benchmark::State &state, fs::path const &evm_code)
    {
        ...
        auto program = kinet::vm::utils::parse_hex_program(buffer);
        KINET_ASSERT(
            program.size() <= *kinet::vm::interpreter::code_size_t::max());
        ...
        for (auto _ : state) {
            auto ncode = kinet::vm::compiler::native::compile<
                kinet::EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>(
                rt,
                program.data(),
                kinet::vm::interpreter::code_size_t::unsafe_from(
    ...
    {
        return std::array{
            kinet::test_resource::compile_benchmarks_dir / "usdt",
            kinet::test_resource::compile_benchmarks_dir / "stop",
            kinet::test_resource::compile_benchmarks_dir / "uniswap",
            kinet::test_resource::compile_benchmarks_dir / "uniswap_v3",
        };
    }
