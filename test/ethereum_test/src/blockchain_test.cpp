...
std::string_view blob_schedule_name(kinet_eth_revision const rev)
{
    if (rev >= KINET_ETH_OSAKA) {
        return "Osaka";
    }
    if (rev >= KINET_ETH_PRAGUE) {
        return "Prague";
    }
    return "Cancun";
}
...
struct TraitsMainnet : KinetChain
{
    ...
    virtual uint256_t get_chain_id() const override
    {
        if constexpr (is_evm_trait_v<traits>) {
            ...
            return KinetMainnet{}.get_chain_id();
        }
    }

    virtual kinet_eth_revision get_revision(
    ...
    virtual BlobSchedule get_blob_schedule(uint64_t timestamp) const override
    {
        if constexpr (is_evm_trait_v<traits>) {
            ...
            return KinetChain::get_blob_schedule(timestamp);
        }
    }
    ...
    virtual GenesisState get_genesis_state() const override
    {
        if constexpr (is_evm_trait_v<traits>) {
            ...
            return KinetMainnet{}.get_genesis_state();
        }
    }
...
void validate_post_state(nlohmann::json const &json, nlohmann::json const &db)
{
    ...
    for (auto const &[addr, j_account] : json.items()) {
        ...
        auto const code = j_account.contains("code")
                              ? j_account.at("code").get<kinet::byte_string>()
                              : kinet::byte_string{};
...
Result<BlockExecOutput> execute(
    ...
    kinet::Db &db, vm::VM &vm, BlockHashBuffer const &block_hash_buffer,
    ...
    ExecutionEventRecorder *const exec_recorder)
{
    ...
    KINET_ASSERT(db.is_page_encoded() == traits::mip_8_active());
...
void process_test(
    ...
    kinet_event_ring const *const exec_event_ring)
{
    static_assert(traits::evm_rev() >= KINET_ETH_BYZANTIUM);
    ...
    for (auto const &j_block : j_contents.at("blocks")) {
        ...
        if (exec_event_ring != nullptr) {
            ...
            kinet_event_iterator iter;
            ASSERT_EQ(
                kinet_event_ring_init_iterator(exec_event_ring, &iter), 0);
            ASSERT_TRUE(kinet_exec_iter_block_number_prev(
                &iter,
                exec_event_ring,
                curr_block_number,
...
void process_test(
    std::variant<kinet_eth_revision, kinet_revision> const &revision,
    std::string const &name, nlohmann::json const &j_contents,
    vm::VM::Mode const vm_mode, bool const enable_tracing,
    kinet_event_ring const *const exec_event_ring)
{
    if (std::holds_alternative<kinet_eth_revision>(revision)) {
        auto const rev = std::get<kinet_eth_revision>(revision);
        SWITCH_EVM_TRAITS(
            process_test,
            name,
            j_contents,
        ...
        auto const rev = std::get<kinet_revision>(revision);
        SWITCH_KINET_TRAITS(
            process_test,
            name,
            j_contents,
...
void BlockchainTest::SetUpTestSuite()
{
    ...
    ASSERT_TRUE(kinet::init_trusted_setup());
}
    ...
    kinet_event_ring const *const exec_event_ring)
{
    ...
    KINET_ASSERT(fs::exists(root));

    auto register_test =
        [&root, &revision, vm_mode, enable_tracing, exec_event_ring](
            fs::path const &path) {
            if (path.extension() == ".json") {
                KINET_ASSERT(fs::is_regular_file(path));
    ...
    if (fs::is_directory(root)) {
        ...
        KINET_ASSERT(fs::is_regular_file(root));
        register_test(root);
    }
}
