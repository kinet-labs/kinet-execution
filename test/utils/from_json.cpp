...
kinet::BlockHeader read_genesis_blockheader(nlohmann::json const &genesis_json)
{
    ...
    KINET_ASSERT(extra_data.has_value());
    ...
    KINET_ASSERT(mix_hash_byte_string.has_value());
    std::copy_n(
    ...
    KINET_ASSERT(parent_hash_byte_string.has_value());
    std::copy_n(
    ...
    if (genesis_json.contains("coinbase")) {
        ...
        KINET_ASSERT(coinbase.has_value());
        std::copy_n(
    ...
    if (genesis_json.contains("parentBeaconBlockRoot")) {
        ...
        KINET_ASSERT(parent_beacon_block_root.has_value());
        auto &write_to =
    ...
    if (genesis_json.contains("requestsHash")) {
        ...
        KINET_ASSERT(requests_hash.has_value());
...
namespace nlohmann
{
    ...
    void
    adl_serializer<kinet::State>::from_json(json const &j, kinet::State &state)
    {
        for (auto const &[j_addr, j_acc] : j.items()) {
            auto const account_address =
                kinet::from_hex<kinet::Address>(j_addr).value();
            ...
            if (j_acc.contains("code")) {
                state.set_code(
                    account_address,
                    j_acc.at("code").get<kinet::byte_string>());
            }

            state.add_to_balance(
                account_address, j_acc.at("balance").get<kinet::uint256_t>());
            ...
            if (j_acc.contains("storage")) {
                KINET_ASSERT(j_acc["storage"].is_object());
                for (auto const &[key, value] : j_acc["storage"].items()) {
                    nlohmann::json const key_json = key;
                    kinet::bytes32_t const key_bytes32 =
                        key_json.get<kinet::bytes32_t>();
                    kinet::bytes32_t const value_bytes32 = value;
                    if (value_bytes32 == kinet::bytes32_t{}) {
