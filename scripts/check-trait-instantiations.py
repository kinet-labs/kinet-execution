...
- EXPLICIT_EVM_TRAITS, EXPLICIT_KINET_TRAITS, EXPLICIT_TRAITS (functions)
- EXPLICIT_EVM_TRAITS_CLASS, EXPLICIT_KINET_TRAITS_CLASS, EXPLICIT_TRAITS_CLASS (classes)
- EXPLICIT_EVM_TRAITS_MEMBER, EXPLICIT_KINET_TRAITS_MEMBER, EXPLICIT_TRAITS_MEMBER (members)

It checks for overlaps according to these rules:

1. Use of `EXPLICIT_EVM_TRAITS*` in one TU and either `EXPLICIT_KINET_TRAITS*`
...
3. It's explicitly OK to use `EXPLICIT_KINET_TRAITS*` and `EXPLICIT_EVM_TRAITS*`
...
def find_macro_usages(root_path: str) -> List[MacroUsage]:
    ...
    pattern = re.compile(
        r"^\s*(EXPLICIT_EVM_TRAITS(?:_CLASS|_MEMBER)?|EXPLICIT_KINET_TRAITS(?:_CLASS|_MEMBER)?|EXPLICIT_TRAITS(?:_CLASS|_MEMBER)?)\s*\(\s*([^)]+)\s*\)",
        re.MULTILINE,
    )
...
def check_overlaps(
    ...
    for argument, arg_usages in by_argument.items():
        ...
        # (EXPLICIT_KINET_TRAITS or EXPLICIT_EVM_TRAITS) in another TU
        files_with_evm = set()
        files_with_kinet = set()
        ...
        for file_path, file_usages in by_file.items():
            ...
            has_kinet = any(
                u.macro_name
                in (
                    "EXPLICIT_KINET_TRAITS",
                    "EXPLICIT_KINET_TRAITS_CLASS",
                    "EXPLICIT_KINET_TRAITS_MEMBER",
                    "EXPLICIT_TRAITS",
                    "EXPLICIT_TRAITS_CLASS",
                    "EXPLICIT_TRAITS_MEMBER",
                )
            ...
            if has_kinet:
                files_with_kinet.add(file_path)
        ...
        # Check if EXPLICIT_KINET_TRAITS (or EXPLICIT_TRAITS) is used in multiple files
        # This also violates Rule 1
        if len(files_with_kinet) > 1:
            rel_paths = [get_relative_path(f, root_path) for f in files_with_kinet]
            errors.append(
                f"OVERLAP: {argument} has Kinet traits instantiated in multiple TUs: "
                f"{', '.join(rel_paths)}"
            )
...
def main():
    ...
    if args.root_path:
        ...
        # Default to the kinet repository root
