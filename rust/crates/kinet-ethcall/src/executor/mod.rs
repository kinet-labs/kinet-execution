...
pub(super) const EVMC_KINET_RESERVE_BALANCE_VIOLATION: i32 = 18;
...
#[derive(Debug)]
pub struct KinetExecutor {
    eth_call_executor: *mut ffi::kinet_executor,
}

unsafe impl Send for KinetExecutor {}
unsafe impl Sync for KinetExecutor {}
...
impl Drop for KinetExecutor {
    fn drop(&mut self) {
        info!("dropping eth_call_executor");
        unsafe {
            ffi::kinet_executor_destroy(self.eth_call_executor);
        }
        info!("eth_call_executor successfully destroyed");
    }
}

pub(crate) struct KinetExecutorResult {
    inner: NonNull<ffi::kinet_executor_result>,
}

impl KinetExecutorResult {
    pub fn from_c_handle(c_handle: *mut ffi::kinet_executor_result) -> Option<Self> {
...
impl Drop for KinetExecutorResult {
    fn drop(&mut self) {
        unsafe {
            ffi::kinet_executor_result_release(self.inner.as_ptr());
        }
    }
}
