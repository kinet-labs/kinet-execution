...
use super::{
    decode_revert_message, EthCallResult, MessageError, KinetExecutor, KinetExecutorResult,
    ETH_CALL_SUCCESS, EVMC_KINET_RESERVE_BALANCE_VIOLATION, EVMC_OUT_OF_GAS,
};
use crate::{
    ffi,
...
pub struct EthCallSenderContext {
    sender: futures::channel::oneshot::Sender<*mut ffi::kinet_executor_result>,
    state_override_ctx: *mut std::ffi::c_void,
}

/// # Safety
/// This should be used only as a callback for kinet_eth_call_executor_submit
///
...
pub unsafe extern "C" fn eth_call_submit_callback(
    result: *mut ffi::kinet_executor_result,
    ...
    if user.sender.send(result).is_err() && !result.is_null() {
        unsafe { ffi::kinet_executor_result_release(result) };
    }
    ...
    if !user.state_override_ctx.is_null() {
        unsafe { ffi::kinet_state_override_destroy(user.state_override_ctx as *mut _) };
    }
}
...
impl KinetExecutor {
    pub async fn eth_call(
        ...
        let override_ctx = unsafe { ffi::kinet_state_override_create() };
        ...
        unsafe {
            ffi::kinet_executor_eth_call_submit(
        ...
        let result = KinetExecutorResult::from_c_handle(result_raw).ok_or_else(|| {
        ...
        match result.status_code() {
            ETH_CALL_SUCCESS => {
                let output_data = if tracer_cval == ffi::kinet_tracer_config_NOOP_TRACER {
            ...
            EVMC_KINET_RESERVE_BALANCE_VIOLATION => {
                if tracer_cval == ffi::kinet_tracer_config_NOOP_TRACER {
