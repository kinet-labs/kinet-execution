...
use super::{MessageError, KinetExecutor, KinetExecutorResult, ETH_CALL_SUCCESS};
use crate::{
    ffi,
    overrides::{
...
/// # Safety
/// This should be used only as a callback for kinet_executor_eth_simulate_submit
///
...
pub unsafe extern "C" fn eth_simulate_v1_submit_callback(
    ...
    if !user.state_override_vec_ctx.is_null() {
        unsafe { ffi::kinet_state_override_vec_destroy(user.state_override_vec_ctx as *mut _) };
    }
    if !user.block_override_vec_ctx.is_null() {
        unsafe { ffi::kinet_block_override_vec_destroy(user.block_override_vec_ctx as *mut _) };
    }
}
...
impl KinetExecutor {
    ...
    pub async fn eth_simulate_v1(
        ...
        unsafe {
            ffi::kinet_executor_eth_simulate_submit(
        ...
        let Some(result) = KinetExecutorResult::from_c_handle(result_raw) else {
