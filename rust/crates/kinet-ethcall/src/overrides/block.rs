...
pub(crate) struct CBlockOverrideVec {
    c_handle: NonNull<ffi::kinet_block_override_vec>,
}

impl CBlockOverrideVec {
    pub(crate) fn with_capacity(size: usize) -> Option<Self> {
        NonNull::new(unsafe { ffi::kinet_block_override_vec_create(size) })
            .map(|c_handle| Self { c_handle })
    }

    pub(crate) fn as_mut_ptr(&mut self) -> *mut ffi::kinet_block_override_vec {
...
impl Drop for CBlockOverrideVec {
    fn drop(&mut self) {
        unsafe {
            ffi::kinet_block_override_vec_destroy(self.c_handle.as_ptr());
        }
    }
}
