...
pub(crate) struct CStateOverrideVec {
    c_handle: NonNull<ffi::kinet_state_override_vec>,
}

impl CStateOverrideVec {
    pub(crate) fn with_capacity(size: usize) -> Option<Self> {
        NonNull::new(unsafe { ffi::kinet_state_override_vec_create(size) })
            .map(|c_handle| Self { c_handle })
    }

    pub(crate) fn as_mut_ptr(&mut self) -> *mut ffi::kinet_state_override_vec {
...
impl Drop for CStateOverrideVec {
    fn drop(&mut self) {
        unsafe {
            ffi::kinet_state_override_vec_destroy(self.c_handle.as_ptr());
        }
    }
}
