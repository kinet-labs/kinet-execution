...
//! Idiomatic and safe types for interacting with `kinet` execution event rings.
//!
//! # Quickstart
//!
//! Add `kinet-event-ring` and `kinet-exec-events` as dependencies to your Cargo.toml.
//!
//! ```custom
//! cargo add --git https://github.com/category-labs/kinet kinet-event-ring kinet-exec-events
//! ```
//!
...
//! # };
//! # use kinet_exec_events::{ExecEventRing};
//! #
...
//! [`kinet_event_ring`](../kinet_event_ring/index.html) library documentation.
//!
//! # Block-Oriented Updates
//!
//! `EventRing`s using an [`ExecEventDecoder`] (primarily [`ExecEventRing`]) can produce individual
//! `kinet` execution events. While many applications may benefit from operating on individual
...
use kinet_event_ring::{EventRing, SnapshotEventRing};
...
/// A type alias for an event ring that produces kinet execution events.
pub type ExecEventRing = EventRing<ExecEventDecoder>;

/// A type alias for a snapshot event ring that produces kinet execution events.
