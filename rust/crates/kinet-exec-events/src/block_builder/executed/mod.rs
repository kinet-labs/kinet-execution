...
use kinet_event_ring::{EventDescriptor, EventPayloadResult};
...
use crate::{
    ffi::{
        kinet_c_access_list_entry, kinet_c_auth_list_entry, kinet_c_bytes32,
        kinet_exec_txn_access_list_entry, kinet_exec_txn_auth_list_entry,
        kinet_exec_txn_evm_output, kinet_exec_txn_header_start,
    },
    ...
    fn process_exec_event(
        ...
        match exec_event {
            ...
            ExecEvent::BlockEnd(block_result) => {
                ...
                Some(Ok(ExecutedBlock {
                    ...
                    txns: txns
                        ...
                        .into_iter()
                        .map(|txn_opt| {
                            txn_opt.expect("ExecutedBlockBuilder received TxnHeaderStart for txn")
                        })
                        .map(
                            |TxnReassemblyState {
                                 hash,
            ...
            ExecEvent::TxnHeaderStart {
                ...
                let kinet_exec_txn_header_start {
                    txn_hash,
                    sender,
                    txn_header,
                } = txn_start;
                ...
                let txn_ref = state
                    ...
                    .expect("ExecutedBlockBuilder TxnHeaderStart txn_index within bounds");
            ...
            ExecEvent::TxnAccessListEntry {
                ...
                let txn_ref = state
                    ...
                    .as_mut()
                    .expect("ExecutedBlockBuilder TxnAccessListEntry txn_index populated from preceding TxnHeaderStart");
                ...
                let storage_keys = storage_key_bytes
                    .into_vec()
                    .into_iter()
                    .chunks(std::mem::size_of::<kinet_c_bytes32>())
                    .into_iter()
                    .map(|chunk| kinet_c_bytes32 {
            ...
            ExecEvent::TxnAuthListEntry {
                ...
                let txn_ref = state
                    ...
                    .as_mut()
                    .expect("ExecutedBlockBuilder TxnAuthListEntry txn_index populated from preceding TxnHeaderStart");
            ...
            ExecEvent::TxnEvmOutput {
                ...
                let txn_ref = state
                    ...
                    .as_mut()
                    .expect("ExecutedBlockBuilder TxnReceipt txn_index populated from preceding TxnHeaderStart");
            ...
            ExecEvent::TxnLog {
                ...
                let txn_ref = state
                    ...
                    .as_mut()
                    .expect(
                        "ExecutedBlockBuilder TxnLog txn_index populated from preceding TxnHeaderStart",
                    );
            ...
            ExecEvent::TxnCallFrame {
                ...
                let txn_ref = state
                    ...
                    .as_mut()
                    .expect(
                        "ExecutedBlockBuilder TxnCallFrame txn_index populated from preceding TxnHeaderStart",
                    );
            ...
            ExecEvent::EvmError(kinet_exec_evm_error) => {
                let _state = self.state.as_mut()?;

                unimplemented!("EvmError {kinet_exec_evm_error:#?}");
            }
        }
    }
}
...
mod test {
    ...
    #[ignore]
    #[test]
    fn basic_test_kinet_testnet() {
        const SNAPSHOT_NAME: &str = "KINET_DEVNET_500B_GENESIS";
        const SNAPSHOT_ZSTD_BYTES: &[u8] =
