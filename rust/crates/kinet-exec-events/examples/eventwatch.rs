...
use kinet_exec_events::{
    ffi::{g_kinet_exec_event_metadata, DEFAULT_FILE_NAME, KINET_EXEC_EVENT_COUNT},
    ExecEventDecoder, ExecEventDescriptorExt, ExecEventReaderExt, ExecEventRing, ExecEventType,
    ExecSnapshotEventRing,
};

lazy_static! {
    static ref EXEC_EVENT_NAMES: [&'static str; KINET_EXEC_EVENT_COUNT] =
        std::array::from_fn(|event_type| unsafe {
            CStr::from_ptr(g_kinet_exec_event_metadata[event_type].c_name)
                .to_str()
                .unwrap()
        });
}
...
//      need to be running an active kinet node to use them
//
