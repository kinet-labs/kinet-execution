...
use kinet_event_ring::{DecodedEventRing, EventNextResult, EventRingPath};
use kinet_exec_events::{
    ffi::{kinet_exec_block_end, kinet_exec_block_start, kinet_exec_block_tag},
...
#[derive(Debug, Parser)]
#[command(name = "kinet-exec-events-explorer", about, long_about = None)]
pub struct Cli {
    #[arg(long)]
...
impl App {
    ...
    pub fn move_forward(&mut self) {
        match self.view {
            ...
            View::Block => match self.block_tab {
                BlockTab::Header => {
                    self.block_tab = BlockTab::ExecutionResult;
                }
                BlockTab::ExecutionResult => {
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let logo = Paragraph::new("Kinet Labs").centered().block(
            Block::new()
                .fg(Color::Black)
    ...
    fn render_block_header(
        ...
        start: &kinet_exec_block_start,
    ) {
        let kinet_exec_block_start {
            block_tag: kinet_exec_block_tag { id, block_number },
            round,
            epoch,
    ...
    fn render_block_execution_result(
        ...
        area: Rect,
        table_state: &mut TableState,
        block_result: &kinet_exec_block_end,
    ) {
        let kinet_exec_block_end {
            eth_block_hash,
            exec_output,
        } = block_result;
