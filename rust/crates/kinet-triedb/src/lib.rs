...
/// `kinet-mpt` reports. Fully determines each timeline's encoding, so
...
pub enum MigrationPhase {
    ...
    DualTimeline = 1,
    /// Primary kinet, no secondary: migration complete.
    PageEncoded = 2,
    /// Primary kinet, slot secondary kept as pre-cutoff history (archive
...
/// Converts a C `kinet_c_bytes32` sentinel value (all-zeros = not found) to `Option<[u8; 32]>`.
fn parse_triedb_block_id(value: ffi::kinet_c_bytes32) -> Option<[u8; 32]> {
...
impl TriedbHandle {
    ...
    /// True if the primary timeline is page-encoded (Kinet state machine), in
    ...
    /// (kinet-mpt with readers and the daemon stopped), so the answer is
