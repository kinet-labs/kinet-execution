...
use crate::{
    ffi::{kinet_event_descriptor, kinet_event_ring_payload_check, kinet_event_ring_payload_peek},
    EventPayloadResult, RawEventRing,
};

#[derive(Debug)]
pub(crate) struct RawEventDescriptor<'ring> {
    pub(super) inner: kinet_event_descriptor,
...
impl<'ring> RawEventDescriptor<'ring> {
    ...
    pub(super) fn try_filter_map<T>(
        ...
        let Some(bytes) = kinet_event_ring_payload_peek(&self.ring.inner, &self.inner) else {
        ...
        if kinet_event_ring_payload_check(&self.ring.inner, &self.inner) {
