...
use crate::{
    ffi::{kinet_event_descriptor, kinet_event_ring},
    ...
    D: EventDecoder,
{
    ...
    /// `f`'s execution, it is possible for the underlying payload bytes to be partially or
    ...
    pub fn with_raw<R>(
        &self,
        f: impl FnOnce(&kinet_event_ring, &kinet_event_descriptor) -> R,
    ) -> R {
