...
//! Idiomatic and safe APIs for interacting with the `kinet` execution event ring library.
//!
//! # Introduction
//!
//! The `kinet` execution daemon serves a real-time event notification system that informs clients
...
//! API." A general overview of event rings is provided in the `kinet`
//! [execution daemon repository](<https://github.com/category-labs/kinet/tree/main/libs/event/doc/event.md>).
...
//! emit different kinds of events, not just `kinet` execution events. As such, this crate only
//! contains bindings to use the event ring API and **not** the underlying event types themselves.
//! For `kinet` execution event types to interact with the `kinet` execution daemon, see the
//! [`kinet_exec_events`](../kinet_exec_events/index.html) library.
//!
//! ## API Usage Overview
//!
...
//! [`EventNextResult::NotReady`] when there isn't. If another event is written to the event ring
