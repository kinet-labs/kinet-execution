...
impl EventRingPath {
    ...
    pub fn resolve(path: impl AsRef<Path>) -> Result<Self, String> {
        let path = ffi::kinet_event_resolve_ring_path(path)?;
    ...
    pub fn resolve_with_default_path(
        ...
        let path = ffi::kinet_event_resolve_ring_path_with_basepath(Some(basepath), path)?;
    ...
    pub fn is_snapshot_file(&self) -> Result<bool, String> {
        ...
        ffi::kinet_event_is_snapshot_file(&file, self.as_error_name())
    }
