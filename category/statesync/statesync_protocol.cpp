...
bytes32_t read_storage(
    kinet_statesync_client_context &ctx, Address const &addr,
    bytes32_t const &key)
{
...
{
    using StorageDeltas = kinet_statesync_client_context::StorageDeltas;
    ...
    if (ctx.buffered.contains(addr)) {
        KINET_ASSERT(!ctx.tdb.read_account(addr).has_value() && !updated);
        if (acct.has_value()) {
            KINET_ASSERT(
                ctx.deltas
                    .emplace(
        ...
        if (acct.has_value()) {
            ...
            KINET_ASSERT(ctx.deltas.emplace(addr, std::nullopt).second);
        }
    }
    // incarnation
    ...
    if (ctx.buffered.contains(addr)) {
        ...
        if (val == bytes32_t{}) {
            ...
            if (sit != ctx.buffered[addr].end()) {
                ...
                KINET_ASSERT(ctx.buffered[addr].emplace(key, val).second);
            }
        }
    }
    else if (
...
void StatesyncProtocolV1_2::send_request(
    kinet_statesync_client_context *const ctx, uint64_t const prefix) const
{
    auto const tgrt = ctx->tgrt.number;
    auto const &[progress, old_target] = ctx->progress[prefix];
    KINET_ASSERT(progress == INVALID_BLOCK_NUM || progress < tgrt);
    KINET_ASSERT(old_target == INVALID_BLOCK_NUM || old_target <= tgrt);
    auto const from = progress == INVALID_BLOCK_NUM ? 0 : progress + 1;
    ctx->statesync_send_request(
        ctx->sync,
        kinet_sync_request{
            .prefix = prefix,
            .prefix_bytes = kinet_statesync_client_prefix_bytes(),
            .target = tgrt,
            .from = from,
...
bool StatesyncProtocolV1_2::handle_upsert(
    kinet_statesync_client_context *const ctx, kinet_sync_type const type,
    unsigned char const *const val, uint64_t const size) const
{
    ...
    if (type == SYNC_TYPE_UPSERT_CODE) {
        ...
        KINET_ASSERT(type == SYNC_TYPE_UPSERT_HEADER);
