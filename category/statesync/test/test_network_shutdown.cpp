...
namespace
{
    ...
    struct TempDb
    {
            ...
            , path{"/proc/self/fd/" + std::to_string(fd)}
        {
            ...
            // after this point.
            (void)kinet::mpt::Db{
                std::make_unique<kinet::OnDiskMachine>(),
                kinet::mpt::OnDiskDbConfig{
...
TEST(StateSyncThread, shutdown_via_jthread_stop_token)
{
    ...
    kinet::mpt::AsyncIOContext io_ctx{
        kinet::mpt::ReadOnlyOnDiskDbConfig{.dbname_paths = {db_file.path}}};
    kinet::mpt::Db db{io_ctx};
    kinet::TrieDb triedb(db);
    ...
    std::unique_ptr<kinet::StateSyncServer> sync_server =
        kinet::make_statesync_server(kinet::StateSyncServerConfig{
            .triedb = &triedb,
            .network = &*net,
