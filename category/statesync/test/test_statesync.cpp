...
namespace kinet::mpt::test
{
    ...
    // state_machine_kind on a fresh pool, simulating what kinet-mpt --create
...
namespace
{
    ...
    std::filesystem::path tmp_dbname()
    {
        ...
        // kinet_statesync_client_context uses internally — finds a valid kind.
        mpt::Db db{
    ...
    void handle_target(
        kinet_statesync_client_context *const ctx, BlockHeader const &hdr)
    {
        auto const rlp = rlp::encode_block_header(hdr);
        kinet_statesync_client_handle_target(ctx, rlp.data(), rlp.size());
    }
        ...
        size_t const len)
    {
        if (len == 1) {
            ...
            EXPECT_EQ(len, sizeof(kinet_sync_request));
            std::memcpy(
                buf, &net->client->rqs.front(), sizeof(kinet_sync_request));
    ...
    struct StateSyncFixtureT : public ::testing::Test
    {
        ...
        // timestamp changes or KINET_NEXT is promoted to a concrete revision.
        // Pre-mip8: testnet at timestamp 0 maps to KINET_ZERO (slot canonical).
        static constexpr RevisionConfig PRE_MIP8{CHAIN_CONFIG_KINET_TESTNET, 0};
        // Post-mip8: devnet currently maps to KINET_NEXT for any timestamp
        static constexpr RevisionConfig POST_MIP8{CHAIN_CONFIG_KINET_DEVNET, 0};
        ...
        {
            KINET_ASSERT(MIP_8_ACTIVE == stdb.is_page_encoded());
            sctx.ro = &ro;
            ...
            KINET_ASSERT(primary.timeline_active(mpt::timeline_id::secondary));
        }

        void init()
        {
            // Production C ABI (kinet_statesync_client_context_create) does
            ...
            cctx = new kinet_statesync_client_context{
        ...
        kinet_revision get_kinet_revision() const
        {
            return make_kinet_chain(revision_config.chain_config)
                ->get_kinet_revision(revision_config.timestamp);
        }
    ...
    void commit_block_dispatch(
        kinet::Db &primary, kinet::Db *const secondary,
        kinet_revision const rev, bytes32_t const &block_id,
    ...
    bytes32_t commit_simple_revision_aware(
        kinet::Db &primary, kinet::Db *const secondary,
        kinet_revision const rev, StateDeltas const &deltas, Code const &code,
...
TYPED_TEST(StateSyncTestBothForks, sync_from_latest)
{
    ...
    handle_target(
        ...
        BlockHeader{
            ...
            .number = N});
    EXPECT_TRUE(kinet_statesync_client_has_reached_target(this->cctx));
    EXPECT_TRUE(kinet_statesync_client_finalize(this->cctx));
}
...
TEST_F(StateSyncFixture, sync_from_empty)
{
    ...
    run();
    EXPECT_TRUE(kinet_statesync_client_has_reached_target(cctx));
    EXPECT_TRUE(kinet_statesync_client_finalize(cctx));
...
{
    {
        ...
        KINET_ASSERT(db2_opt.has_value());
...
TEST_F(StateSyncFixture, pre_fork_dual_timeline_server_to_dual_db_client)
{
    mpt::Db secondary_sdb =
        sdb.activate_secondary_timeline(std::make_unique<KinetOnDiskMachine>());
    ...
    kinet_revision const rev = cctx->chain->get_kinet_revision(timestamp);
    ...
    PageServerStateSyncFixture, post_fork_sync_page_primary_to_dual_db_client)
{
    ...
    kinet_revision const rev = this->cctx->chain->get_kinet_revision(timestamp);
...
TEST(ProtocolValidation, upserts_reject_trailing_bytes)
{
    ...
    auto const dbname = tmp_dbname();
    {
        kinet::register_ethereum_state_machines();
        kinet_statesync_client client;
        kinet_statesync_client_context ctx{
            CHAIN_CONFIG_KINET_TESTNET,
            {dbname},
            std::nullopt,
            4,
            &client,
...
{
    EXPECT_EQ(KINET_STATESYNC_VERSION_1_2, 0x00010002u);
}

TEST(StatesyncVersion, compatibility_range)
{
    EXPECT_EQ(kinet_statesync_version(), KINET_STATESYNC_VERSION);
    EXPECT_TRUE(kinet_statesync_client_compatible(KINET_STATESYNC_VERSION_MIN));
    EXPECT_TRUE(kinet_statesync_client_compatible(KINET_STATESYNC_VERSION_1_2));
    // 0x00010001 is v1, which peers no longer speak.
    EXPECT_FALSE(
        kinet_statesync_client_compatible(KINET_STATESYNC_VERSION_MIN - 1));
    EXPECT_FALSE(
        kinet_statesync_client_compatible(KINET_STATESYNC_VERSION + 1));
}
