...
kinet_statesync_client_context::kinet_statesync_client_context(
    ...
    kinet_statesync_client *const sync,
    void (*statesync_send_request)(
        struct kinet_statesync_client *, struct kinet_sync_request))
    : chain{make_kinet_chain(chain_config)}
    ...
    , secondary_db{[this] {
        if (db.timeline_active(timeline_id::secondary)) {
            ...
            KINET_ASSERT(ret.has_value());
            KINET_ASSERT(
                ret->state_machine_type() == state_machine_kind::kinet);
    ...
    , progress(
          kinet_statesync_client_prefixes(),
          {db.get_latest_version(), db.get_latest_version()})
    , protocol(kinet_statesync_client_prefixes())
...
{
    KINET_ASSERT(db.get_latest_version() == db.get_latest_finalized_version());
    KINET_ASSERT((secondary_db != nullptr) == (secondary_tdb != nullptr));
    KINET_ASSERT(secondary_tdb == nullptr || secondary_tdb->is_page_encoded());
}

void kinet_statesync_client_context::prepare_current_state()
{
    ...
    auto const roll_forward = [&](mpt::Db &target_db, auto &target_tdb) {
        ...
        KINET_ASSERT(finalized_res.has_value());
        KINET_ASSERT(finalized_res.value().node->number_of_children() == 2);
        KINET_ASSERT(target_db.find(dest_root, state_key, current).has_value());
        KINET_ASSERT(target_db.find(dest_root, code_key, current).has_value());
        KINET_ASSERT(dest_root->value() == src_root->value());
        target_tdb.reset_root(dest_root, current);
        KINET_ASSERT(target_db.get_latest_version() == current);
    };
...
void kinet_statesync_client_context::commit()
{
