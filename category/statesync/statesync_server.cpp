...
{
    kinet_statesync_server_context *context;
    kinet_statesync_server_network *net;
    ssize_t (*statesync_server_recv)(
        kinet_statesync_server_network *, unsigned char *, size_t);
    void (*statesync_server_send_upsert)(
        struct kinet_statesync_server_network *, kinet_sync_type,
        unsigned char const *v1, uint64_t size1, unsigned char const *v2,
        uint64_t size2);
    void (*statesync_server_send_done)(
        kinet_statesync_server_network *, kinet_sync_done);
};
...
bool static_validate_request(kinet_sync_request const &rq)
{
...
bool send_deletion(
    kinet_statesync_server *const sync, kinet_sync_request const &rq,
    kinet_statesync_server_context &ctx, uint64_t *const num_upserts,
    ...
    kinet_statesync_server *const sync, kinet_sync_request const rq)
{
    ...
    auto const *io = kinet::async::AsyncIO::thread_instance();
    if (io != nullptr) {
    ...
    struct Traverse final : public TraverseMachine
    {
        ...
        Traverse(
            kinet_statesync_server *const sync, NibblesView const prefix,
        ...
        virtual bool down(unsigned char const branch, Node const &node) override
        {
            ...
            if (node.has_value() && v <= until) {
                auto const send_upsert = [&](kinet_sync_type const type,
                ...
                if (nibble == CODE_NIBBLE) {
                    ...
                    if (depth == HASH_SIZE) {
                        ...
                        KINET_ASSERT(depth == (HASH_SIZE * 2));
    ...
    if (KINET_UNLIKELY(!static_validate_request(rq))) {
        return false;
    }
...
void kinet_statesync_server_handle_request(
    kinet_statesync_server *const sync, kinet_sync_request const rq)
{
...
struct kinet_statesync_server *kinet_statesync_server_create(
    kinet_statesync_server_context *const ctx,
    kinet_statesync_server_network *const net,
    ssize_t (*statesync_server_recv)(
        kinet_statesync_server_network *, unsigned char *, size_t),
    void (*statesync_server_send_upsert)(
        kinet_statesync_server_network *, kinet_sync_type,
        unsigned char const *v1, uint64_t size1, unsigned char const *v2,
        uint64_t size2),
    void (*statesync_server_send_done)(
        kinet_statesync_server_network *, struct kinet_sync_done))
{
    return new kinet_statesync_server(kinet_statesync_server{
        .context = ctx,
        .net = net,
...
void kinet_statesync_server_run_once(struct kinet_statesync_server *const sync)
{
    unsigned char buf[sizeof(kinet_sync_request)];
    if (sync->statesync_server_recv(sync->net, buf, 1) != 1) {
        return;
    }
    KINET_ASSERT(buf[0] == SYNC_TYPE_REQUEST);
    if (sync->statesync_server_recv(
            sync->net, buf, sizeof(kinet_sync_request)) !=
        static_cast<ssize_t>(sizeof(kinet_sync_request))) {
        return;
    }
    auto const &rq = unaligned_load<kinet_sync_request>(buf);
    kinet_statesync_server_handle_request(sync, rq);
}

void kinet_statesync_server_destroy(kinet_statesync_server *const sync)
{
    delete sync;
}
