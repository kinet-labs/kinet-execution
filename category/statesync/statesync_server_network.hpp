...
struct kinet_statesync_server_network
{
    ...
    kinet_statesync_server_network(char const *const path)
        : path{path}
    {
    ...
    ~kinet_statesync_server_network()
    {
...
{
    KINET_ASSERT(v1 != nullptr || size1 == 0);
    KINET_ASSERT(v2 != nullptr || size2 == 0);
    KINET_ASSERT(
...
inline void statesync_server_send_done(
    kinet_statesync_server_network *const net, kinet_sync_done const msg)
{
    [[maybe_unused]] auto const start = std::chrono::steady_clock::now();
