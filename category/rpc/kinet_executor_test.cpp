...
#include <category/rpc/kinet_executor.h>
...
namespace
{
    ...
    {
        kinet_executor_pool_config const conf = {1, 2, max_timeout, 1000};
        unsigned const tx_exec_num_fibers = 10;
        return kinet_executor_create(
            conf,
            conf,
            conf,
            tx_exec_num_fibers,
    ...
    // KinetOnDiskMachine = page-encoded. The executor opens its own RODb on
    ...
    {
        kinet_executor_result *result;
        boost::fibers::promise<void> promise;

        ~callback_context()
        {
            kinet_executor_result_release(result);
        }
    };

    void
    complete_callback(kinet_executor_result *const result, void *const user)
    {
        ...
        bool const gas_specified)
    {
        ...
        auto *state_override = kinet_state_override_create();

        struct callback_context ctx;
        boost::fibers::future<void> f = ctx.promise.get_future();

        kinet_executor_eth_call_submit(
            executor,
            CHAIN_CONFIG_KINET_DEVNET,
            rlp_tx.data(),
            rlp_tx.size(),
            rlp_header.data(),
            rlp_header.size(),
        ...
        CallFrame const expected{
            ...
            .to = ADDR_B,
            .value = 0x10000,
            .gas = gas_specified ? tx.gas_limit : KINET_ETH_CALL_LOW_GAS_LIMIT,
            .gas_used =
                gas_specified ? tx.gas_limit : KINET_ETH_CALL_LOW_GAS_LIMIT,
            .status = KINET_STATUS_SUCCESS,
            .depth = 0,
        ...
        // the final CallFrame is expected. This is because Kinet currently does
        ...
        kinet_state_override_destroy(state_override);
        kinet_executor_destroy(executor);
    }
}

// Storage-reading tests run on both encodings: slot (pre-mip8) and page
// (post-mip8).
using EthCallMachines = ::testing::Types<OnDiskMachine, KinetOnDiskMachine>;
...
{
    using namespace kinet::vm::utils;
...
// Intercode::pad's KINET_ASSERT_THROW.
...
TEST_F(EthCallFixture, loop_out_of_gas)
{
    ...
    auto const icode = kinet::vm::make_shared_intercode(code);
...
TEST_F(EthCallFixture, concurrent_eth_calls)
{
    ...
    std::deque<kinet_state_override *> state_overrides;
    ...
    for (uint64_t b = 200; b < 300; ++b) {
        ...
        auto *const state_override =
            state_overrides.emplace_back(kinet_state_override_create());
...
TEST_F(EthCallFixture, call_trace_with_logs)
{
    ...
    auto const a_icode = kinet::vm::make_shared_intercode(a_code);
    ...
    auto const b_icode = kinet::vm::make_shared_intercode(b_code);
    ...
    auto const c_icode = kinet::vm::make_shared_intercode(c_code);
    ...
    auto const d_icode = kinet::vm::make_shared_intercode(d_code);
...
TEST_F(EthCallFixture, static_precompile_OOG_with_call_trace)
{
    ...
    CallFrame const expected{
        ...
        .status = KINET_STATUS_OUT_OF_GAS,
        .depth = 0,
        .logs = std::vector<CallFrame::Log>{},
    };
...
TEST_F(EthCallFixture, trace_block_with_prestate)
{
    ...
    {
        boost::fibers::future<void> f = prestate_ctx.promise.get_future();
        kinet_executor_run_transactions(
            executor,
            CHAIN_CONFIG_KINET_DEVNET,
            rlp_header.data(),
            rlp_header.size(),
...
TEST_F(EthCallFixture, kinet_executor_run_reserve_balance)
{
    // This test is ported from `test_kinet_chain.cpp` (reserve balance). It
...

    {
        ...
        ChainContext<kinet::KinetTraits<KINET_NEXT>> const chain_context{
            .grandparent_senders_and_authorities =
                grandparent_senders_and_authorities,
        ...
        init_reserve_balance_context<kinet::KinetTraits<KINET_NEXT>>(
            state,
            sender,
            tx,
        ...
        bool const should_revert =
            revert_transaction<kinet::KinetTraits<KINET_NEXT>>(
                sender,
                tx,
                BASE_FEE_PER_GAS,
                0, // transaction index
    ...
    // StateDiff trace
    {
        ...
        // `kinet_executor` returns `EVMC_SUCCESS` if the `Result`-object
...
TEST_F(EthCallFixture, access_list_trace)
{
    ...
    auto const contract_icode = kinet::vm::make_shared_intercode(contract_code);
...
// Requires CHAIN_CONFIG_KINET_DEVNET (KINET_NEXT), which activates
// mip_8_active().
...
TEST_F(EthCallFixture, eth_call_reserve_balance)
{
    ...
    auto const delegated_eoa_icode =
        kinet::vm::make_shared_intercode(delegated_eoa_code);
    ...
    EXPECT_EQ(ctx.result->status_code, EVMC_KINET_RESERVE_BALANCE_VIOLATION);
...
{
    auto *bo = kinet_block_override_create();
    ASSERT_NE(bo, nullptr);
    kinet_block_override_destroy(bo);
}
...
{
    auto *vec = kinet_state_override_vec_create(2);
    ASSERT_NE(vec, nullptr);
    EXPECT_EQ(vec->size, 2u);
    ASSERT_NE(vec->overrides, nullptr);
    kinet_state_override_vec_destroy(vec);
}
...
{
    auto *vec = kinet_block_override_vec_create(2);
    ASSERT_NE(vec, nullptr);
    EXPECT_EQ(vec->size, 2u);
    ASSERT_NE(vec->overrides, nullptr);
    kinet_block_override_vec_destroy(vec);
}
...
TEST_F(EthCallFixture, eth_simulate_v1_simple_transfer)
{
    ...
    auto *const state_override = kinet_state_override_vec_create(1);
    auto *const block_override = kinet_block_override_vec_create(1);
    ...
    kinet_executor_eth_simulate_submit(
        executor,
        CHAIN_CONFIG_KINET_DEVNET,
        rlp_senders.data(),
        rlp_senders.size(),
    ...
    kinet_block_override_vec_destroy(block_override);
    kinet_state_override_vec_destroy(state_override);
    kinet_executor_destroy(executor);
}
...
TEST_F(EthCallFixture, eth_simulate_v1_simple_transfers_multiple_blocks)
{
    ...
    auto *const state_overrides = kinet_state_override_vec_create(2);
    auto *const block_overrides = kinet_block_override_vec_create(2);
    ...
    kinet_block_override_vec_destroy(block_overrides);
    kinet_state_override_vec_destroy(state_overrides);
    kinet_executor_destroy(executor);
}
...
TEST_F(EthCallFixture, eth_simulate_v1_single_call_block_255)
{
    ...
    auto *const so_overrides = kinet_state_override_vec_create(1);
    auto *const bo_overrides = kinet_block_override_vec_create(1);
    ...
    kinet_block_override_vec_destroy(bo_overrides);
    kinet_state_override_vec_destroy(so_overrides);
...
TEST_F(EthCallFixture, eth_simulate_v1_empty_input)
{
    ...
    auto *const so_overrides = kinet_state_override_vec_create(0);
    auto *const bo_overrides = kinet_block_override_vec_create(0);
...
TEST_F(EthCallFixture, eth_simulate_v1_block_override_no_synthetic_gaps)
{
    ...
    auto *const bo_overrides = kinet_block_override_vec_create(256);
    auto *const so_overrides = kinet_state_override_vec_create(256);
...
TEST_F(EthCallFixture, eth_simulate_v1_stress_queue_rejection)
{
    ...
    kinet_executor_pool_config const block_conf = {1, 2, max_timeout, 2};
    kinet_executor_pool_config const default_conf = {1, 2, max_timeout, 1000};
    unsigned const tx_exec_num_fibers = 10;
    auto *executor = kinet_executor_create(
        default_conf,
        default_conf,
        block_conf,
        tx_exec_num_fibers,
    ...
    struct submission
    {
        ...
        kinet_state_override_vec *so;
        kinet_block_override_vec *bo;
    };
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        subs[i]->so = kinet_state_override_vec_create(1);
        subs[i]->bo = kinet_block_override_vec_create(1);
    ...
    for (auto &s : subs) {
        ...
        kinet_block_override_vec_destroy(s->bo);
        kinet_state_override_vec_destroy(s->so);
    }
...
//
// Kinet has a reserve balance of 10 MON (10e18 wei). EOAs are allowed to dip
...
TEST_F(EthCallFixture, eth_simulate_v1_reserve_balance)
{
    ...
    auto *const so = kinet_state_override_vec_create(1);
    auto *const bo = kinet_block_override_vec_create(1);
    ...
    kinet_block_override_vec_destroy(bo);
    kinet_state_override_vec_destroy(so);
    kinet_executor_destroy(executor);
}
...
TEST_F(EthCallFixture, eth_simulate_v1_reserve_balance_chain_context_buffer)
{
    ...
    // Case 1: Sender in parent / grandparent
    {
        ...
        auto *const so = kinet_state_override_vec_create(4);
        auto *const bo = kinet_block_override_vec_create(4);
...
TEST_F(EthCallFixture, eth_simulate_v1_call_types)
{
    ...
    struct CompiledCode
    {
        std::vector<uint8_t> bytecode;
        bytes32_t hash;
        std::shared_ptr<kinet::vm::Intercode const> icode;
    };

    auto const compile = [](auto const &eb) -> CompiledCode {
        KINET_ASSERT(evm_as::validate(eb));
        std::vector<uint8_t> bytecode{};
        evm_as::compile(eb, bytecode);
...
TEST_F(EthCallFixture, eth_simulate_v1_deploy_and_call)
{
    ...
    auto *const so = kinet_state_override_vec_create(3);
    auto *const bo = kinet_block_override_vec_create(3);
...
TYPED_TEST(EthCallEncodingFixture, eth_simulate_v1_time_travel)
{
    ...
    auto *const so = kinet_state_override_vec_create(2);
    auto *const bo = kinet_block_override_vec_create(2);
...
TEST_F(EthCallFixture, eth_simulate_v1_legacy_transactions)
{
    ...
    auto *const state_overrides = kinet_state_override_vec_create(1);
    auto *const block_overrides = kinet_block_override_vec_create(1);
...
TEST_F(EthCallFixture, eth_simulate_v1_gas_limit_enforcement)
{
    ...
    auto const burner_icode = kinet::vm::make_shared_intercode(burner_code);
    ...
    auto *const state_override = kinet_state_override_vec_create(3);
    auto *const block_override = kinet_block_override_vec_create(3);
...
// Tests that withdrawals are rejected on Kinet.
TEST_F(EthCallFixture, eth_simulate_v1_simple_transfer_withdrawals_kinet)
{
    static constexpr Address sender =
        0x00000000000000000000000000000000deadbeef_address;
    static constexpr Address recipient =
    ...
    kinet_executor_eth_simulate_submit(
        executor,
        CHAIN_CONFIG_KINET_DEVNET, // <- Must be configured for Kinet.
        rlp_senders.data(),
        rlp_senders.size(),
        rlp_calls.data(),
        rlp_calls.size(),
    ...
    ASSERT_EQ(ctx.result->status_code, EVMC_INTERNAL_ERROR);
    ASSERT_NE(ctx.result->message, nullptr);
    EXPECT_STREQ(ctx.result->message, "Withdrawals are not supported on Kinet");
...
TEST_F(EthCallFixture, eth_simulate_v1_state_override_graceful_failure)
{
    ...
    auto *const state_override = kinet_state_override_vec_create(2);
    auto *const block_override = kinet_block_override_vec_create(2);
...
// kinet_executor_eth_simulate_submit asserts that decoding succeeds, so
...
TEST_F(EthCallFixture, eth_simulate_v1_transaction_input_too_long_causes_death)
{
    ...
    // KINET_ASSERT, which fires before the executor pointer is ever
    // dereferenced, therefore it suffices to pass a non-null sentinel.
    auto *const sentinel_executor =
        reinterpret_cast<struct kinet_executor *>(uintptr_t{1});
