...
"                        [default: %s]\n",
    KINET_EVENT_DEFAULT_EXEC_FILE_NAME);
    exit(0);
}
...
static void hexdump_event_payload(
    ...
    struct kinet_event_descriptor const *const event, FILE *const out)
{
    ...
    for (uint8_t const *line = payload; line < payload_end; line += 16) {
        ...
        *o++ = '\n';
        ...
        if ((line - payload + 16) % 512 == 0 &&
            !kinet_event_ring_payload_check(event_ring, event)) {
            break; // Escape to the end, which checks the final time
        }
    }

    if (!kinet_event_ring_payload_check(event_ring, event)) {
        fprintf(
            stderr,
    ...
    struct kinet_event_descriptor const *const event, FILE *const out)
{
    ...
    struct kinet_event_metadata const *event_md =
        &g_kinet_exec_event_metadata[event->event_type];
    ...
    if (event->content_ext[KINET_FLOW_BLOCK_SEQNO] != 0) {
        // When `event->content_ext[KINET_FLOW_BLOCK_SEQNO]` is non-zero, it
        // is set to the sequence number of the KINET_EXEC_BLOCK_START event
        ...
        struct kinet_event_descriptor start_block_event;
        struct kinet_exec_block_start const *block_start = nullptr;
        if (kinet_event_ring_try_copy(
                event_ring,
                event->content_ext[KINET_FLOW_BLOCK_SEQNO],
                &start_block_event)) {
            block_start =
                kinet_event_ring_payload_peek(event_ring, &start_block_event);
        }
        if (block_start) {
            uint64_t const block_number = block_start->eth_block_input.number;
            if (kinet_event_ring_payload_check(
    ...
    if (event->content_ext[KINET_FLOW_TXN_ID] != 0) {
        o += sprintf(
            o,
            " TXN: %lu",
            (unsigned long)(event->content_ext[KINET_FLOW_TXN_ID] - 1));
    }
    *o++ = '\n';
    ...
    //            (struct kinet_exec_txn_header_start const *)payload, ...);
    //        break;
    //
...
static void event_loop(
    struct kinet_event_ring const *const event_ring,
    struct kinet_event_iterator *const iter, int const pidfd, FILE *const out)
{
    struct kinet_event_descriptor event;
    uint64_t not_ready_count = 0;

    while (g_should_stop == 0) {
        switch (kinet_event_iterator_try_next(iter, &event)) {
        case KINET_EVENT_NOT_READY:
        ...
        case KINET_EVENT_GAP:
            ...
            kinet_event_iterator_reset(iter);
            break;

        case KINET_EVENT_SUCCESS:
...
static void
find_initial_iteration_point(struct kinet_event_iterator *const iter)
{
    ...
    // `kinet_event_ring_init_iterator`, the initial iteration point is set to
    ...
    (void)kinet_exec_iter_consensus_prev(iter, KINET_EXEC_BLOCK_START, nullptr);
}

int main(int const argc, char **const argv)
{
    char event_ring_pathbuf[PATH_MAX];
    char const *event_ring_input = KINET_EVENT_DEFAULT_EXEC_FILE_NAME;
    ...
    // case, KINET_EVENT_DEFAULT_EXEC_FILE_NAME); the below function will place
    // "pure" file names (i.e., with no '/' in path) in the best subdirectory
    if (kinet_event_resolve_ring_file(
    ...
    // We could pass the `ring_fd` file descriptor to kinet_event_ring_mmap now,
    // but we first call the helper function kinet_event_is_snapshot_file. This
    ...
    bool is_snapshot;
    if (kinet_event_is_snapshot_file(
            ring_fd, event_ring_pathbuf, &is_snapshot) != 0) {
        goto Error; // Cannot determine if snapshot is fatal
    }
    if (is_snapshot) {
        // We have a snapshot; use kinet_event_decompress_snapshot_fd to
        ...
        // crashed or hung.
        int snapshot_fd;
        if (kinet_event_decompress_snapshot_fd(
    ...
    struct kinet_event_ring exec_ring;
    if (kinet_event_ring_mmap(
    ...
    // event content.
    if (kinet_event_ring_check_content_type(
            &exec_ring,
            KINET_EVENT_CONTENT_TYPE_EXEC,
            g_kinet_exec_event_schema_hash) != 0) {
        goto Error;
    }
    ...
    if (is_snapshot) {
        ...
        pid_t writer_pid;
        if (kinet_event_ring_query_excl_writer_pid(ring_fd, &writer_pid) != 0) {
            goto Error;
        }
    ...
    if (kinet_event_ring_init_iterator(&exec_ring, &iter) != 0) {
        goto Error;
    }
