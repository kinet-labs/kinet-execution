...
namespace
{
    struct TempDb
    {
        ...
        TempDb()
            : fd{KINET_ASYNC_NAMESPACE::make_temporary_inode()}
    ...
    struct TempDir
    {
        ...
        TempDir()
        {
            std::filesystem::path tmpl =
                ...
                "kinet_snapshot_test_XXXXXX";
            char *const result = ::mkdtemp((char *)tmpl.native().data());
            KINET_ASSERT(result != nullptr);
            path = result;
        }
    ...
    constexpr std::array STREAM_FILES{
        std::pair{"eth_header", KINET_SNAPSHOT_ETH_HEADER},
        std::pair{"account", KINET_SNAPSHOT_ACCOUNT},
        std::pair{"storage", KINET_SNAPSHOT_STORAGE},
        std::pair{"code", KINET_SNAPSHOT_CODE}};
    static_assert(STREAM_FILES.size() == KINET_SNAPSHOT_FILES_PER_SHARD);

    kinet::byte_string read_file(std::filesystem::path const &path)
    {
        std::ifstream in{path, std::ios::binary};
        KINET_ASSERT(in.is_open());
        std::stringstream buffer;
        buffer << in.rdbuf();
        auto const bytes = buffer.str();
        return kinet::byte_string{
            reinterpret_cast<unsigned char const *>(bytes.data()),
            bytes.size()};
    }

    void write_file(
        std::filesystem::path const &path, kinet::byte_string_view const bytes)
    {
        std::ofstream out{path, std::ios::binary | std::ios::trunc};
        KINET_ASSERT(out.is_open());
        out.write(
            reinterpret_cast<char const *>(bytes.data()),
            static_cast<std::streamsize>(bytes.size()));
        out.close();
        KINET_ASSERT(out.good());
    }
    ...
    kinet::byte_string_view strip_stream_header(
        kinet::byte_string_view view, kinet_snapshot_type const kind)
    {
        using namespace kinet;
        KINET_ASSERT(view.size() >= sizeof(kinet_snapshot_stream_header));
        auto const header =
            unaligned_load<kinet_snapshot_stream_header>(view.data());
        KINET_ASSERT(header.magic == KINET_SNAPSHOT_STREAM_MAGIC);
        KINET_ASSERT(header.version == KINET_SNAPSHOT_STREAM_VERSION);
        KINET_ASSERT(header.kind == kind);
        KINET_ASSERT(header.reserved == 0);
        KINET_ASSERT(header.guard == KINET_SNAPSHOT_STREAM_GUARD);
    ...
    // Load a snapshot directory the way kinet_db_snapshot_load_filesystem does,
        ...
        uint64_t const block, bool const load_to_secondary)
    {
        ...
        auto *const loader = kinet_db_snapshot_loader_create(
            block,
            dbname_paths,
            1,
        ...
        for (auto const &dir : std::filesystem::directory_iterator{
            ...
            auto const ptr = [](kinet::byte_string const &b) {
...
TEST(DbBinarySnapshot, Basic)
{
...

    {
        ...
        EXPECT_TRUE(kinet_db_dump_snapshot(
            dbname_paths,
            1,
            static_cast<unsigned>(-1),
            100,
            kinet_db_snapshot_write_filesystem,
            context,
        ...
        kinet_db_snapshot_filesystem_write_user_context_destroy(context);

        {
            ...
            // Db ctor (via kinet_db_snapshot_loader_create) can resolve
            // it.
            kinet::mpt::test::DbAccessor::aux(dest_init)
                .metadata_ctx()
                .set_state_machine_kind(
                    timeline_id::primary, state_machine_kind::ethereum);
        }
        char const *dbname_paths_new[] = {dest_db.path.c_str()};
        kinet_db_snapshot_load_filesystem(
            dbname_paths_new,
            1,
            static_cast<unsigned>(-1),
...
namespace
{
    ...
    struct LeafCounter final : public kinet::mpt::TraverseMachine
    {
...
TEST(DbBinarySnapshot, LoadPageModeOnSecondaryDb)
{
    ...
    // Dump slot-encoded snapshot, then load it into a page-encoded secondary.
    {
        ...
        // and stamp its kind (= kinet) up front. Both handles are destroyed
        // before the load so the loader holds the only timeline references.
        {
            ...
            [[maybe_unused]] auto const secondary =
                primary.activate_secondary_timeline(
                    std::make_unique<kinet::KinetOnDiskMachine>());
            KINET_ASSERT(primary.timeline_active(timeline_id::secondary));
        }
...
namespace
{
    ...
    std::array<kinet::Address, 4> const PAGE_ADDRS{
        kinet::Address{1},
        kinet::Address{2},
        kinet::Address{3},
        kinet::Address{4}};

    kinet::bytes32_t page_slot_key(uint16_t const raw)
    {
        kinet::bytes32_t key{};
    ...
    kinet::bytes32_t
    page_slot_value(kinet::Address const &addr, uint16_t const raw)
    {
        kinet::bytes32_t value{};
    ...
    kinet::byte_string page_code(kinet::Address const &addr)
    {
        return kinet::byte_string(64, addr.bytes[19]);
    }

    // Populate the page-encoded secondary timeline of `dbname` and return its
    // state root.
    kinet::bytes32_t build_page_source(std::string const &dbname)
    {
        using namespace kinet;
        using namespace kinet::mpt;
        ...
        TrieDb tdb2{db2};
        KINET_ASSERT(tdb2.is_page_encoded());
    ...
    void verify_page_restore(
        std::string const &dbname, kinet::bytes32_t const &expected_root)
    {
        using namespace kinet;
        using namespace kinet::mpt;
...
TEST(DbBinarySnapshot, SnapshotStreamHeaders)
{
    ...
    std::array<size_t, KINET_SNAPSHOT_FILES_PER_SHARD> headers_checked{};
    for (auto const &dir : std::filesystem::directory_iterator{
...
// holds page-encoded state, kinet_db_dump_snapshot(dump_from_secondary=true)
...
TEST(DbBinarySnapshot, DumpFromSecondaryPageDb)
{
    ...
    // the secondary (KinetOnDiskMachine stamps its kind = kinet).
    {
        mpt::Db db1{
            std::make_unique<OnDiskMachine>(),
            OnDiskDbConfig{
