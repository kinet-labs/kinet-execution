...
namespace
{
    ...
    template <kinet_eth_revision r>
    using rev = std::integral_constant<kinet_eth_revision, r>;
...
{
    static_assert(TestFixture::Trait::evm_rev() >= KINET_ETH_ISTANBUL);
...
{
    auto value_since = []<kinet_eth_revision rev, typename T>(
                           std::integral_constant<kinet_eth_revision, rev>,
                           T val) consteval {
        if constexpr (TestFixture::Trait::evm_rev() >= rev) {
    ...
    static constexpr auto base_fee_per_gas =
        value_since(rev<KINET_ETH_LONDON>{}, uint256_t{});
    static constexpr auto withdrawals_root =
        value_since(rev<KINET_ETH_SHANGHAI>{}, bytes32_t{});
    static constexpr auto blob_gas_used =
        value_since(rev<KINET_ETH_CANCUN>{}, uint64_t{});
    static constexpr auto excess_blob_gas =
        value_since(rev<KINET_ETH_CANCUN>{}, uint64_t{});
    static constexpr auto parent_beacon_block_root =
        value_since(rev<KINET_ETH_CANCUN>{}, bytes32_t{});
    static constexpr auto requests_hash =
        value_since(rev<KINET_ETH_PRAGUE>{}, bytes32_t{});
    ...
    TEST_OPTIONAL_FIELD(base_fee_per_gas, uint256_t{}, KINET_ETH_LONDON)
    TEST_OPTIONAL_FIELD(withdrawals_root, bytes32_t{}, KINET_ETH_SHANGHAI)
    TEST_OPTIONAL_FIELD(blob_gas_used, uint64_t{}, KINET_ETH_CANCUN)
    TEST_OPTIONAL_FIELD(excess_blob_gas, uint64_t{}, KINET_ETH_CANCUN)
    TEST_OPTIONAL_FIELD(parent_beacon_block_root, bytes32_t{}, KINET_ETH_CANCUN)
    TEST_OPTIONAL_FIELD(requests_hash, bytes32_t{}, KINET_ETH_PRAGUE)
}
...
TYPED_TEST(TraitsTest, validate_excess_blob_gas_against_parent)
{
    if constexpr (!TestFixture::Trait::eip_4844_active()) {
        ...
        static constexpr uint64_t timestamp =
            TestFixture::Trait::evm_rev() >= KINET_ETH_PRAGUE
                ? uint64_t{1746612311}
                : uint64_t{1710338135};
...
TYPED_TEST(TraitsTest, invalid_nonce)
{
    ...
    auto const result =
        static_validate_header<typename TestFixture::Trait>(header);
    if constexpr (TestFixture::Trait::evm_rev() >= KINET_ETH_PARIS) {
        ASSERT_TRUE(result.has_error());
