...
namespace
{
        ...
        uint64_t const block_number, uint64_t const txn_idx)
    {
        ...
        KINET_ASSERT(!call_frame.has_error());
        KINET_ASSERT(view.empty());
...
TYPED_TEST(TraitsTest, call_frames_stress_test)
{
    ...
    for (unsigned i = 0; i < recovered_senders.size(); ++i) {
        KINET_ASSERT(recovered_senders[i].has_value());
...
TYPED_TEST(TraitsTest, assertion_exception)
{
    ...
    if constexpr (is_kinet_trait_v<typename TestFixture::Trait>) {
        EXPECT_THROW({ execute(KinetMainnet{}); }, KinetException);
    }
    else {
        EXPECT_THROW({ execute(EthereumMainnet{}); }, KinetException);
    }
}
...
TYPED_TEST(TraitsTest, call_frames_refund)
{
    ...
    static constexpr auto gas_used = [] {
        if constexpr (is_evm_trait_v<typename TestFixture::Trait>) {
            if constexpr (TestFixture::Trait::evm_rev() <= KINET_ETH_BERLIN) {
                // value from
            ...
            if constexpr (TestFixture::Trait::kinet_rev() > KINET_ZERO) {
                // full gas_limit is charged since >=KINET_ONE
                return 0x186a0;
            }
            else {
                static_assert(TestFixture::Trait::evm_rev() > KINET_ETH_BERLIN);
                // same cost as >KINET_ETH_BERLIN
                return static_cast<uint64_t>((10'000'000 - 9'631'760) / 10);
            }
        }
    }();
