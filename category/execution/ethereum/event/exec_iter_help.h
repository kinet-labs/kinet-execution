...
static bool kinet_exec_ring_get_block_number(
...
static bool kinet_exec_ring_block_id_matches(
    struct kinet_event_ring const *, struct kinet_event_descriptor const *,
    kinet_c_bytes32 const *);

/// Rewind the event ring iterator so that the next event produced by
/// `kinet_event_iterator_try_next` will be the most recent consensus event
...
static bool kinet_exec_iter_consensus_prev(
    struct kinet_event_iterator *, enum kinet_exec_event_type filter,
    struct kinet_event_descriptor *);

/// Rewind the event ring iterator, as if by repeatedly calling
/// `kinet_exec_iter_consensus_prev`, stopping only when the block number
/// associated with the event matches the specified block number
static bool kinet_exec_iter_block_number_prev(
    struct kinet_event_iterator *, struct kinet_event_ring const *,
    uint64_t block_number, enum kinet_exec_event_type filter,
    struct kinet_event_descriptor *);

/// Rewind the event ring iterator, as if by repeatedly calling
/// `kinet_exec_iter_consensus_prev`, stopping only when the block ID
...
/// these events
static bool kinet_exec_iter_block_id_prev(
    struct kinet_event_iterator *, struct kinet_event_ring const *,
    kinet_c_bytes32 const *, enum kinet_exec_event_type filter,
    struct kinet_event_descriptor *);
...
static bool kinet_exec_iter_rewind_for_simple_replay(
    struct kinet_event_iterator *, struct kinet_event_ring const *,
    uint64_t block_number, struct kinet_event_descriptor *);

#ifdef __cplusplus
} // extern "C"
#endif

#define KINET_EXEC_ITER_HELP_INTERNAL
#include "exec_iter_help_inline.h"
#undef KINET_EXEC_ITER_HELP_INTERNAL
