...
void init_txn_header_start(
    ...
    kinet_exec_txn_header_start *const event)
{
    *event = kinet_exec_txn_header_start{
        .txn_hash = to_bytes(keccak256(rlp::encode_transaction(txn))),
        .sender = sender,
        .txn_header = {
            .txn_type = std::bit_cast<kinet_c_transaction_type>(txn.type),
            ...
            .r = txn.sc.signature.r,
            .s = txn.sc.signature.s,
            .y_parity = txn.sc.signature.y_parity == 1,
...
ReservedEvent<T> reserve_event(
    ExecutionEventRecorder *exec_recorder, kinet_exec_event_type event_type,
...
// Records a KINET_EXEC_STORAGE_ACCESS event for all reads and writes in the
// AccountState prestate and modified maps
void record_storage_events(
    ExecutionEventRecorder *const exec_recorder,
    kinet_exec_account_access_context const ctx,
    ...
    bool const is_transient)
{
    for (size_t index = 0; auto const &[key, value] : *prestate_storage) {
        ...
        ReservedEvent const storage_access =
            reserve_event<kinet_exec_storage_access>(
                exec_recorder, KINET_EXEC_STORAGE_ACCESS, opt_txn_num);
        *storage_access.payload = kinet_exec_storage_access{
            .address = *address,
        ...
        storage_access.event->content_ext[KINET_FLOW_ACCOUNT_INDEX] =
            account_index;
...
// Records an KINET_EXEC_ACCOUNT_ACCESS event, and delegates to
...
{
    KINET_ASSERT(account_info.prestate);
    kinet_c_eth_account_state initial_state;
    std::optional<Account> const &prestate_account =
    ...
    ReservedEvent const account_access =
        reserve_event<kinet_exec_account_access>(
            exec_recorder, KINET_EXEC_ACCOUNT_ACCESS, opt_txn_num);
    *account_access.payload = kinet_exec_account_access{
        .index = index,
    ...
    std::optional<uint32_t> const opt_txn_num, State const &state)
{
    ...
    ReservedEvent const list_header =
        reserve_event<kinet_exec_account_access_list_header>(
            exec_recorder, KINET_EXEC_ACCOUNT_ACCESS_LIST_HEADER, opt_txn_num);
    *list_header.payload = kinet_exec_account_access_list_header{
    ...
    std::span<std::optional<Address> const> const authorities)
{
    ...
    ReservedEvent const txn_header_start =
        exec_recorder->reserve_txn_event<kinet_exec_txn_header_start>(
    ...
    for (uint32_t index = 0; AccessEntry const &e : transaction.access_list) {
        ReservedEvent const access_list_entry =
            exec_recorder->reserve_txn_event<kinet_exec_txn_access_list_entry>(
                KINET_EXEC_TXN_ACCESS_LIST_ENTRY,
                txn_num,
                as_bytes(std::span{e.keys}));
        *access_list_entry.payload = kinet_exec_txn_access_list_entry{
            .index = index,
            .entry = {
                .address = e.a,
    ...
    for (uint32_t index = 0;
        ...
        ReservedEvent const auth_list_entry =
            exec_recorder->reserve_txn_event<kinet_exec_txn_auth_list_entry>(
                KINET_EXEC_TXN_AUTH_LIST_ENTRY, txn_num);
        *auth_list_entry.payload = kinet_exec_txn_auth_list_entry{
            .index = index,
            .entry =
                {
                    ...
                    .address = e.address,
                    .nonce = e.nonce,
                    .y_parity = e.sc.signature.y_parity == 1,
                    .r = e.sc.signature.r,
                    .s = e.sc.signature.s,
                },
    ...
    // TXN_HEADER_END
    exec_recorder->record_txn_marker_event(KINET_EXEC_TXN_HEADER_END, txn_num);
}
    ...
    State const &txn_state)
{
    ...
    ReservedEvent const txn_evm_output =
        exec_recorder->reserve_txn_event<kinet_exec_txn_evm_output>(
            KINET_EXEC_TXN_EVM_OUTPUT, txn_num);
    *txn_evm_output.payload = kinet_exec_txn_evm_output{
        .receipt =
    ...
    for (uint32_t index = 0; auto const &log : receipt.logs) {
        ReservedEvent const txn_log =
            exec_recorder->reserve_txn_event<kinet_exec_txn_log>(
                KINET_EXEC_TXN_LOG,
                txn_num,
                as_bytes(std::span{log.topics}),
                as_bytes(std::span{log.data}));
        *txn_log.payload = kinet_exec_txn_log{
            .index = index,
            .address = log.address,
    ...
    for (uint32_t index = 0; auto const &call_frame : call_frames) {
        ...
        ReservedEvent const txn_call_frame =
            exec_recorder->reserve_txn_event<kinet_exec_txn_call_frame>(
                KINET_EXEC_TXN_CALL_FRAME,
                txn_num,
                as_bytes(input_bytes),
                as_bytes(return_bytes));
        *txn_call_frame.payload = kinet_exec_txn_call_frame{
            .index = index,
    ...
    record_account_access_events_internal(
        exec_recorder, KINET_ACCT_ACCESS_TRANSACTION, txn_num, txn_state);

    exec_recorder->record_txn_marker_event(KINET_EXEC_TXN_END, txn_num);
}
    ...
    Result<Receipt>::error_type const &txn_error)
{
    ...
    if (error_domain == txn_err_domain) {
        ReservedEvent const txn_reject =
            exec_recorder->reserve_txn_event<kinet_exec_txn_reject>(
                KINET_EXEC_TXN_REJECT, txn_num);
        *txn_reject.payload = static_cast<uint32_t>(error_value);
        exec_recorder->commit(txn_reject);
    }
    else {
        ReservedEvent const evm_error =
            exec_recorder->reserve_txn_event<kinet_exec_evm_error>(
                KINET_EXEC_EVM_ERROR, txn_num);
        *evm_error.payload = kinet_exec_evm_error{
...
void record_account_access_events(
    ExecutionEventRecorder *const exec_recorder,
    kinet_exec_account_access_context const ctx, State const &state)
{
    if (exec_recorder != nullptr) {
