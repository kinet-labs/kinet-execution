...
public:
    ...
    from_event_ring(kinet_event_ring const *const ring)
    {
        ExecutionEventRecorder r{};
        if (int const rc = kinet_event_ring_init_recorder(ring, &r.recorder_)) {
    ...
    [[nodiscard]] ReservedEvent<kinet_exec_block_start>
    ...
    uint64_t record_block_marker_event(kinet_exec_event_type);

    /// Record a transaction-level event with no payload in one step
    uint64_t record_txn_marker_event(kinet_exec_event_type, uint32_t txn_num);
...
inline ReservedEvent<kinet_exec_block_start>
ExecutionEventRecorder::reserve_block_start_event()
{
    ReservedEvent const block_start =
        reserve_block_event<kinet_exec_block_start>(KINET_EXEC_BLOCK_START);
    cur_block_start_seqno_ = block_start.seqno;
    block_start.event->content_ext[KINET_FLOW_BLOCK_SEQNO] = block_start.seqno;
    return block_start;
}

template <typename T, std::same_as<std::span<std::byte const>>... U>
ReservedEvent<T> ExecutionEventRecorder::reserve_block_event(
    kinet_exec_event_type event_type, U... trailing_bufs)
{
    ...
    r.event->content_ext[KINET_FLOW_BLOCK_SEQNO] = cur_block_start_seqno_;
    r.event->content_ext[KINET_FLOW_TXN_ID] = 0;
    r.event->content_ext[KINET_FLOW_ACCOUNT_INDEX] = 0;
    return r;
}
...
inline uint64_t ExecutionEventRecorder::record_block_marker_event(
    kinet_exec_event_type const event_type)
{
    uint64_t seqno;
    uint8_t *payload_buf;
    kinet_event_descriptor *const event =
        kinet_event_recorder_reserve(&recorder_, 0, &seqno, &payload_buf);
    event->event_type = std::to_underlying(event_type);
    event->content_ext[KINET_FLOW_BLOCK_SEQNO] = cur_block_start_seqno_;
    event->content_ext[KINET_FLOW_TXN_ID] = 0;
    event->content_ext[KINET_FLOW_ACCOUNT_INDEX] = 0;
    kinet_event_recorder_commit(event, seqno);
    return seqno;
}

inline uint64_t ExecutionEventRecorder::record_txn_marker_event(
    kinet_exec_event_type const event_type, uint32_t const txn_num)
{
    uint64_t seqno;
    uint8_t *payload_buf;
    kinet_event_descriptor *const event =
    ...
    event->event_type = std::to_underlying(event_type);
    event->content_ext[KINET_FLOW_BLOCK_SEQNO] = cur_block_start_seqno_;
    event->content_ext[KINET_FLOW_TXN_ID] = txn_num + 1;
    event->content_ext[KINET_FLOW_ACCOUNT_INDEX] = 0;
    kinet_event_recorder_commit(event, seqno);
    return seqno;
}
