...
BlockState::BlockState(Db &db, vm::VM &kinet_vm, Db *const secondary_db)
    : db_{db}
...
std::optional<Account> BlockState::read_account(Address const &address)
{
    // block state
    {
        ...
        KINET_ASSERT(state_);
        if (KINET_LIKELY(state_->find(it, address))) {
    ...
    Address const &address, Incarnation const incarnation, bytes32_t const &key)
{
    ...
    // block state
    {
        StateDeltas::const_accessor it{};
        KINET_ASSERT(state_);
        KINET_ASSERT(state_->find(it, address));
        ...
        auto const &storage = it->second.storage;
        {
            ...
            if (KINET_LIKELY(storage.find(it2, key))) {
