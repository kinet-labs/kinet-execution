...
namespace
{
        ...
        size_t &pos)
    {
        ...
        while (pos + 1 < frames.size()) {
            KINET_ASSERT(json.contains("depth"));
    ...
    frames_.emplace_back(CallFrame{
        .type =
            [kind = msg.kind] {
                ...
                case EVMC_EOFCREATE:
                    KINET_ABORT(); // unsupported
                }
                KINET_ABORT(); // unreachable
            }(),
        .flags = msg.flags,
        .from = from,
        .to = to,
...
{
    KINET_ASSERT(!frames_.empty());
    KINET_ASSERT(!last_.empty());
    KINET_ASSERT(!positions_.empty());

    auto &frame = frames_.at(last_.top());

    KINET_ASSERT(frame.gas >= static_cast<uint64_t>(res.gas_left));
...
void CallTracer::on_log(Receipt::Log log)
{
    ...
    auto &frame = frames_.at(last_.top());
    KINET_ASSERT(frame.logs.has_value());
    ...
    uint256_t const &transferred_balance)
{
    ...
    frames_.emplace_back(CallFrame{
        ...
        .input = {},
        .output = {},
        .status = KINET_STATUS_SUCCESS, // TODO
...
{
    KINET_ASSERT(!frames_.empty());
    KINET_ASSERT(frames_[0].depth == 0);
