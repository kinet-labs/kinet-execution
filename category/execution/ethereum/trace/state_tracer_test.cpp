...
TEST(PrestateTracer, prestate_empty_block_no_reward)
{
    ...
    // Apply block reward.
    apply_block_reward<KinetTraits<KINET_NEXT>>(s, block);
    EXPECT_TRUE(s.original().find(ADDR_A) == s.original().end());
    EXPECT_TRUE(s.current().find(ADDR_A) == s.current().end());
...
//   7. dipped_into_reserve + is_delegated (Kinet reserve-balance revert path)
//
...
namespace
{
    ...
    inline auto const SYSTEM_STUB_CODE = kinet::from_hex("00").value();
    inline auto const SYSTEM_STUB_CODE_HASH =
        to_bytes(kinet::keccak256(SYSTEM_STUB_CODE));
    inline auto const SYSTEM_STUB_ICODE =
        kinet::vm::make_shared_intercode(SYSTEM_STUB_CODE);
}
...
// Kinet-specific ChainContext for init_reserve_balance_context, which runs
// at the top of operator() for kinet traits. The Kinet KINET_FOUR+ exercise
...
// Sites 7+8: dipped_into_reserve and is_delegated (Kinet KINET_FOUR+).
//
...
TYPED_TEST(KinetTraitsTest, code_tracer_records_reserve_balance_code)
{
    using Trait = typename TestFixture::Trait;
    if (TestFixture::REV < KINET_FOUR) {
        GTEST_SKIP() << "reserve-balance code reads are KINET_FOUR+ only";
    }
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_same_page)
{
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_same_page_three_slots)
{
    if constexpr (!TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP() << "page-gas dedup only active when mip_8_active";
    }
...
TYPED_TEST(KinetTraitsTest, access_list_mip8_passthrough_two_addresses)
{
    if constexpr (!TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP() << "page-gas dedup only active when mip_8_active";
    }
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_one_addr_two_pages)
{
    if constexpr (!TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP() << "page-gas dedup only active when mip_8_active";
    }
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_old_revision)
{
    if constexpr (TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP()
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_page_boundary)
{
    if constexpr (!TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP() << "page-gas dedup only active when mip_8_active";
    }
...
TYPED_TEST(KinetTraitsTest, access_list_page_dedup_multi_byte_page_boundary)
{
    if constexpr (!TestFixture::Trait::mip_8_active()) {
        GTEST_SKIP() << "page-gas dedup only active when mip_8_active";
    }
