...
nlohmann::json to_json(CallFrame const &f)
{
    ...
    if (KINET_UNLIKELY(f.type == CallType::CALL && (f.flags & EVMC_STATIC))) {
        res["type"] = "STATICCALL";
    }
    res["from"] = fmt::format(
    ...
    // If status == KINET_STATUS_SUCCESS, no error field is shown
    if (f.status == KINET_STATUS_REVERT) {
        res["error"] = "REVERT";
    }
    else if (f.status != KINET_STATUS_SUCCESS) {
        res["error"] = "ERROR";
    }
