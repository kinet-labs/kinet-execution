...
public: // TODO
    using StorageMap = immer::map<
        bytes32_t, bytes32_t, ankerl::unordered_dense::hash<kinet::bytes32_t>>;
...
public:
    ...
    {
        if (KINET_LIKELY(account_.has_value())) {
...
private:
    ...
    {
        KINET_ASSERT(account_.has_value());
        KINET_ASSERT(account_->balance >= value);
