...
AccountState &State::current_account_state(Address const &address)
{
    ...
    if (KINET_UNLIKELY(it == current_.end())) {
        // original
...
{
    KINET_ASSERT(version_);
    KINET_ASSERT(dirty_.size() == version_);
...
{
    auto const &account = recent_account(address);
    if (KINET_LIKELY(account.has_value())) {
...
bytes32_t State::get_storage(Address const &address, bytes32_t const &key)
{
    ...
    if (it == current_.end()) {
        ...
        KINET_ASSERT(it2 != original_.end());
        auto &account_state = it2->second;
        auto const &account = account_state.account_;
        KINET_ASSERT(account.has_value());
...
void State::set_nonce(Address const &address, uint64_t const nonce)
{
    ...
    if (KINET_UNLIKELY(!account.has_value())) {
    ...
    Address const &address, uint256_t const &delta)
{
    ...
    KINET_ASSERT_THROW(delta <= account.value().balance, "balance underflow");
    ...
    Address const &address, bytes32_t const &key, bytes32_t const &value)
{
    ...
    auto &account_state = current_account_state(address);
    KINET_ASSERT(account_state.account_);
    // original
    {
...
void State::destruct_suicides()
{
    ...
    for (auto &it : current_) {
        ...
        KINET_ASSERT(stack.size() == 1);
        KINET_ASSERT(stack.version() == 0);
...
void State::destruct_touched_dead()
{
    ...
    for (auto &it : current_) {
        ...
        KINET_ASSERT(stack.version() == 0);
        auto &account_state = stack.current(0);
        if (KINET_LIKELY(!account_state.is_touched())) {
            continue;
        }
        auto &account = account_state.account_;
        if (is_dead(account)) {
...
void State::create_contract(Address const &address)
{
    ...
    if (KINET_UNLIKELY(account.has_value())) {
        // EIP-684
        KINET_ASSERT(account->nonce == 0);
        KINET_ASSERT(account->code_hash == NULL_HASH);
    ...
    Address const &address, std::optional<Account> const &actual)
{
    ...
    KINET_ASSERT(original_it != original_.end());
    ...
    KINET_ASSERT(original->balance != actual->balance);
    ...
    if (current_it != current_.end()) {
        KINET_ASSERT(current_it->second.size() == 1);
