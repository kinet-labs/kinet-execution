...
// do not bump nonce prior to KINET_FIVE but do bump it after
TYPED_TEST(TraitsTest, create_insufficient_balance_nonce_bump)
{
    ...
    if constexpr (is_kinet_trait_v<typename TestFixture::Trait>) {
        if constexpr (TestFixture::Trait::kinet_rev() >= KINET_FIVE) {
...
TYPED_TEST(TraitsTest, create_inside_delegated_call)
{
    ...
    if constexpr (TestFixture::Trait::evm_rev() >= KINET_ETH_PRAGUE) {
        ...
        // CREATE should fail on Kinet chains and succeed on Ethereum chains
        if constexpr (TestFixture::Trait::can_create_inside_delegated()) {
            static_assert(TestFixture::is_evm_trait());
            EXPECT_EQ(result.status_code, EVMC_SUCCESS);
        }
        else {
            static_assert(TestFixture::is_kinet_trait());
...
TYPED_TEST(TraitsTest, create2_inside_delegated_call_via_delegatecall)
{
    ...
    if constexpr (TestFixture::Trait::evm_rev() >= KINET_ETH_PRAGUE) {
        auto const result = h.call(m);
        // CREATE2 should fail on Kinet chains and succeed on Ethereum chains
        if constexpr (TestFixture::Trait::can_create_inside_delegated()) {
            static_assert(TestFixture::is_evm_trait());
            EXPECT_EQ(result.status_code, EVMC_SUCCESS);
        }
