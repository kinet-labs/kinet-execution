...
{
    if (KINET_UNLIKELY(enc.size() > sizeof(T))) {
...
namespace detail
{
    ...
    extract_payload(byte_string_view &enc, size_t const i, size_t const length)
    {
        ...
        if (KINET_UNLIKELY(end > enc.size() || end < i)) {
    ...
    {
        KINET_DEBUG_ASSERT(!enc.empty());
        KINET_DEBUG_ASSERT(enc[0] < 0xc0);
        ...
        else if (enc[0] < 0xb8) // [0x80, 0xb7] - short string (0-55 bytes)
        {
            ...
            ++i;
            if (length == 1) {
                if (KINET_UNLIKELY(enc.size() < 2)) {
                    return DecodeError::InputTooShort;
                }
                if (KINET_UNLIKELY(enc[1] < 0x80)) {
             ...
             // payload
        {
            ...
            ++i;
            if (KINET_UNLIKELY(i + length_of_length >= enc.size())) {
                return DecodeError::InputTooShort;
            }
            BOOST_OUTCOME_TRY(
                length, decode_length(enc.substr(i, length_of_length)));
            if (KINET_UNLIKELY(length < 56)) {
                return DecodeError::TypeUnexpected;
            }
            i += length_of_length;
        }
...
decode_byte_string_fixed(byte_string_view &enc)
{
    ...
    if (KINET_UNLIKELY(payload.size() != N)) {
