...
{
    {
        auto const remaining = decode_length(kinet::byte_string({}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 0);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x00}));
        EXPECT_TRUE(
            remaining.has_error() &&
    ...
    {
        auto const remaining = decode_length(kinet::byte_string({0x0f}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 15);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x7a}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 122);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x04, 0x00}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 1024);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x03, 0x04}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 772);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x02, 0x29}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 553);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x04, 0x98}));
        EXPECT_TRUE(!remaining.has_error() && remaining.assume_value() == 1176);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x41, 0x42}));
        EXPECT_TRUE(
            !remaining.has_error() && remaining.assume_value() == 16706);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0x7b, 0x2a}));
        EXPECT_TRUE(
            !remaining.has_error() && remaining.assume_value() == 31530);
    }

    {
        auto const remaining = decode_length(kinet::byte_string({0xff, 0xff}));
        EXPECT_TRUE(
...
TEST(Rlp_Number, DecodeEncodeUnsigned)
{
    ...
    // integer 15
    {
        ...
        EXPECT_EQ(encoding, kinet::byte_string({0x0f}));
    ...
    // integer 1024
    {
        ...
        auto const ten_twenty_four_encoding =
            kinet::byte_string({0x82, 0x04, 0x00});
...
TEST(Rlp_Number, DecodeEncodeBigNumers)
{
    ...
    // uint128_t
    {
        ...
        auto const sorta_big_num = kinet::byte_string(
            {0x90,
             0xbe,
             0xa3,
             0x4d,
    ...
    // uint256_t
    {
        ...
        auto const big_num = kinet::byte_string(
    ...
    // bytes32
    {
        ...
        auto const big_be_num = kinet::byte_string(
            {0xa0, 0xbe, 0xa3, 0x4d, 0xd0, 0x4b, 0x09, 0xad, 0x3b, 0x60, 0x14,
             0x25, 0x1e, 0xe2, 0x45, 0x78, 0x07, 0x40, 0x87, 0xee, 0x60, 0xfd,
             0xa8, 0xc3, 0x91, 0xcf, 0x46, 0x6d, 0xfe, 0x5d, 0x68, 0x7d, 0x7b});
    ...
    // address
    {
        ...
        auto const address = kinet::byte_string(
...
TEST(Rlp_Number, EncodeCombinations)
{
    ...
    EXPECT_EQ(encoded_comb1, kinet::byte_string({0xC2, 0x80, 0x09}));
    ...
    auto const expected_list_encoding =
        kinet::byte_string({0xf7 + 1, 58}) + fifty_six_char_string_encoding;
