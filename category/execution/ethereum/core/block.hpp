...
struct BlockHeader
{
    ...
    // in KinetConsensusBlockHeader::block_round, so replay paths can
    // repopulate this from there without any on-wire encoding.
    // Coverage: only the consensus execution path (propose_block in
    // runloop_kinet) populates this. Any entrypoint that does not repopulate
    // it -- RPC/trace re-execution in kinet_executor, historical-block replay
