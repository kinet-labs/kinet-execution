...
class TrieRODb final : public ::kinet::Db
{
    ::kinet::mpt::RODb &db_;
    uint64_t block_number_;
    ::kinet::mpt::NodeCursor prefix_cursor_;
...
public:
    explicit TrieRODb(mpt::RODb &db)
        ...
        , prefix_cursor_()
        , page_encoded_(
              db_.state_machine_type() == mpt::state_machine_kind::kinet)
    {
    }
        ...
        bytes32_t const &block_id = bytes32_t{}) override
    {
        ...
        if (res.has_error()) {
            KINET_ASSERT_PRINTF(
                res.assume_error() ==
                    ::kinet::mpt::DbError::version_no_longer_exist,
    ...
    {
        KINET_ABORT("TrieRODb read_storage_page is currently not supported");
    }
