...
TrieDb::TrieDb(mpt::Db &db, bool const enable_multiblock_cache)
    ...
    , page_encoded_{db_.state_machine_type() == mpt::state_machine_kind::kinet}
{
}
...
{
    if (!page_encoded_) {
        KINET_ABORT("read_storage_page is only valid on a page-encoded TrieDb");
    }
    ...
    std::function<void(BlockHeader &)> const populate_header_fn)
{
    ...
    KINET_ASSERT(block_number <= std::numeric_limits<int64_t>::max());

    KINET_ASSERT(block_id != bytes32_t{});
    if (db_.is_on_disk() && block_id != proposal_block_id_) {
        ...
        if (db_.get_latest_version() != INVALID_BLOCK_NUM) {
            KINET_ASSERT(block_number != block_number_);
    ...
    KINET_ASSERT(populate_header_fn);
    ...
    uint64_t const block_number, bytes32_t const &block_id)
{
    ...
    // set read state
    if (!db_.is_on_disk()) {
        KINET_ASSERT(proposal_block_id_ == bytes32_t{});
        block_number_ = block_number;
        return;
    }
    prefix_ =
...
void TrieDb::finalize(uint64_t const block_number, bytes32_t const &block_id)
{
    ...
    if (db_.is_on_disk()) {
        ...
        auto root = (block_number_ == block_number)
                        ? curr_root_
                        : db_.load_root_for_version(block_number);
        KINET_ASSERT(db_.find(root, src_prefix, block_number).has_value());
        curr_root_ = db_.copy_trie(
...
std::optional<bytes32_t> TrieDb::withdrawals_root()
{
    ...
    KINET_ASSERT(data.size() == sizeof(bytes32_t));
...
BlockHeader TrieDb::read_eth_header()
{
    ...
    KINET_ASSERT(!query_res.has_error());
...
nlohmann::json TrieDb::to_json(size_t const concurrency_limit)
{
    struct Traverse : public TraverseMachine
    {
        ...
        virtual bool down(unsigned char const branch, Node const &node) override
        {
            if (branch == INVALID_BRANCH) {
                KINET_ASSERT(node.path_nibble_view().nibble_size() == 0);
                return true;
            }
        ...
        void handle_storage(Node const &node)
        {
            ...
            if (db.is_page_encoded()) {
                ...
                KINET_ASSERT(raw_res.has_value());
    ...
    KINET_ASSERT(res_cursor.has_value());
    KINET_ASSERT(res_cursor.value().is_valid());
