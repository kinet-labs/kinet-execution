...
CommitBuilder &CommitBuilder::add_receipts(std::vector<Receipt> const &receipts)
{
    ...
    KINET_ASSERT(receipts.size() <= std::numeric_limits<uint32_t>::max());
    ...
    std::vector<Address> const &senders)
{
    ...
    KINET_ASSERT(transactions.size() <= std::numeric_limits<uint32_t>::max());
    KINET_ASSERT(transactions.size() == senders.size());
    ...
    std::vector<std::vector<CallFrame>> const &call_frames)
{
    ...
    KINET_ASSERT(call_frames.size() <= std::numeric_limits<uint32_t>::max());
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(call_frames.size()); ++i) {
        ...
        while (!frame_view.empty()) {
            KINET_ASSERT(chunk_index <= std::numeric_limits<uint8_t>::max());
