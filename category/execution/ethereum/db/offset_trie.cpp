    ...
    , root{read_root(blob_)}
{
    ...
    KINET_ASSERT(node.bytes() == region_end || node.tag() != EMPTY);
    ...
    KINET_ASSERT(node.bytes() == region_end); // nodes tile exactly
    ...
    while (id != NULL_ID) {
        id = match(
            ...
            Cases{
                ...
                [](DigestView) -> NodeId {
                    KINET_ABORT("incomplete witness: lookup hit a Digest");
                },
                [](NullView) -> NodeId {
    ...
    return match(
        ...
        Cases{
            ...
            [&](auto) {
                ...
                KINET_ASSERT(rem.rlp_size() >= 32);
                bytes32_t h;
                // RLP occupies the tail: [rem.end(), buf_end).
                kinet_keccak256(rem.rlp_data(), rem.rlp_size(), h.bytes);
    ...
    NodeId const id, NodeViewBase const node, OffsetTrie::node_rlp_span dest)
{
    ...
    if constexpr (priming_pass) {
        KINET_ABORT("offset trie: unprimed hash-referenced node (bad offset)");
    }
    bytes32_t h;
    kinet_keccak256(child_rlp, child_rlp_len, h.bytes);
    hashes_.emplace(id, h);
...
{
    KINET_DEBUG_ASSERT(node.tag() != EMPTY && node.tag() != DIGEST);
    ...
    return match(
        ...
        Cases{
            ...
            [&, encode_path, wrap](AccountLeafView l) -> node_rlp_span {
                ...
                KINET_DEBUG_ASSERT(account_len > 55 && account_len <= 0xFF);
...
namespace
{
    ...
    void append_path(byte_string &b, NibblesView const path)
    {
        ...
        KINET_ASSERT(nlen <= MAX_PATH_NIBBLES);
    ...
    NibblesView const path)
{
    ...
    KINET_DEBUG_ASSERT(len >= 2 && len <= MAX_NONCE_BALANCE_RLP_LEN);
...
{
    KINET_ASSERT(ext_parent != NULL_ID);
    KINET_DEBUG_ASSERT(child.tag() != EMPTY);
    ...
    match(
        ...
        Cases{
            ...
            [](DigestView) {
                KINET_ABORT("incomplete witness: collapse hit a Digest");
            },
...
OffsetTrie::upsert_node(NodeId const id, NibblesView const key)
{
    ...
    auto const split_leaf =
        ...
        KINET_ASSERT(cp < path.nibble_size() && cp < key.nibble_size());
    ...
    return match(
        ...
        Cases{
            ...
            [&](BranchView b) -> std::pair<NodeId, Nibbles> {
                KINET_ASSERT(key.nibble_size() > 0); // never ends at branch
            ...
            [&](DigestView) -> std::pair<NodeId, Nibbles> {
                KINET_ABORT("incomplete witness: upsert hit a Digest");
            },
        });
}
    ...
    return match(
        ...
        Cases{
                ...
                return match(
                    ...
                    Cases{
                        [](NullView) -> OffsetTrie::EraseResult {
                            KINET_ABORT("malformed trie: node not found");
                        },
                        [&](auto) {
            ...
            [&](BranchView b) -> OffsetTrie::EraseResult {
                KINET_ASSERT(key.nibble_size() > 0);
            ...
            [&](DigestView) -> OffsetTrie::EraseResult {
                KINET_ABORT("incomplete witness: erase hit a Digest");
            },
        });
}
