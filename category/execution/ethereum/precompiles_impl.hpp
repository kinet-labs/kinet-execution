...
namespace
{
    ...
    kinet::bytes32_t kzg_to_version_hashed(KZGCommitment const &commitment)
    {
    ...
    constexpr bytes64_t blob_precompile_return_value()
    {
        ...
        constexpr auto r = kinet::from_hex<bytes64_t>(v);
...
ripemd160_impl(byte_string_view const input, std::span<uint8_t, 32> const out)
{
    ...
    kinet_rmd160(&out[12], input.data(), input.size());
...
{
    if (!kinet_bn_add(out.data(), input.data(), input.size())) {
...
{
    if (!kinet_bn_mul(out.data(), input.data(), input.size())) {
        return PrecompileImplResult::failure();
    }
    return {out.data(), out.size()};
}
...
{
    if (!kinet_snarkv(out.data(), input.data(), input.size())) {
        return PrecompileImplResult::failure();
    }
    return {out.data(), out.size()};
}
...
blake2bf_impl(byte_string_view const input, std::span<uint8_t, 64> const out)
{
    ...
    uint8_t block[KINET_BLAKE2B_BLOCKBYTES];
    std::memcpy(block, input.data() + 68, KINET_BLAKE2B_BLOCKBYTES);

    std::memcpy(&state.t, input.data() + 196, 8 * 2);

    uint32_t const r{load_be_unsafe<uint32_t>(input.data())};
    kinet_blake2b_compress(&state, block, r);
...
p256_verify_impl(byte_string_view const input, std::span<uint8_t, 32> const out)
{
    ...
    KINET_THREAD_LOCAL DL_GroupParameters_EC<ECP> const params(
        ASN1::secp256r1());
