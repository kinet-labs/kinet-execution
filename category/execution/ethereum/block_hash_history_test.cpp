...
TYPED_TEST_SUITE(
    ...
    ::detail::KinetEvmRevisionTypesSince<KINET_ETH_PRAGUE>,
    ...
    for (uint64_t i = 1; i <= window_size; i++) {
        ...
        auto const expected = [i] {
            if constexpr (is_kinet_trait_v<typename TestFixture::Trait>) {
                if constexpr (TestFixture::Trait::kinet_rev() < KINET_SIX) {
                    return bytes32_t{};
                }
            }
            return to_bytes(i - 1);
    ...
    BlockHashHistoryTraitsTest, blockhash_opcode_buffer_history_agreement)
{
    ...
    for (uint64_t i = 0; i < 256; i++) {
        ...
        auto const expected = [i] {
            if constexpr (is_kinet_trait_v<typename TestFixture::Trait>) {
                // before KINET_SIX, the block_hash_history contract is not
