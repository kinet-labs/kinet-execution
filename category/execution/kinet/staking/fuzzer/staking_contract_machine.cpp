...
#include <category/execution/kinet/staking/util/secp256k1.hpp>
...
using namespace kinet::vm::fuzzing;
...
namespace kinet::staking::test
{
    ...
    void StakingContractMachine<traits>::assert_valset_invariants()
    {
        ...
        KINET_ASSERT(valset_execution.size() == valset_execution_length);

        for (uint64_t v : valset_execution) {
            KINET_ASSERT(model_.val_execution(v).auth_address() != Address{});
        }

        // pairwise distinct elements:
        KINET_ASSERT(valset_consensus.size() == valset_consensus_length);

        KINET_ASSERT(valset_consensus_length <= ACTIVE_VALSET_SIZE);

        // pairwise distinct elements:
        KINET_ASSERT(valset_snapshot.size() == valset_snapshot_length);

        KINET_ASSERT(valset_snapshot_length <= ACTIVE_VALSET_SIZE);

        for (uint64_t v : valset_consensus) {
            KINET_ASSERT(valset_execution.contains(v));
        }
    ...
    void StakingContractMachine<traits>::assert_val_execution_invariants()
    {
        for_all_val_ids([&, this](u64_be v) {
            ...
            KINET_ASSERT(model_.val_execution(v).get_flags() < 4);
            ...
            if (enable_pubkey_assertions_) {
                ...
                if (secp_pubkey.is_valid()) {
                    ...
                    KINET_ASSERT(model_.val_id(secp_eth_address) != 0);
                }
                else {
                    KINET_ASSERT(
                        v.native() == 0 || v.native() > model_.last_val_id());
                ...
                if (bls_pubkey.is_valid()) {
                    ...
                    KINET_ASSERT(model_.val_id_bls(bls_eth_address) != 0);
                }
                else {
                    KINET_ASSERT(
                        v.native() == 0 || v.native() > model_.last_val_id());
    ...
    void StakingContractMachine<traits>::assert_delegator_invariants()
    {
        for_all_val_ids_and_addresses([&, this](u64_be v, Address const &a) {
            ...
            KINET_ASSERT(del.get_delta_epoch().native() <= model_.epoch() + 1);
            ...
            for (uint8_t const i : withdrawal_ids) {
                ...
                std::tuple<uint64_t, Address, uint8_t> const key = {
                    v.native(), a, i};
                KINET_ASSERT(all_withdrawal_requests_.contains(key));
            }
    ...
    void StakingContractMachine<traits>::assert_accumulated_rewards_invariants()
    {
        for_all_val_ids([&, this](u64_be v) {
            ...
            for (uint64_t e = begin_epoch; e <= model_.epoch() + 4; ++e) {
                ...
                KINET_ASSERT(refcount == computed_refcount);
                ...
                KINET_ASSERT(refcount != 0 || value == 0);
                ...
                KINET_ASSERT(next_refcount == 0 || value <= next_value);
                ...
                for (uint8_t const i : withdrawal_ids) {
                    ...
                    auto const upper_acc =
                        model_.accumulated_reward_per_token(withdrawal.epoch, v)
                            .value.native();
                    KINET_ASSERT(withdrawal.acc.native() <= upper_acc);
                }
    ...
    void StakingContractMachine<traits>::assert_linked_list_invariants()
    {
        ...
        for_all_val_ids_and_addresses([&, this](u64_be v, Address const &a) {
            ...
            auto const stake_sum = del.stake().load().native() +
                                   ...
                                   del.next_delta_stake().load().native();
            KINET_ASSERT(del_sets[v.native()].contains(a) == (stake_sum > 0));
        });
    }
    ...
    void StakingContractMachine<traits>::assert_solvency_invariants()
    {
        ...
        KINET_ASSERT(model_.balance_of(STAKING_CA) == computed_balance);
    ...
    void StakingContractMachine<traits>::skip_epochs(uint64_t const n)
    {
        ...
        for (; i < n; ++i) {
            ...
            KINET_ASSERT(res1.has_value());
            auto const res2 = model_.syscall_on_epoch_change(++epoch);
            KINET_ASSERT(res2.has_value());
        }
    }
    ...
    {
        KINET_ASSERT(!all_addresses_.empty());
    ...
    StakingContractMachine<traits>::gen_validator_auth_address()
    {
        ...
        if (v.empty()) {
            ...
            model_precompile_add_validator(
                signer, msg, secp, bls, sender, value);
            KINET_ASSERT(!validator_auth_addresses_.empty());
        }
    ...
    u64_be StakingContractMachine<traits>::gen_delegable_val_id()
    {
        if (!delegable_val_ids_.empty()) {
            ...
            KINET_ASSERT(model_.val_execution(v).exists());
            return v;
        }
        auto const [signer, msg, secp, bls, sender, value] =
            gen_precompile_add_validator_input(
    ...
    std::pair<u64_be, Address> StakingContractMachine<traits>::gen_delegator()
    {
        if (all_delegators_.empty()) {
            ...
            model_precompile_add_validator(
                signer, msg, secp, bls, sender, value);
            KINET_ASSERT(!all_delegators_.empty());
        }
    ...
    {
        KINET_ASSERT(lower_bound <= upper_bound);
    ...
    uint256_t StakingContractMachine<traits>::gen_stake(
        uint256_t const lower_bound, uint256_t const upper_bound)
    {
        KINET_ASSERT(upper_bound >= lower_bound);
    ...
    {
        KINET_ASSERT(model_.val_execution(val_id).exists());
        auto ds = model_.get_delegators_for_validator(val_id);
        KINET_ASSERT(!ds.empty());
    ...
    void StakingContractMachine<traits>::syscall_on_epoch_change()
    {
        ...
        KINET_ASSERT(!model_.in_epoch_delay_period());

        KINET_ASSERT(model_.epoch() == next_epoch);
    ...
    bool StakingContractMachine<traits>::syscall_reward()
    {
        ...
        KINET_ASSERT(model_.error_bound() == error_bound_before + 1);
...

        {
            ...
            for_all_addresses([&, this](Address const &a) {
                ...
                auto const r = d * rpt;
                if (a == auth_address) {
                    KINET_ASSERT(after == before + r + commission * UNIT_BIAS);
                }
                else {
                    KINET_ASSERT(after == before + r);
                }
            });
        }
        ...
        if constexpr (traits::mip_11_active()) {
            KINET_ASSERT(model_.proposer_val_id() == val_id.native());
        }
    ...
    bool StakingContractMachine<traits>::distribute_priority_fees()
    {
        ...
        if (fees.native() - commission >= MIN_EXTERNAL_REWARD) {
            ...
            KINET_ASSERT(model_.error_bound() == error_bound_before);
            size_t i = 0;
            for_all_addresses([&, this](Address const &a) {
                auto const before = unit_bias_rewards_before.at(i++);
                auto const after = model_.unit_bias_rewards(val_id, a);
                ...
                if (a == auth_address) {
                    KINET_ASSERT(after == before + commission * UNIT_BIAS);
                }
                else {
                    KINET_ASSERT(after == before);
                }
            });
            KINET_ASSERT(unclaimed_rewards_after == unclaimed_rewards_before);
        }
    ...
    {
        KINET_ASSERT(min_stake >= MIN_VALIDATE_STAKE);
        KINET_ASSERT(max_stake >= min_stake);
        KINET_ASSERT(max_stake <= MAX_STAKE);
    ...
    void StakingContractMachine<traits>::precompile_add_validator()
    {
        ...
        KINET_ASSERT(balance_after - balance_before == stake);

        auto const last_val_id_after = model_.last_val_id();

        KINET_ASSERT(last_val_id_after == last_val_id_before + 1);
        ...
        KINET_ASSERT(keys.secp_pubkey == secp_pubkey_compressed);
        KINET_ASSERT(keys.bls_pubkey == bls_pubkey_compressed);

        KINET_ASSERT(
            model_.val_execution(v).commission().load().native() == commission);

        KINET_ASSERT(model_.val_execution(v).stake().load().native() == stake);
        ...
        for (auto e = new_stake_epoch; e < model_.epoch() + 5; ++e) {
            KINET_ASSERT(model_.delegator_stake(v, auth_address, e) == stake);
        }
        ...
        for (uint64_t i = 0; i < valset_execution_length; ++i) {
            ...
            if (v == w) {
                KINET_ASSERT(stake >= ACTIVE_VALIDATOR_STAKE);
                break;
            }
        }
    ...
    StakingContractMachine<traits>::gen_precompile_delegate_input()
    {
        ...
        return discrete_choice<R>(
            ...
            [&, this](auto &) -> R {
                ...
                KINET_ASSERT(val_stake <= MAX_DELEGABLE_STAKE);
    ...
    void StakingContractMachine<traits>::precompile_delegate()
    {
        ...
        auto const val_stake_after =
            model_.val_execution(val_id).stake().load().native();
        KINET_ASSERT(val_stake_after - val_stake_before == stake);
        ...
        KINET_ASSERT(error_bound_after == error_bound_before + 3);
    }
        ...
        Address const &sender, evmc_uint256be const &value)
    {
        ...
        KINET_ASSERT(pos != wis->second.end());
    ...
    void StakingContractMachine<traits>::precompile_undelegate()
    {
        ...
        KINET_ASSERT(withdrawal_request.amount.native() == effective_stake);

        KINET_ASSERT(withdrawal_request.epoch.native() == new_stake_epoch);
        ...
        u64_be const val_id, Address const &sender, evmc_uint256be const &value)
    {
        ...
        auto const val_stake =
            model_.val_execution(val_id).stake().load().native();
        KINET_ASSERT(val_stake <= MAX_STAKE);
        if (val_stake > MAX_DELEGABLE_STAKE) {
            auto const er = delegable_val_ids_.erase(val_id.native());
            KINET_ASSERT(er);
        }
    ...
    bool StakingContractMachine<traits>::precompile_compound()
    {
        ...
        auto const val_stake_after =
            model_.val_execution(val_id).stake().load().native();
        KINET_ASSERT(val_stake_after - val_stake_before == rewards);
    ...
    StakingContractMachine<traits>::gen_precompile_withdraw_input()
    {
        if (all_withdrawal_requests_.empty()) {
            ...
            auto const [val_id, stake, wid, sender, value] = *input;
            model_precompile_undelegate(val_id, stake, wid, sender, value);
            KINET_ASSERT(!all_withdrawal_requests_.empty());
        }
    ...
    bool StakingContractMachine<traits>::precompile_withdraw()
    {
        ...
        auto const error_bound_after = model_.error_bound();
        KINET_ASSERT(error_bound_after == error_bound_before + 1);
    ...
    void StakingContractMachine<traits>::precompile_claim_rewards()
    {
        ...
        auto const staking_balance_after = model_.balance_of(STAKING_CA);
        KINET_ASSERT(staking_balance_before - staking_balance_after == reward);

        auto const sender_balance_after = model_.balance_of(sender);
        KINET_ASSERT(sender_balance_after - sender_balance_before == reward);
    ...
    void StakingContractMachine<traits>::precompile_get_delegator()
    {
        ...
        KINET_ASSERT(model_.error_bound() == error_bound_before + 3);

        KINET_ASSERT(
            model_.delegator(val_id, addr).rewards().load().native() ==
            delegator_rewards_before + unaccumulated_rewards_before);

        KINET_ASSERT(model_.unaccumulated_rewards(val_id, addr) == 0);
    }

    EXPLICIT_KINET_TRAITS_CLASS(StakingContractMachine)
}
