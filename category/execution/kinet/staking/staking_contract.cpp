...
KINET_STAKING_ANONYMOUS_NAMESPACE_BEGIN
...
KINET_STAKING_ANONYMOUS_NAMESPACE_END
...
StakingContract::pull_delegator_up_to_date(u64_be const val_id, Delegator &del)
{
    ...
    if (KINET_UNLIKELY(can_compound_boundary)) {
        KINET_ASSERT_THROW(
            can_compound,
    ...
    if (KINET_UNLIKELY(can_compound)) {
        BOOST_OUTCOME_TRY(auto rewards, apply_compound(val_id, del));
        BOOST_OUTCOME_TRY(reward_invariant(val, rewards));
        BOOST_OUTCOME_TRY(
            u256_be const new_rewards,
...
{
    if (KINET_UNLIKELY(input.size() < 4)) {
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_dispatch);
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_get_delegations);
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_get_delegators);
    ...
    byte_string_view input, Address const &, uint256_be_t const &msg_value)
{
    ...
    if (KINET_UNLIKELY(commission.native() > limits::max_commission())) {
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_add_validator)
...
{
    auto val = vars.val_execution(val_id);
    if (KINET_UNLIKELY(!val.exists())) {
        return StakingError::UnknownValidator;
    }

    if (KINET_UNLIKELY(stake < limits::dust_threshold())) {
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::delegate);
    ...
    uint256_be_t const &msg_value)
{
    ...
    auto withdrawal_request =
        vars.withdrawal_request(val_id, msg_sender, withdrawal_id)
            .load_checked();
    if (KINET_UNLIKELY(withdrawal_request.has_value())) {
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_undelegate);
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::precompile_compound);
    ...
    uint256_be_t const &msg_value)
{
    ...
    if (KINET_LIKELY(rewards.native() != 0)) {
    ...
    uint256_be_t const &msg_value)
{
    ...
    if (KINET_UNLIKELY(!validator.exists())) {
        return StakingError::UnknownValidator;
    }

    if (KINET_UNLIKELY(msg_sender != validator.auth_address())) {
        return StakingError::RequiresAuthAddress;
    }

    if (KINET_UNLIKELY(new_commission.native() > limits::max_commission())) {
        return StakingError::CommissionTooHigh;
    }

    // set in execution view. will go live next epoch.
    u256_be const old_commission = validator.commission().load();
    if (KINET_LIKELY(old_commission != new_commission)) {
    ...
    byte_string_view input, uint256_t const &value)
{
    ...
    if (KINET_UNLIKELY(next_epoch.native() <= last_epoch.native())) {
        LOG_ERROR(
    ...
    byte_string_view input, uint256_t const &raw_reward)
{
    ...
    if (KINET_UNLIKELY(!val_id.has_value())) {
        return StakingError::NotInValidatorSet;
    }
...
EXPLICIT_KINET_TRAITS_MEMBER(StakingContract::syscall_reward);
    ...
    byte_string_view input, uint256_t const &value)
{
    ...
    if (KINET_UNLIKELY(vars.in_epoch_delay_period.load())) {
    ...
    for (uint64_t i = 0; i < execution_valset_length; ++i) {
        ...
        if (KINET_LIKELY(flags == ValidatorFlagsOk)) {
    ...
    auto cmp = [](Candidate const &a, Candidate const &b) {
        if (KINET_LIKELY(a.second != b.second)) {
