...
// kinet::commit_block helper. Both Dbs get the same header; pre fork it
...
namespace
{
    ...
    // This forces KinetCommitBuilder to merge two slot deltas onto the same
    ...
    // KINET_TEN literal out of the call sites.
    using PreMip8Fork = KinetTraits<KINET_ZERO>;
    using PostMip8Fork = KinetTraits<KINET_TEN>;
    static_assert(
    ...
    {
        KINET_ASSERT(!tdb1.is_page_encoded() && tdb2.is_page_encoded());
...
TEST(MigrationFork, dual_write_state_root_handoff)
{
    ...
    mpt::Db db2 =
        db1.activate_secondary_timeline(std::make_unique<KinetOnDiskMachine>());
    TrieDb tdb1{db1, true /* enable_multi_block_cache */};
    TrieDb tdb2{db2};
    ASSERT_TRUE(tdb1.is_page_encoded() == false);
    ASSERT_TRUE(tdb2.is_page_encoded() == true);

    GenesisState const GENESIS_STATE = KinetTestnet{}.get_genesis_state();
