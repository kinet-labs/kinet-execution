...
TYPED_TEST(KinetTraitsTest, system_txn_comes_after_user_txn)
{
    ...
    auto const res = static_validate_kinet_body<Trait>(senders, txns);
    if constexpr (Trait::kinet_rev() < KINET_FOUR) {
        EXPECT_FALSE(res.has_error());
    }
    else {
        ASSERT_TRUE(res.has_error());
        EXPECT_EQ(
            res.error(), KinetBlockError::SystemTransactionNotFirstInBlock);
    }
}

TYPED_TEST(KinetTraitsTest, system_txns_then_user_txns_ok)
{
    std::vector<Address> senders{
        SYSTEM_SENDER,
        SYSTEM_SENDER,
    ...
    auto const res =
        static_validate_kinet_body<typename TestFixture::Trait>(senders, txns);
    EXPECT_FALSE(res.has_error());
}

TYPED_TEST(KinetTraitsTest, valid_syscall_ordering)
{
    ...
    for (auto const &order : orders) {
        ...
        auto const res =
            static_validate_kinet_body<typename TestFixture::Trait>(
                senders, txns);
        EXPECT_FALSE(res.has_error());
    }
}

TYPED_TEST(KinetTraitsTest, invalid_syscall_ordering)
{
    using Trait = typename TestFixture::Trait;
    ...
    for (auto const &order : orders) {
        ...
        if constexpr (Trait::kinet_rev() < KINET_FOUR) {
            ...
            ASSERT_TRUE(res.has_error());
            EXPECT_EQ(
                res.error(), KinetBlockError::SystemTransactionOutOfOrder);
        }
    }
}

TYPED_TEST(KinetTraitsTest, duplicate_syscalls_error)
{
    using Trait = typename TestFixture::Trait;
    ...
    for (auto const &order : orders) {
        ...
        if constexpr (Trait::kinet_rev() < KINET_FOUR) {
            ...
            ASSERT_TRUE(res.has_error());
            EXPECT_EQ(res.error(), KinetBlockError::DuplicateSystemTransaction);
        }
    }
}

TYPED_TEST(KinetTraitsTest, unknown_syscall_error)
{
    using Trait = typename TestFixture::Trait;
    ...
    for (auto const &data : datas) {
        ...
        if constexpr (Trait::kinet_rev() < KINET_FOUR) {
            ...
            ASSERT_TRUE(res.has_error());
            EXPECT_EQ(res.error(), KinetBlockError::UnknownSystemTransaction);
        }
    }
}

TYPED_TEST(KinetTraitsTest, reward_txn_exceeds_maximum)
{
    using Trait = typename TestFixture::Trait;
    ...
    if constexpr (Trait::kinet_rev() < KINET_FOUR) {
        ...
        ASSERT_TRUE(res.has_error());
        EXPECT_EQ(res.error(), KinetBlockError::InvalidRewardValue);
    }
}

TYPED_TEST(KinetTraitsTest, mip12_migration)
{
    using Trait = typename TestFixture::Trait;
...
using ValidateKinetBlockLatest =
    KinetTraitsTest<::detail::KinetRevisionConstant<KINET_NEXT>>;
...
class BlockRewardLatest
    : public ValidateKinetBlockLatest
