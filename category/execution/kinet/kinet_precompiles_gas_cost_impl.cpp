...
{
    // Kinet specification §4.3: Precompiles
    static constexpr auto pricing_factor =
        traits::kinet_pricing_version() >= 1 ? 2 : 1;

    return pricing_factor * 3'000;
}

EXPLICIT_KINET_TRAITS(ecrecover_gas_cost);
...
EXPLICIT_KINET_TRAITS(ecadd_gas_cost);
...
uint64_t ecmul_gas_cost(byte_string_view const)
{
    // Kinet specification §4.3: Precompiles
    static constexpr auto pricing_factor =
        traits::kinet_pricing_version() >= 1 ? 5 : 1;
...
EXPLICIT_KINET_TRAITS(ecmul_gas_cost);
...
EXPLICIT_KINET_TRAITS(snarkv_gas_cost);
...
EXPLICIT_KINET_TRAITS(blake2bf_gas_cost);
...
uint64_t point_evaluation_gas_cost(byte_string_view)
{
    // Kinet specification §4.3: Precompiles
    static constexpr auto pricing_factor =
        traits::kinet_pricing_version() >= 1 ? 4 : 1;

    return pricing_factor * 50'000;
}

EXPLICIT_KINET_TRAITS(point_evaluation_gas_cost);
