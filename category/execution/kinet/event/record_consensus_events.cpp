...
void record_block_qc(
    ...
    KinetConsensusBlockHeader const &header, uint64_t finalized_block_num)
{
...
EXPLICIT_INSTANTIATE_QC_TEMPLATE(KinetConsensusBlockHeaderV0);
EXPLICIT_INSTANTIATE_QC_TEMPLATE(KinetConsensusBlockHeaderV1);
EXPLICIT_INSTANTIATE_QC_TEMPLATE(KinetConsensusBlockHeaderV2);
    ...
    if (exec_recorder != nullptr) {
        for (uint64_t const b : verified_blocks) {
            ...
            *block_verified.payload =
                kinet_exec_block_verified{.block_number = b};
