...
template <>
struct quick_status_code_from_enum<KINET_ASYNC_NAMESPACE::sender_errc>
    : quick_status_code_from_enum_defaults<KINET_ASYNC_NAMESPACE::sender_errc>
{
    ...
    static std::initializer_list<mapping> const &value_mappings()
    {
        static std::initializer_list<mapping> const v = {
            {KINET_ASYNC_NAMESPACE::sender_errc::operation_must_be_reinitiated,
             "operation_must_be_reinitiated",
             {}},
            {KINET_ASYNC_NAMESPACE::sender_errc::
                 initiation_immediately_completed,
             "initiation_immediately_completed",
             {}},
            {KINET_ASYNC_NAMESPACE::sender_errc::unknown, "unknown", {}},
        };
        return v;
    }
};
...
protected:
    ...
    virtual bool _do_equivalent(
        ...
        const noexcept override
    {
        KINET_ASSERT(code1.domain() == *this);
        auto const &c1 =
    ...
    struct thread_local_sender_errc_with_payload_code_allocator
    {
        ...
        [[nodiscard]] value_type *allocate(size_type n)
        {
            ...
            if (!tls.in_use.exchange(true, std::memory_order_acq_rel)) {
                KINET_ASSERT(size_type(-1) / sizeof(value_type) >= n);
