...
struct shared_state_t
{
    ...
    constexpr explicit shared_state_t(KINET_ASYNC_NAMESPACE::AsyncIO &io)
        : chunk_sizes_div_disk_page_size([&] {
            ...
            for (uint32_t n = 0; n < uint32_t(ret.size()); n++) {
                ret[n] = {
                    ...
                    uint32_t(
                        io.storage_pool()
                            .chunk(KINET_ASYNC_NAMESPACE::storage_pool::seq, n)
                            .size() /
                        KINET_ASYNC_NAMESPACE::DISK_PAGE_SIZE)};
                if (ret[n].second > 0) {
    ...
    KINET_ASYNC_NAMESPACE::file_offset_t test_surface_available() const noexcept
    {
        KINET_ASYNC_NAMESPACE::file_offset_t ret = 0;
        for (auto const &i : chunk_sizes_div_disk_page_size) {
            ret += i.second;
        }
        ret *= KINET_ASYNC_NAMESPACE::DISK_PAGE_SIZE;
        return ret;
    }

    KINET_ASYNC_NAMESPACE::chunk_offset_t add_op(uint64_t const elapsed_ns)
    {
        ops++;
        ...
        return KINET_ASYNC_NAMESPACE::chunk_offset_t(
            chunk_id,
            offset_into_chunk * KINET_ASYNC_NAMESPACE::DISK_PAGE_SIZE);
    }
};
...
struct receiver_t
{
    ...
    inline void set_value(
        KINET_ASYNC_NAMESPACE::erased_connected_operation *rawstate,
        KINET_ASYNC_NAMESPACE::read_single_buffer_sender::result_type buffer);

    void reset() {}
};

using connected_state_ptr_type =
    KINET_ASYNC_NAMESPACE::AsyncIO::connected_operation_unique_ptr_type<
        KINET_ASYNC_NAMESPACE::read_single_buffer_sender, receiver_t>;

inline void receiver_t::set_value(
    KINET_ASYNC_NAMESPACE::erased_connected_operation *const rawstate,
    KINET_ASYNC_NAMESPACE::read_single_buffer_sender::result_type const buffer)
{
    if (!buffer) {
    ...
    if (!shared->done) {
        ...
        state->reset(
            std::tuple{offset, KINET_ASYNC_NAMESPACE::DISK_PAGE_SIZE},
            std::tuple{});
...
set it to the desired size beforehand).
        ...
        try {
            ...
            kinet::io::RingConfig ringconfig{128};
            ...
            if (highest_io_priority) {
                ...
                KINET_ASSERT(CAP_IS_SUPPORTED(CAP_SYS_NICE));
                auto *caps = cap_get_proc();
                KINET_ASSERT(caps != nullptr);
                auto const uncaps =
                    kinet::make_scope_exit([&]() noexcept { cap_free(caps); });
...
#else // KINET_HAVE_LIBCAP
            KINET_ASSERT(!highest_io_priority); // Not supported without libcap
#endif
            ...
            kinet::io::Ring ring(ringconfig);
            kinet::io::Buffers rwbuf = kinet::io::make_buffers_for_read_only(
                ring,
                concurrent_io,
                KINET_ASYNC_NAMESPACE::AsyncIO::READ_BUFFER_SIZE);
            ...
            if (destroy_and_really_fill_count > 0) {
                ...
                kinet::HugeMem storage;
                auto make_storage = [&](size_t bytes) {
                    storage = kinet::HugeMem{bytes};
                    buffer = {
...

            {
                ...
                for (size_t n = 0; n < concurrent_io; n++) {
                    states.push_back(io.make_connected(
                        KINET_ASYNC_NAMESPACE::read_single_buffer_sender(
                            {0, 0}, 0),
                ...
                for (auto &i : states) {
                    KINET_ASYNC_NAMESPACE::filled_read_buffer res;
                    if (highest_io_priority) {
                        i->set_io_priority(
