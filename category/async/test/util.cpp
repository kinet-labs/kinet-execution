...
namespace
{
    ...
    using KINET_ASYNC_NAMESPACE::detail::resolve_working_temporary_directory;

    constexpr char const *ENV = "KINET_TMPDIR_FORCE";

    // Save and restore KINET_TMPDIR_FORCE around each test so cases are
...
{
    std::string templ =
        (fs::temp_directory_path() / "kinet_wtd_XXXXXX").native();
...
TEST_F(WorkingTempDir, ForceHonoredForTmpfs)
{
    ...
    std::string templ = (shm / "kinet_wtd_tmpfs_XXXXXX").native();
    char const *const p = ::mkdtemp(templ.data());
    ASSERT_NE(p, nullptr);
    fs::path const dir{p};
    ::setenv(ENV, dir.c_str(), 1);
...
{
    ::setenv(ENV, "/nonexistent/kinet/tmpdir", 1);
...
{
    std::string templ =
        (fs::temp_directory_path() / "kinet_wtd_file_XXXXXX").native();
