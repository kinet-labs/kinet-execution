...
{
    KINET_ASSERT(!is_zoned_device(), "zonefs support isn't implemented yet");
...
std::pair<file_offset_t, file_offset_t> storage_pool::device_t::capacity() const
{
    ...
    case device_t::type_t_::zoned_device:
        KINET_ABORT("zonefs support isn't implemented yet");
    default:
        KINET_ABORT();
    }
}
...
{
    bytes = std::min(uint32_t(size()), bytes);
    KINET_ASSERT(capacity_ <= std::numeric_limits<off_t>::max());
    KINET_ASSERT(offset_ <= std::numeric_limits<off_t>::max());
    if (device().is_file()) {
        KINET_ASSERT_PRINTF(
            -1 != ::fallocate(
                      write_fd_,
    ...
    if (device().is_block_device()) {
        ...
        KINET_ASSERT(remainder < DISK_PAGE_SIZE);
        if (remainder > 0) {
        ...
        if (range[1] > 0) {
            KINET_ASSERT(range[0] >= offset_ && range[0] < offset_ + capacity_);
            KINET_ASSERT(range[1] <= capacity_);
            KINET_ASSERT((range[1] & (DISK_PAGE_SIZE - 1)) == 0);
            KINET_ASSERT_PRINTF(
    ...
    creation_flags const flags)
{
    ...
    size_t total_size = 0;
    {
        ...
        KINET_ASSERT(offset <= std::numeric_limits<off_t>::max());
        KINET_ASSERT(static_cast<size_t>(stat.st_size) > offset);
        ...
        if (memcmp(metadata_footer->magic, "MND0", 4) != 0 ||
            ...
            for (off_t offset2 = static_cast<off_t>(
                     offset - round_up_align<DISK_PAGE_BITS>(
                                  (kinet::async::file_offset_t(stat.st_size) /
    ...
    KINET_ASSERT(0 == memcmp(metadata->magic, "MND0", 4));
...
void storage_pool::fill_chunks_(creation_flags const &flags)
{
    ...
    for (auto const &device : devices_) {
        if (device.is_file() || device.is_block_device()) {
            ...
            KINET_ASSERT_PRINTF(
                ...
                devicechunks,
                cnv_chunks_count + 1);
            KINET_ASSERT(devicechunks <= std::numeric_limits<uint32_t>::max());
        ...
        if (device.metadata_->config_hash == 0) {
            ...
            if (!flags.disable_mismatching_storage_pool_check) {
                KINET_ABORT_PRINTF(
                    ...
                    "was first created?\n\nYou should use the kinet-mpt tool "
    ...
    if (flags.interleave_chunks_evenly) {
        ...
        for (size_t n = 0; n < chunks.size(); n++) {
            ...
            KINET_ASSERT(chunks[n] == devicechunks);
        }
#endif
    }
    else {
    ...
    for (auto const &src_device : src->devices_) {
        devices_.push_back([&] {
            ...
            if (src_device.is_zoned_device()) {
                KINET_ABORT("zonefs support isn't actually implemented yet");
            }
            KINET_ABORT();
        }());
    }
...
storage_pool::chunk(chunk_type const which, uint32_t const id)
{
    ...
    if (id >= chunks_[which].size()) {
        KINET_ABORT("Requested chunk which does not exist");
    }
    ...
    uint32_t const id_within_zone)
{
    ...
    chunk_t const ret = [&]() {
        ...
        KINET_ABORT_PRINTF("chunk type not supported: %d", which);
    }();
    KINET_ASSERT_PRINTF(
