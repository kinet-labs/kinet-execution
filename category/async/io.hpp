...
private:
    ...
    kinet::io::Ring &uring_, *wr_uring_{nullptr};
    kinet::io::Buffers &rwbuf_;
    kinet::io::BufferPool rd_pool_;
    kinet::io::BufferPool wr_pool_;
...
public:
    AsyncIO(class storage_pool &pool, kinet::io::Buffers &rwbuf);
    ...
    {
        KINET_ASSERT(storage_pool_ != nullptr);
    ...
    */
    ...
    static constexpr size_t KINET_IO_BUFFERS_READ_SIZE = READ_BUFFER_SIZE;
    static constexpr size_t KINET_IO_BUFFERS_WRITE_SIZE = WRITE_BUFFER_SIZE;
...
#pragma GCC diagnostic pop
    ...
    {
        KINET_ASSERT(bytes <= READ_BUFFER_SIZE);
...
private:
    ...
    auto make_connected_impl_(F &&connect)
    {
        ...
        KINET_ASSERT(ret->sender().buffer().data() == nullptr);
        if constexpr (is_write) {
            KINET_ASSERT(rwbuf_.get_write_size() >= WRITE_BUFFER_SIZE);
            ...
            KINET_ASSERT(rwbuf_.get_read_size() >= READ_BUFFER_SIZE);
        }
        return ret;
    }
...
public:
    ...
    {
        (void)state;
#if KINET_TRIE_ENABLE_WRITEBACK_CACHE
        if constexpr (detail::connected_operation_storage<
            ...
            KINET_ASSERT(p->key == state->sender().offset().raw());
