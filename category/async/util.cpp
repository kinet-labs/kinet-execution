...
std::filesystem::path detail::resolve_working_temporary_directory()
{
    ...
    auto test_path = [&](std::filesystem::path const &path) -> bool {
        ...
        if (-1 == fd && ENOTSUP == errno) {
            auto const path2 = path / "kinet_XXXXXX";
    ...
    if (getuid() == geteuid() && getgid() == getegid()) {
        ...
        // (e.g. tests and fuzzers): if KINET_TMPDIR_FORCE names a writable
...
int make_temporary_inode() noexcept
{
    ...
    if (-1 == fd && ENOTSUP == errno) {
        ...
        auto const buffer = working_temporary_directory() / "kinet_XXXXXX";
        fd = mkstemp(const_cast<char *>(buffer.native().c_str()));
        if (-1 != fd) {
            unlink(buffer.c_str());
        }
    }
    KINET_ASSERT_PRINTF(fd != -1, "failed due to %s", strerror(errno));
    return fd;
}
