...
#define KINET_ASYNC_IO_URING_RETRYABLE2(unique, ...)                           \
    ({                                                                         \
        int unique;                                                            \
        for (;;) {                                                             \
            unique = (__VA_ARGS__);                                            \
            if (unique < 0) {                                                  \
                ...
                KINET_ABORT_PRINTF("FATAL: %s", buffer)                        \
            }                                                                  \
            break;                                                             \
        }                                                                      \
        unique;                                                                \
    })
#define KINET_ASYNC_IO_URING_RETRYABLE(...)                                    \
    KINET_ASYNC_IO_URING_RETRYABLE2(BOOST_OUTCOME_TRY_UNIQUE_NAME, __VA_ARGS__)

KINET_ASYNC_NAMESPACE_BEGIN
...
AsyncIO::AsyncIO(class storage_pool &pool, kinet::io::Buffers &rwbuf)
    ...
    , rwbuf_(rwbuf)
    , rd_pool_(kinet::io::BufferPool(rwbuf, true))
    , wr_pool_(kinet::io::BufferPool(rwbuf, false))
{
    ...
    for (size_t n = 0; n < count; n++) {
        ...
        KINET_ASSERT_PRINTF(
            seq_chunks_.back().chunk.capacity() >= KINET_IO_BUFFERS_WRITE_SIZE,
    ...
    */
    ...
    auto replace_fds_with_iouring_fds = [&](auto &p) {
        ...
        KINET_ASSERT(it != fd_to_iouring_map.end());
...
AsyncIO::~AsyncIO()
{
    ...
    if (wr_uring_ != nullptr) {
        KINET_ASSERT(!io_uring_unregister_files(&wr_uring_->get_ring()));
    }
    KINET_ASSERT(!io_uring_unregister_files(&uring_.get_ring()));
}
...
{
    KINET_ASSERT(sqe != nullptr);
    KINET_ASSERT(uring_data != nullptr);
    KINET_ASSERT((chunk_and_offset.offset & (DISK_PAGE_SIZE - 1)) == 0);
    KINET_ASSERT(buffer.size() <= READ_BUFFER_SIZE);
    ...
    enum erased_connected_operation::io_priority const prio)
{
    ...
    KINET_ASYNC_IO_URING_RETRYABLE(io_uring_submit(&uring_.get_ring()));
}
...
{
    KINET_ASSERT(uring_data != nullptr);
    KINET_ASSERT(!rwbuf_.is_read_only());
    KINET_ASSERT((chunk_and_offset.offset & (DISK_PAGE_SIZE - 1)) == 0);
    KINET_ASSERT(buffer.size() <= WRITE_BUFFER_SIZE);
    ...
    */
    ...
    io_uring_sqe_set_data(sqe, uring_data);
    KINET_ASYNC_IO_URING_RETRYABLE(io_uring_submit(wr_ring));
}
...
void AsyncIO::poll_uring_while_submission_queue_full_()
{
    ...
    while (io_uring_sq_space_left(ring) == 0) {
        ...
        KINET_ASSERT(io_uring_sqring_wait(ring) >= 0);
    }
}
...
size_t AsyncIO::poll_uring_(bool blocking, unsigned const poll_rings_mask)
{
    ...
    // completions
    KINET_ASSERT((poll_rings_mask & 3) != 3);
    auto const h = detail::AsyncIO_per_thread_state().enter_completions();
    KINET_ASSERT(owning_tid_ == get_tl_tid());
        ...
        if (concurrent_read_io_limit_ > 0) {
            while (!concurrent_read_ios_pending_.empty() &&
                ...
                *sqe = stored_sqe;
                ...
                KINET_ASYNC_IO_URING_RETRYABLE(io_uring_submit(other_ring));
                ...
                if (sqe->opcode == IORING_OP_READ_FIXED ||
                    ...
                    KINET_ABORT("unexpected opcode in deferred read");
                }
    ...
    auto get_cqe = [&] {
        if (wr_ring != nullptr && records_.inflight_wr > 0 &&
            ...
            if ((poll_rings_mask & 1) != 0) {
                if (blocking && detail::AsyncIO_per_thread_state().empty()) {
                    KINET_ASYNC_IO_URING_RETRYABLE(
        ...
        if (cqe == nullptr) {
            ...
            if (blocking && records_.inflight_wr == 0 &&
                ...
                KINET_ASYNC_IO_URING_RETRYABLE(io_uring_wait_cqe(ring, &cqe));
            }
            else {
...
void AsyncIO::dump_fd_to(size_t const which, std::filesystem::path const &path)
{
    ...
    KINET_ASSERT(fromfd.second <= std::numeric_limits<off64_t>::max());
    ...
    for (;;) {
        ...
        if (0 == io_in_flight()) {
            std::cerr
                ...
                << std::endl;
            KINET_ABORT("no i/o buffers remaining");
        }
