...
namespace detail
{
    ...
    inline void initiate_async_read_update(
        KINET_ASYNC_NAMESPACE::AsyncIO &io, Receiver &&receiver,
        size_t bytes_to_read)
    {
        [[likely]] if (
            bytes_to_read <= KINET_ASYNC_NAMESPACE::AsyncIO::READ_BUFFER_SIZE) {
    ...
    inline Node::SharedPtr deserialize_node_from_receiver_result(
        ...
        KINET_ASYNC_NAMESPACE::erased_connected_operation *io_state)
    {
        KINET_ASSERT(buffer_);
        Node::SharedPtr node;
        if constexpr (std::is_same_v<
                          std::decay_t<ResultType>,
                          typename kinet::async::read_single_buffer_sender::
                              result_type>) {
            auto &buffer = std::move(buffer_).assume_value().get();
            KINET_ASSERT(buffer.size() > buffer_off);
