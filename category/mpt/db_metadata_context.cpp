...
#endif
// KINET007 -> KINET008 fixed-header offsets used by the in-place migration
// in DbMetadataContext::DbMetadataContext. KINET007 had
// root_offsets_ring_t::SIZE_ = 65536; KINET008 dropped it to 32, shrinking
...
namespace detail
{
        ...
        db_metadata *const m, uint32_t const chunk_count)
    {
        ...
        KINET_ASSERT(
            ...
            "KINET007 pool has more primary cnv chunks than KINET008's "
        ...
        (void)memcpy(
            ...
            m_bytes + KINET007_LIST_TRIPLE_OFFSET,
            DB_METADATA_LIST_TRIPLE_BYTES);
        std::array<std::byte, KINET007_DB_OFFSETS_THROUGH_BLOCK_IDS_BYTES>
            db_offsets_buf{};
        (void)memcpy(
            db_offsets_buf.data(),
            m_bytes + KINET007_DB_OFFSETS_OFFSET,
            KINET007_DB_OFFSETS_THROUGH_BLOCK_IDS_BYTES);

        // 2. Relocate chunk_info[]. memmove because src/dst may overlap for
        //    pathologically large chunk_count.
        (void)memmove(
            m_bytes + sizeof(db_metadata),
            m_bytes + KINET007_DB_METADATA_SIZE,
        ...
        //    dropping KINET007's global auto_expire_version (sandwiched
        //    between latest_proposed_version and latest_voted_block_id) since
        //    KINET008 promotes it to a per-timeline field in
        //    timeline_state_t. Save the value, then copy the two halves
        //    either side of the gap into the new gap-less layout. Offsets
        //    into db_offsets_buf are relative to KINET007_DB_OFFSETS_OFFSET.
        static_assert(offsetof(db_metadata, db_offsets) == 312);
        static_assert(offsetof(db_metadata, secondary_timeline) == 432);
        constexpr size_t KINET007_AUTO_EXPIRE_OFFSET =
            KINET007_DB_OFFSETS_OFFSET + 16 + 5 * 8; // 524384
        constexpr size_t KINET007_PRE_AUTO_EXPIRE_BYTES =
            KINET007_AUTO_EXPIRE_OFFSET - KINET007_DB_OFFSETS_OFFSET; // 56
        constexpr size_t KINET007_POST_AUTO_EXPIRE_BYTES =
            KINET007_DB_OFFSETS_THROUGH_BLOCK_IDS_BYTES -
            KINET007_PRE_AUTO_EXPIRE_BYTES - 8; // 64
        int64_t saved_auto_expire = 0;
        (void)memcpy(
            &saved_auto_expire,
            db_offsets_buf.data() + KINET007_PRE_AUTO_EXPIRE_BYTES,
            sizeof(saved_auto_expire));
        (void)memcpy(
            m_bytes + offsetof(db_metadata, db_offsets),
            db_offsets_buf.data(),
            KINET007_PRE_AUTO_EXPIRE_BYTES);
        (void)memcpy(
            m_bytes + offsetof(db_metadata, latest_voted_block_id),
            db_offsets_buf.data() + KINET007_PRE_AUTO_EXPIRE_BYTES + 8,
            KINET007_POST_AUTO_EXPIRE_BYTES);
        // KINET007 had no secondary, so the saved value belongs on what is now
        // the primary.
        ...
        // KINET007's huge cnv_chunks[] (0xff for unused slots), which would
        // trip create_state_machine()'s range check on the next open. KINET007
        ...
        //    is unreachable until a later kinet-mpt --activate-secondary
    ...
    : io_(&io)
{
    ...
    // metadata layout (KINET007's 528 KiB header or KINET008's 4.4 KiB
    ...
    KINET_ASSERT(
        metadata_mmap_size_ >=
            KINET007_DB_METADATA_SIZE +
                chunk_count * sizeof(detail::db_metadata::chunk_info_t),
        "cnv chunk 0 is too small to hold a KINET007 metadata header plus "
    ...
    copies_[0].main = start_lifetime_as<detail::db_metadata>(::mmap(
        ...
        fd.first,
        off_t(fdr.second)));
    KINET_ASSERT(copies_[0].main != MAP_FAILED);
    ...
    KINET_ASSERT(copies_[1].main != MAP_FAILED);
    ...
    // Migration: KINET007 -> KINET008. The fixed header shrank from
    ...
    // step 4 (auto_expire from KINET007's global, state_machine_kind stamped
    // ethereum). The block at [432, 4456) — secondary_timeline,
    // secondary_timeline_state, the role/identity bytes and
    // pending_shrink_grow — was KINET007 padding and is zeroed so new fields
    ...
    if (is_previous_magic(copies_[0].main) ||
        ...
        if (!io_->storage_pool().is_migration_allowed()) {
            KINET_ABORT_PRINTF(
                ...
                "%s. Run 'kinet-mpt --upgrade --storage <path>' before "
                "starting kinet services.",
                detail::db_metadata::PREVIOUS_MAGIC,
                detail::db_metadata::MAGIC);
        }
        ...
        for (auto const &copy : copies_) {
            ...
            detail::migrate_kinet007_to_kinet008(
    ...
    if (0 != memcmp(
        ...
        KINET_ASSERT((chunk_count & ~0xfffffU) == 0);
        ...
        KINET_ASSERT(cnv_chunks_total > UpdateAux::cnv_chunks_for_db_metadata);
        ...
        KINET_ASSERT(tofill != nullptr);
        auto const untofill =
            kinet::make_scope_exit([&]() noexcept { ::free(tofill); });
    ...
    uint64_t const history_len) noexcept
{
    auto do_ = [&](unsigned const which) {
        ...
        KINET_ASSERT(history_len > 0 && history_len <= ro.capacity());
    ...
    timeline_id const tid) noexcept
{
    auto do_ = [&](unsigned const which) {
        ...
        if (root_offset == INVALID_OFFSET && i == db_history_max_version(tid) &&
            ...
            ro.reset_all(0);
            KINET_ASSERT(ro.max_version() == INVALID_BLOCK_NUM);
        }
    };
    do_(0);
    do_(1);
...
{
    KINET_ASSERT(main()->at(fast_offset.id)->in_fast_list);
    KINET_ASSERT(main()->at(slow_offset.id)->in_slow_list);
    auto do_ = [&](unsigned const which) {
...
{
    KINET_ASSERT(can_write_to_map_);
...
namespace
{
    ...
    void install_chunk_mapping_(
        KINET_ASYNC_NAMESPACE::AsyncIO &io, std::span<chunk_offset_t> ring_span,
        ...
        int mapflags, bool can_write, bool second_copy)
    {
        ...
        KINET_ASSERT_PRINTF(
            ...
            "pre-fix binary, run 'kinet-mpt --repair'.",
            slot_index);
        auto &chunk = io.storage_pool().chunk(
            KINET_ASYNC_NAMESPACE::storage_pool::cnv, cnv_chunk_id);
...
void DbMetadataContext::sync_metadata_to_disk_()
{
    ...
    auto &cnv_chunk_0 =
        io_->storage_pool().chunk(KINET_ASYNC_NAMESPACE::storage_pool::cnv, 0);
        ...
        [this](detail::db_metadata::root_offsets_ring_t const &ring) {
            ...
            for (uint32_t k = 0; k < len; k++) {
                ...
                auto &chunk = io_->storage_pool().chunk(
                    KINET_ASYNC_NAMESPACE::storage_pool::cnv, id);
                auto const fdw = chunk.write_fd(0);
                KINET_ASSERT_PRINTF(
                    0 == ::fsync(fdw.first),
...
{
    KINET_ASSERT(op_kind != detail::db_metadata::PENDING_OP_NONE);
    ...
    for (unsigned which = 0; which < 2; which++) {
        ...
        if (cur_primary_chunks == old_chunks) {
            for (uint32_t k = new_chunks; k < old_chunks; k++) {
                ...
                KINET_ASSERT(chunk_id != detail::db_metadata::NULL_CHUNK);
        ...
        for (uint32_t k = 0; k < new_chunks; k++) {
            ...
            KINET_ASSERT(moved_id != detail::db_metadata::NULL_CHUNK);
...
{
    KINET_ASSERT(!timeline_active(timeline_id::secondary));
    KINET_ASSERT(
...
// consecutive
// #define KINET_MPT_INITIALIZE_POOL_WITH_RANDOM_SHUFFLED_CHUNKS 1
#define KINET_MPT_INITIALIZE_POOL_WITH_REVERSE_ORDER_CHUNKS 1

void DbMetadataContext::init_new_pool(
    std::optional<uint64_t> const history_len,
    uint32_t const initial_insertion_count)
{
    KINET_ASSERT(is_new_pool_);
    auto const chunk_count = io_->chunk_count();
    ...
    for (uint32_t n = 0; n < chunk_count; n++) {
        auto const chunk = io_->storage_pool().chunk(storage_pool::seq, n);
        KINET_ASSERT(chunk.zone_id().first == storage_pool::seq);
        KINET_ASSERT(chunk.zone_id().second == n);
        KINET_ASSERT(chunk.size() == 0); // chunks must actually be free
        chunks.push_back(n);
    }

#if KINET_MPT_INITIALIZE_POOL_WITH_REVERSE_ORDER_CHUNKS
    std::reverse(chunks.begin(), chunks.end());
    LOG_INFO(
        "Initialize db pool with {} chunks in reverse order.", chunk_count);
#elif KINET_MPT_INITIALIZE_POOL_WITH_RANDOM_SHUFFLED_CHUNKS
    ...
    auto append_with_insertion_count_override = [&](chunk_list list,
        ...
        KINET_ASSERT(i->index(copies_[0].main) == id);
    };
    ...
    for (uint32_t const i : chunks_after_second) {
        ...
        KINET_ASSERT(i_->index(copies_[0].main) == i);
    }
    ...
    // from earlier on-disk formats (the KINET007->008 migration also
    // zeros this region).
    for (auto const i : {0, 1}) {
        auto *const m = copies_[i].main;
        auto const g = m->hold_dirty();
