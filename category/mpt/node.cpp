...
{
    KINET_ASSERT(index < number_of_children());
...
{
    KINET_ASSERT(bitpacked.path_nibble_index_start <= path_nibble_index_end);
...
uint32_t Node::get_disk_size() const noexcept
{
    ...
    KINET_ASSERT(nd >= (unsigned char *)this);
    ...
    KINET_ASSERT(total_disk_size <= Node::max_disk_size);
...
unsigned Node::get_mem_size() const noexcept
{
    ...
    KINET_ASSERT(mem_size <= Node::max_size);
    return mem_size;
}
...
{
    KINET_ASSERT(is_valid());
    ptr = std::move(node);
    auto const length = compute.compute(data, *ptr);
    KINET_ASSERT(length <= std::numeric_limits<uint8_t>::max());
...
void ChildData::copy_old_child(Node *const old, unsigned const i)
{
    ...
    KINET_ASSERT(old_data.size() <= std::numeric_limits<uint8_t>::max());
    ...
    size_t const data_size, int64_t const version)
{
    ...
    for (auto const &child : children) {
        if (child.is_valid()) {
            KINET_ASSERT(mask & (1u << child.branch));
...
{
    KINET_ASSERT(disk_size > 0 && disk_size <= Node::max_disk_size);
    if (offset < Node::disk_size_bytes) { // serialize node disk size
        KINET_ASSERT(bytes_to_append <= disk_size - offset);
