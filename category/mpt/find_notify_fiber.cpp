...
namespace
{
    struct find_receiver
    {
            ...
            ResultType buffer_)
        {
            ...
            KINET_ASSERT(cont(NodeCursor{node}));
        }
    };
    ...
    struct find_owning_receiver
    {
            ...
            ResultType buffer_)
        {
            ...
            if (virtual_offset_after == virtual_offset) {
                {
                    ...
                    KINET_ASSERT(node_cache.find(acc, virtual_offset) == false);
                }
            ...
            KINET_ASSERT(it != inflights.end());
            auto pendings = std::move(it->second);
            inflights.erase(it);
            for (auto &cont : pendings) {
                KINET_ASSERT(cont(start_cursor));
            }
        }
    };
    ...
    void async_read_with_continuation(
        ...
        virtual_chunk_offset_t const virtual_offset)
    {
        KINET_ASSERT(aux.io->owning_thread_id() == get_tl_tid());
    ...
    NodeCursor const &root, NibblesView const key)
{
    ...
    KINET_ASSERT(prefix_index < key.nibble_size());
