    ...
    timeline_id const tid)
{
    ...
    auto [src_cursor, res] = find_blocking(
        ...
        tid);
    KINET_ASSERT(res == find_result::success);
    ...
    while (prefix_index < dest_prefix.nibble_size()) {
        ...
        if (node->mask & (1u << nibble)) {
            ...
            if (node->next(index) == nullptr) {
                ...
                KINET_ASSERT(next_node_ondisk != nullptr);
        ...
        KINET_ASSERT(prefix_index < std::numeric_limits<unsigned char>::max());
    ...
    if (prefix_index ==
        ...
        KINET_ASSERT(node_prefix_index == node->path_nibbles_len());
        new_node = make_node(
            src_node,
            node->path_nibble_view(),
            src_node.opt_value(),
    ...
    if (node.get() == dest_root.get()) {
        KINET_ASSERT(parents_and_indexes.empty());
