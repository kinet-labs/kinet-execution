...
public:
    ...
    public:
        root_offsets_delegator(
            ...
            , capacity_(capacity)
        {
            KINET_ASSERT(capacity_ <= root_offsets_chunks_.size());
            KINET_ASSERT_PRINTF(
                capacity_ == 0 ||
        ...
        {
            KINET_ASSERT(version < max_version());
            KINET_ASSERT(max_version() - version <= capacity_);
    ...
    explicit DbMetadataContext(KINET_ASYNC_NAMESPACE::AsyncIO &io);
    ...
    //! point for kinet-mpt --upgrade's post-migration flush. Requires the
    ...
    // at pool create time by kinet-mpt --state-machine and on
    ...
    // KINET007->KINET008 migration artifact state: some cnv_chunks[] slot holds
    ...
    // KINET007->KINET008 migration back to the NULL_CHUNK sentinel, so a later
    ...
    // mapping. Backs `kinet-mpt --repair`.
...
private:
    ...
    KINET_ASYNC_NAMESPACE::AsyncIO *io_{nullptr};
    ...
    // and >= KINET007_DB_METADATA_SIZE + chunk_info[], so migration
    // from a KINET007 pool can read and relocate chunk_info[] without
