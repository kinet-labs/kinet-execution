...
namespace detail
{
  ...
  */
    ...
    struct TraverseSender
    {
        ...
        {
            KINET_ASSERT(traverse_root != nullptr);
            ...
            async::result<void> res) noexcept
        {
            ...
            KINET_ASSERT(within_recursion_count == 0);
        ...
        Node const &node)
    {
        ...
        sender.within_recursion_count--;
        KINET_ASSERT(sender.within_recursion_count == 0);
        ...
        Node const &node, TraverseMachine &machine, unsigned char const branch)
    {
        ...
        KINET_ASSERT(machine.level < 256);
        ++machine.level;
        ...
        for (auto const [idx, branch] : NodeChildrenRange(node.mask)) {
            if (machine.should_visit(node, branch)) {
                ...
                if (next == nullptr) {
                    KINET_ASSERT(sender.aux.is_on_disk());
    ...
    size_t const concurrency_limit = 4096)
{
    ...
    struct TraverseReceiver
    {
        ...
        {
            KINET_ASSERT(traverse_completed);
            version_expired_before_traverse_complete_ =
