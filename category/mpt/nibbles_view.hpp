...
public:
    ...
    {
        KINET_ASSERT(end_nibble <= std::numeric_limits<size_type>::max());
...
public:
    ...
    {
        KINET_ASSERT((s.size() * 2) <= std::numeric_limits<size_type>::max());
    }
    ...
    // NOLINTNEXTLINE(google-explicit-constructor)
    constexpr NibblesView(kinet_hash256 const &h) noexcept
    ...
    {
        KINET_ASSERT(count == npos || count <= (nibble_size() - pos));
...
Nibbles::substr(unsigned const pos, unsigned const count) const
{
    ...
    KINET_ASSERT(ret.begin_nibble_ == 0);
    return ret;
}
