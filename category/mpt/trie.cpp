    ...
    timeline_id const tid)
{
    ...
    if (old) {
        ...
        if (sentinel->npending) {
            aux.io->flush();
            KINET_ASSERT(sentinel->npending == 0);
        }
    }
    else {
        create_new_trie_(
...
struct load_all_impl_
{
    ...
    struct receiver_t
    {
        ...
        {
            KINET_ASSERT(buffer_);
            // load node from read buffer
            {
                KINET_ASSERT(root.node->next(branch_index) == nullptr);
                root.node->set_next(
                    branch_index,
    ...
    UpdateAux &aux, StateMachine &sm, UpdateTNode *tnode, timeline_id const tid)
{
    while (!tnode->npending && tnode->parent()) {
        KINET_ASSERT(tnode->children.size()); // not a leaf
    ...
    timeline_id const tid)
{
    ...
    for (unsigned j = 0; j < number_of_children; ++j) {
        ...
        KINET_ASSERT(ver >= aux.tl(tid).curr_upsert_auto_expire_version);
        node_ver[j] = ver;
    ...
    tnode_unique_ptr tnode, timeline_id const tid, bool const might_on_disk)
{
    if (might_on_disk && tnode->number_of_children() == 1) {
        ...
        if (!child.ptr) {
            KINET_ASSERT(aux.is_on_disk());
            KINET_ASSERT(child.offset != INVALID_OFFSET);
            { // some sanity checks
                auto const virtual_child_offset =
                    aux.physical_to_virtual(child.offset);
                KINET_ASSERT(virtual_child_offset != INVALID_VIRTUAL_OFFSET);
    ...
    timeline_id const tid)
{
    ...
    KINET_ASSERT(requests.opt_leaf == std::nullopt);
    if (update.incarnation) {
        ...
        KINET_ASSERT(update.version >= old->version);
        dispatch_updates_impl_(
            aux,
            sm,
            parent,
    ...
    UpdateList &&updates, timeline_id const tid, unsigned prefix_index)
{
    ...
    if (updates.size() == 1) {
        ...
        KINET_ASSERT(update.value.has_value());
    ...
    while (true) {
        ...
        KINET_ASSERT(num_branches > 0); // because updates.size() > 1
    ...
    timeline_id const tid, unsigned prefix_index, unsigned old_prefix_index)
{
    ...
    KINET_ASSERT(old_prefix_index != INVALID_PATH_INDEX);
    ...
    while (true) {
        ...
        if (updates.size() == 1 &&
            ...
            KINET_ASSERT(old->path_nibbles_len() == old_prefix_index);
            KINET_ASSERT(old->has_value());
    ...
    timeline_id const tid)
{
    ...
    KINET_ASSERT(tnode->children.size() == size_t(std::popcount(orig_mask)));
...
void mismatch_handler_(
    ...
    timeline_id const tid)
{
    KINET_ASSERT(old_ptr);
    Node &old = *old_ptr;
    KINET_ASSERT(old.has_path());
    // Note: no leaf can be created at an existing non-leaf node
    KINET_ASSERT(!requests.opt_leaf.has_value());
...
{
    if (!node) {
        KINET_ASSERT(node_offset != INVALID_OFFSET);
        node_receiver_t recv{
    ...
    KINET_ASSERT(sm.auto_expire() == true && sm.compact() == true);
    ...
    timeline_id const tid)
{
    if (new_node == nullptr) {
        ...
        KINET_ASSERT(new_node_virtual_offset != INVALID_VIRTUAL_OFFSET);
    ...
    UpdateAux &aux, CompactTNode::unique_ptr_type tnode, timeline_id const tid)
{
    ...
    KINET_ASSERT(!min_offsets.any_below(aux.tl(tid).compact_offsets));
    TNodeBase *parent = tnode->parent();
    auto const index = tnode->index;
    if (parent->type == tnode_type::update) {
        auto *const p = static_cast<UpdateTNode *>(parent);
        KINET_ASSERT(tnode->cache_node);
    ...
    UpdateAux &aux, node_writer_unique_ptr_type &node_writer)
{
    ...
    KINET_ASSERT(ci_ != nullptr); // we are out of free blocks!
    ...
    */
    ...
    KINET_ASSERT(my_reentrancy_count >= 0);
    ...
    KINET_ASSERT(reentrancy_detection.count >= 0);
    ...
    UpdateAux &aux, node_writer_unique_ptr_type const &node_writer)
{
    ...
    KINET_ASSERT(offset <= chunk_capacity);
    ...
    if (ci_ != nullptr) {
        KINET_ASSERT(ci_ == aux.metadata_ctx().main()->free_list_end());
        aux.metadata_ctx().remove(idx);
        aux.metadata_ctx().append(
            in_fast_list ? UpdateAux::chunk_list::fast
                         : UpdateAux::chunk_list::slow,
...
retry:
    ...
    [[likely]] if (size <= remaining_bytes) { // Node can fit into current
        ...
        KINET_ASSERT(where_to_serialize != nullptr);
        ...
        while (offset_in_on_disk_node < size) {
            ...
            serialize_node_to_buffer(
                ...
                offset_in_on_disk_node);
            offset_in_on_disk_node += bytes_to_append;
            KINET_ASSERT(offset_in_on_disk_node <= size);
            KINET_ASSERT(
    ...
    UpdateAux &aux, Node &root, uint64_t const version, timeline_id const tid)
{
    ...
    if (KINET_UNLIKELY(max_version_in_db == INVALID_BLOCK_NUM)) {
        ...
        auto const prev_lower_bound =
            aux.metadata_ctx().db_history_range_lower_bound(tid);
        ...
        KINET_ASSERT(version == max_version_in_db + 1);
        ...
        if (version - aux.metadata_ctx().db_history_min_valid_version(tid) >=
            ...
            aux.erase_versions_up_to_and_including(
                version - aux.metadata_ctx().version_history_length(), tid);
            KINET_ASSERT(
