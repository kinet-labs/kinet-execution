...
using chunk_offset_t = KINET_ASYNC_NAMESPACE::chunk_offset_t;
using chunk_offset_t_hasher = KINET_ASYNC_NAMESPACE::chunk_offset_t_hasher;
using file_offset_t = KINET_ASYNC_NAMESPACE::file_offset_t;

using KINET_ASYNC_NAMESPACE::CPU_PAGE_BITS;
using KINET_ASYNC_NAMESPACE::CPU_PAGE_SIZE;
using KINET_ASYNC_NAMESPACE::DISK_PAGE_BITS;
using KINET_ASYNC_NAMESPACE::DISK_PAGE_SIZE;
using KINET_ASYNC_NAMESPACE::DMA_PAGE_BITS;
using KINET_ASYNC_NAMESPACE::DMA_PAGE_SIZE;
using KINET_ASYNC_NAMESPACE::INVALID_OFFSET;

using KINET_ASYNC_NAMESPACE::round_down_align;
using KINET_ASYNC_NAMESPACE::round_up_align;
...
struct virtual_chunk_offset_t
{
    ...
    {
        KINET_ASSERT(spare_ <= MAX_SPARE);
        KINET_ASSERT(count_ <= MAX_COUNT);
        KINET_ASSERT(offset_ <= MAX_OFFSET);
        KINET_ASSERT(is_fast_list_ <= 1);
    }
...
public:
    ...
    {
        KINET_ASSERT(offset != INVALID_VIRTUAL_OFFSET);
    }
...
{
    KINET_ASSERT(N <= sizeof(UnsignedInteger));
...
{
    return ::kinet::mpt::serialize((uint32_t)fast) +
           ::kinet::mpt::serialize((uint32_t)slow);
}

inline compact_offset_pair
compact_offset_pair::deserialize(byte_string_view const bytes)
{
    KINET_ASSERT(bytes.size() == 2 * sizeof(uint32_t));
