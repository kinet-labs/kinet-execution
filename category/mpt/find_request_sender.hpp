...
using AsyncInflightNodes = ankerl::unordered_dense::segmented_map<
    ...
    std::vector<std::function<KINET_ASYNC_NAMESPACE::result<void>(NodeCursor)>>,
...
private:
    ...
    KINET_ASYNC_NAMESPACE::result<void> resume_(
        KINET_ASYNC_NAMESPACE::erased_connected_operation *const io_state,
        NodeCursor const root)
    {
        if (!root.is_valid()) {
            // Version invalidated during async read
            res_ = {T{}, find_result::version_no_longer_exist};
            io_state->completed(KINET_ASYNC_NAMESPACE::success());
            return KINET_ASYNC_NAMESPACE::success();
        }
...
public:
    using result_type = KINET_ASYNC_NAMESPACE::result<find_result_type<T>>;
    ...
    KINET_ASYNC_NAMESPACE::result<void>
    operator()(KINET_ASYNC_NAMESPACE::erased_connected_operation *);

    result_type completed(
        KINET_ASYNC_NAMESPACE::erased_connected_operation *,
        KINET_ASYNC_NAMESPACE::result<void> res) noexcept
    {
        BOOST_OUTCOME_TRY(std::move(res));
        KINET_ASSERT(res_.has_value());
...
static_assert(KINET_ASYNC_NAMESPACE::sender<find_request_sender<byte_string>>);
...
static_assert(
    KINET_ASYNC_NAMESPACE::sender<find_request_sender<std::shared_ptr<Node>>>);
...
struct find_request_sender<T>::find_receiver
{
    ...
    constexpr find_receiver(
        ...
        KINET_ASYNC_NAMESPACE::erased_connected_operation *const io_state_,
        ...
        , branch(branch)
    {
        KINET_ASSERT(sender->root_.node->mask & (1u << branch));
    ...
    void set_value(
        KINET_ASYNC_NAMESPACE::erased_connected_operation *, ResultType buffer_)
    {
        KINET_ASSERT(buffer_);
        KINET_ASSERT(sender->root_.is_valid());
        ...
        if (it != sender->inflights_.end()) {
            ...
            for (auto const &invoc : pendings) {
                KINET_ASSERT(invoc(NodeCursor{sp}));
            }
        }
    }
};
...
template <return_type T>
inline KINET_ASYNC_NAMESPACE::result<void> find_request_sender<T>::operator()(
    KINET_ASYNC_NAMESPACE::erased_connected_operation *const io_state)
{
    ...
    */
    ...
    for (;;) {
        ...
        KINET_ASSERT(prefix_index < key_.nibble_size());
