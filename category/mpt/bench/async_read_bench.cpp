...
static kinet::byte_string to_key(uint64_t const key)
{
    auto const as_bytes = serialize_as_big_endian<sizeof(key)>(key);
    auto const hash = kinet::keccak256(as_bytes);
    return kinet::byte_string{hash.bytes, sizeof(hash.bytes)};
}

static uint64_t
select_rand_version(Db const &db, kinet::small_prng &rnd, double const bias)
{
    ...
    try {
        ...
        try {
            ...
            std::cout << "Starting benchmark with the following parameters:"
                      << std::endl;
            std::cout << "  num_async_reader_threads: "
                      << num_async_reader_threads << std::endl;
            std::cout << "  num_async_reads_inflight: "
                      << num_async_reads_inflight << std::endl;
            ...
            struct CollectKeys : TraverseMachine
            {
                ...
                explicit CollectKeys(std::vector<Nibbles> &keys)
                    : keys(keys)
                {
                }
            ...
            auto const root = ro_db.load_root_for_version(version);

            KINET_ASSERT(root != nullptr);
            ...
            kinet::byte_string const long_value(1ul << 19, 5);
            std::vector<kinet::byte_string> values_overwrite_keys_mode;
            values_overwrite_keys_mode.reserve(num_nodes_per_version);
            for (uint64_t i = 0; i < num_nodes_per_version; ++i) {
                values_overwrite_keys_mode.emplace_back(
                    long_value + serialize_as_big_endian<8>(i));
            }

            auto const prefix = 0x10_bytes;

            auto upsert_new_version = [&](uint64_t const version) {
                UpdateList ul;
                std::list<kinet::byte_string> bytes_alloc;
                ...
                rw_root = rw_db.upsert(
                    std::move(rw_root), std::move(ul_prefix), version);
            };
            ...
            auto random_async_read = [&]() {
                ...
                KINET_ASSERT(ro_db.get_latest_version() != INVALID_BLOCK_NUM);
                KINET_ASSERT(ro_db.get_earliest_version() != INVALID_BLOCK_NUM);
                ...
                struct receiver_t
                {
                    ...
                    ThreadStats &stats;
                    uint64_t version;
                    kinet::byte_string version_bytes;
                    std::chrono::steady_clock::time_point start_time;

                    void set_value(
                        kinet::async::erased_connected_operation *const state,
                        kinet::async::result<kinet::byte_string> const res)
                    {
                        if (res) {
                            ++(stats.nsuccess);
                        }
                ...
                auto rnd = kinet::thread_local_prng();
                ...
                auto const lock =
                    std::lock_guard<std::mutex>(total_stats.mutex);
                ...
                struct VersionValidatorMachine : public TraverseMachine
                {
                    ...
                    up(unsigned char const branch, Node const &node) override
                    {
                        ...
                        auto const rem_size = [&] {
                            ...
                            KINET_ASSERT(
                                path_view.substr(
                                    static_cast<unsigned>(rem_size)) ==
                                concat(branch, node.path_nibble_view()));
                            return rem_size;
                        }();
                        path = path_view.substr(
                            0, static_cast<unsigned>(rem_size));
                    }
                ...
                while (!g_done) {
                    ...
                    if (auto cursor = ro_db.find(prefix, version);
                        ...
                        if (!ro_db.traverse(cursor.value(), machine, version)) {
                            ...
                            KINET_ASSERT(version < min_version);
                            ++nfailed;
                        }
                        else {
                            ++nsuccess;
            ...
            readers.reserve(num_async_reader_threads);
            ...
            std::cout << "Writer finished. Max version in RWDb is "
                      << rw_db.get_latest_version()
                      << ", min version in RWDb is "
