...
namespace
{
    ...
    {
        KINET_ASSERT(node_disk_size > sizeof(Node) + Node::disk_size_bytes);
        auto const node_value_size =
            node_disk_size - sizeof(Node) - Node::disk_size_bytes;
        auto const value = kinet::byte_string(node_value_size, 0xf);
...
struct NodeWriterTestBase : public ::testing::Test
{
    ...
    NodeWriterTestBase()
        ...
        , rwbuf{kinet::io::make_buffers_for_segregated_read_write(
              ring1, ring2, 2, 4, AsyncIO::KINET_IO_BUFFERS_READ_SIZE,
              AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE)}
        , io{pool, rwbuf}
        , aux{io}
    {
    }
...
TEST_F(NodeWriterTest, write_nodes_each_within_buffer)
{
    ...
    unsigned const num_nodes =
        AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE / node_disk_size;
    ...
    auto const node_offset = async_write_node_set_spare(aux, *node, true);
    EXPECT_EQ(node_offset.offset, AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE);
    EXPECT_EQ(
...
TEST_F(NodeWriterTest, write_node_across_buffers_ends_at_buffer_boundary)
{
    ...
    auto const chunk_remaining_bytes =
        2 * AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE + 1024;
    KINET_ASSERT(chunk_remaining_bytes < chunk_size);
    ...
    EXPECT_EQ(
        aux.node_writer_fast->sender().written_buffer_bytes(),
        chunk_remaining_bytes % AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE);
}
