...
TEST(NodeCache, works)
{
    ...
    auto make_node = [&](uint32_t v) {
        kinet::byte_string value(84, 0);
        memcpy(value.data(), &v, 4);
        std::shared_ptr<Node> node =
            kinet::mpt::make_node(0, {}, {}, std::move(value), 0, 0);
        KINET_ASSERT(node->get_mem_size() == NodeCache::AVERAGE_NODE_SIZE);
        return node;
    };
    auto get_acc_value = [&] -> uint32_t {
        auto const view(acc->second->val.first->value());
        KINET_ASSERT(84 == view.size());
    ...
    kinet::byte_string large_value(84 * 3, 0);
    memcpy(large_value.data(), "hihi", 4);
    auto const node =
        kinet::mpt::make_node(0, {}, {}, std::move(large_value), 0, 0);
