...
TEST(cli_tool, no_args_prints_fatal_and_help)
{
    ...
    std::string_view args[] = {"kinet-mpt"};
...
TEST(cli_tool, help_prints_help)
{
    std::stringstream cout;
    std::stringstream cerr;
    std::string_view args[] = {"kinet-mpt", "--help"};
    int const retcode = main_impl(cout, cerr, args);
    ASSERT_EQ(retcode, 0);
    {
...
TEST(cli_tool, create)
{
    ...
    auto const untempfile =
        kinet::make_scope_exit([&]() noexcept { unlink(temppath); });
...
namespace
{
    ...
    // the bytes actually landed, not just that kinet-mpt printed a success
    // line.
    kinet::mpt::state_machine_kind
    read_kind(char const *const path, kinet::mpt::timeline_id const tid)
    {
        kinet::mpt::AsyncIOContext io_ctx{kinet::mpt::ReadOnlyOnDiskDbConfig{
            .dbname_paths = {std::filesystem::path{path}}}};
        kinet::mpt::UpdateAux const aux(io_ctx.io);
    ...
    // instead, which mkstemps kinet_db_test_* into the temporary directory. The
    ...
    struct SweepStaleTempPools : public ::testing::Environment
    {
        void SetUp() override
        {
            kinet::test::remove_stale_temp_files(
    ...
    // Provision `temppath` (an empty temp file) as a KINET008 pool via
    // --create, then overwrite cnv chunk 0 (both metadata copies) with a
    // KINET007 (pre-dual-timeline) layout whose primary root_offsets ring
        ...
        uint64_t const history_length)
    {
        ...
        static constexpr size_t KINET007_HISTORY_LENGTH_OFFSET = 524344;
        ...
        KINET_ASYNC_NAMESPACE::storage_pool::creation_flags const flags;
        ...
        auto const chunk_count = static_cast<uint32_t>(
            pool.chunks(KINET_ASYNC_NAMESPACE::storage_pool::seq));
        auto &cnv_chunk =
            pool.chunk(KINET_ASYNC_NAMESPACE::storage_pool::cnv, 0);
        ...
        memcpy(buf.data() + KINET007_HISTORY_LENGTH_OFFSET, &history_length, 8);
        uint32_t const invalid = db_metadata::NULL_CHUNK;
        for (int i = 0; i < 6; i++) {
            memcpy(
                buf.data() + KINET007_LIST_TRIPLE_OFFSET + i * 4, &invalid, 4);
        }
    ...
    kinet::mpt::Node::SharedPtr upsert_one(
        kinet::mpt::Db &target, kinet::byte_string const &key,
        kinet::byte_string const &value, kinet::mpt::Node::SharedPtr root)
    {
        kinet::mpt::UpdateList ul;
        auto u = kinet::mpt::make_update(
            kinet::mpt::NibblesView{key}, kinet::byte_string_view{value});
        ul.push_front(u);
...
TEST(cli_tool, create_with_conflicting_state_machine_on_existing_pool_rejected)
{
    ...
    EXPECT_EQ(
        read_kind(temppath, kinet::mpt::timeline_id::primary),
        kinet::mpt::state_machine_kind::ethereum);
}
...
TEST(cli_tool, create_without_state_machine_on_existing_pool_accepted)
{
    ...
    EXPECT_EQ(
        read_kind(temppath, kinet::mpt::timeline_id::primary),
        kinet::mpt::state_machine_kind::kinet);
}
...
TEST(cli_tool, activate_secondary_stamps_secondary_kind)
{
    ...
    EXPECT_EQ(
        ...
        kinet::mpt::state_machine_kind::ethereum);
    EXPECT_EQ(
        read_kind(temppath, kinet::mpt::timeline_id::secondary),
        kinet::mpt::state_machine_kind::ethereum);
}
...
TEST(cli_tool, activate_secondary_defaults_to_ethereum_when_flag_omitted)
{
    ...
    }
    {
        ...
        std::stringstream cerr;
        std::string_view args[] = {
            "kinet-mpt", "--storage", temppath, "--activate-secondary"};
        ASSERT_EQ(0, main_impl(cout, cerr, args));
    }
    EXPECT_EQ(
        read_kind(temppath, kinet::mpt::timeline_id::secondary),
...
TEST(cli_tool, deactivate_secondary_clears_active_flag)
{
...

    {
        ...
        std::stringstream cerr;
        std::string_view args[] = {
            "kinet-mpt", "--storage", temppath, "--deactivate-secondary"};
        ASSERT_EQ(0, main_impl(cout, cerr, args));
        EXPECT_NE(
            std::string::npos,
...
TEST(cli_tool, promote_secondary_flips_primary_ring_idx)
{
...

    {
        ...
        std::stringstream cerr;
        std::string_view args[] = {
            "kinet-mpt", "--storage", temppath, "--promote-secondary"};
        ASSERT_EQ(0, main_impl(cout, cerr, args));
        EXPECT_NE(
            std::string::npos,
              ...
              .use_anonymous_inode = false}>
{
    void run_test()
    {
        ...
        ::close(fd);
        auto const untempfile = kinet::make_scope_exit([&]() noexcept {
            unlink(temppath1);
            unlink(dbpath2a);
            unlink(dbpath2b);
        });
        ...
        std::cout << "DB path: " << dbpath1 << std::endl;
        {
            ...
            std::stringstream cerr;
            std::string_view args[] = {
                "kinet-mpt", "--storage", dbpath1, "--archive", temppath1};
        ...
        }
        {
            ...
            std::async(std::launch::async, [&] {
                kinet::async::storage_pool pool(dbpath2);
                ...
                kinet::mpt::Node::SharedPtr const root_ptr{read_node_blocking(
                    aux,
                    aux.metadata_ctx().get_latest_root_offset(),
                    aux.metadata_ctx().db_history_max_version(),
                    kinet::mpt::timeline_id::primary)};
                kinet::mpt::NodeCursor const root(root_ptr);

                for (auto const &key : this->state()->keys) {
                    auto const ret = kinet::mpt::find_blocking(
                        aux,
                        root,
                        key.first,
                        aux.metadata_ctx().db_history_max_version(),
                        kinet::mpt::timeline_id::primary);
                    EXPECT_EQ(ret.second, kinet::mpt::find_result::success);
                }
                EXPECT_EQ(
                    this->state()
                        ->aux.metadata_ctx()
        ...
        if (Config.interleave_multiple_sources) {
            ...
            ::close(fd);
            auto const untempfile2 = kinet::make_scope_exit([&]() noexcept {
                unlink(temppath2);
                unlink(dbpath3);
            });
            {
            ...
            }
            {
                ...
                std::async(std::launch::async, [&] {
                    kinet::async::storage_pool pool({{dbpath3}});
                    kinet::io::Ring testring;
                    kinet::io::Buffers testrwbuf =
                        kinet::io::make_buffers_for_read_only(
                            testring,
                    ...
                    kinet::async::AsyncIO testio(pool, testrwbuf);
                    kinet::mpt::UpdateAux const aux{testio};
                    kinet::mpt::Node::SharedPtr const root_ptr{
                        read_node_blocking(
                            aux,
                            aux.metadata_ctx().get_latest_root_offset(),
                            aux.metadata_ctx().db_history_max_version(),
...
TEST_F(cli_tool_restore_preserves_kind, restore_preserves_state_machine_kind)
{
    constexpr auto synthetic_kind =
        static_cast<kinet::mpt::state_machine_kind>(uint8_t{3});

    auto const dbpath1 =
        this->state()->pool.devices().front().current_path().string();
    this->state()->aux.metadata_ctx().set_state_machine_kind(
        kinet::mpt::timeline_id::primary, synthetic_kind);
    ...
    ASSERT_EQ(
        ...
        truncate(
            ...
            (default_num_cnv_chunks +
             16) * KINET_ASYNC_NAMESPACE::AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE +
                24576));
    auto const cleanup = kinet::make_scope_exit([&]() noexcept {
        unlink(archivepath);
        unlink(dbpath2);
    });
    ...
    auto const restored_kind =
        std::async(std::launch::async, [&] {
            return read_kind(dbpath2, kinet::mpt::timeline_id::primary);
        }).get();
...
TEST(cli_tool, archives_restores_with_secondary_active)
{
    ...
    using kinet::literals::operator""_bytes;
    ...
    ASSERT_NE(arc_fd, -1);
    ::close(arc_fd);
    auto const unfiles = kinet::make_scope_exit([&]() noexcept {
...

    {
        ...
        EXPECT_EQ(kinet::byte_string{pres.value().node->value()}, v_primary);
        ...
        EXPECT_EQ(kinet::byte_string{sres.value().node->value()}, v_secondary);
    }
}
...
TEST(cli_tool, archives_restores_after_promote_and_deactivate)
{
...

    {
        ...
        EXPECT_EQ(kinet::byte_string{res.value().node->value()}, v_kept);
    }
}
...
// --upgrade on a pool created with --create and thus already on KINET008.
// Must be idempotent: exit 0, print "DB is on version KINET008".
TEST(cli_tool, upgrade_idempotent_on_current_pool)
{
    ...
    }
    {
        ...
        std::stringstream cerr;
        std::string_view args[] = {
            "kinet-mpt", "--storage", temppath, "--upgrade"};
        int const retcode = main_impl(cout, cerr, args);
        ASSERT_EQ(0, retcode);
        EXPECT_NE(
            std::string::npos, cout.str().find("DB is on version KINET008"));
    }
}
...
// Full end-to-end: create a fresh KINET008 pool, overwrite cnv chunk 0
// with a KINET007 layout via the storage_pool's own chunk API (so the
// file offset is correct regardless of pool internal layout), then run
// kinet-mpt --upgrade and verify the metadata is now KINET008 and the
// history_length survived at its new offset.
TEST(cli_tool, upgrade_migrates_kinet007_pool)
{
    using kinet::mpt::detail::db_metadata;
    static constexpr size_t KINET007_DB_METADATA_SIZE = 528512;
    static constexpr size_t KINET007_LIST_TRIPLE_OFFSET = 528488;
    ...
    // Provision the file as a KINET008 pool via --create.
    {
        std::stringstream cout;
        std::stringstream cerr;
        std::string_view args[] = {
    ...
    // Overwrite cnv chunk 0 (both halves) with a KINET007 layout. Open
    // the pool writable (allow_migration = false because the pool is
    // already KINET008 — the ctor won't try to migrate), use the chunk
    // API to get the correct file offset for chunk 0 and the pool's
    // real chunk_count, then pwrite KINET007 bytes over both halves.
    // Close the pool without writing metadata.
    //
    // The chunk_info[] array at KINET007's offset KINET007_DB_METADATA_SIZE
    ...
    // resulting KINET008 pool passes UpdateAux::init's chunk_info_count
    // check.
    ...
    // Reopen read-only; magic must now be KINET008 and history_length
    // must survive at its new offset.
    {
        ...
        KINET_ASYNC_NAMESPACE::storage_pool pool{
            ...
            flags};
        kinet::io::Ring ring;
        kinet::io::Buffers rbuf = kinet::io::make_buffers_for_read_only(
            ring,
            2,
            KINET_ASYNC_NAMESPACE::AsyncIO::KINET_IO_BUFFERS_READ_SIZE);
        KINET_ASYNC_NAMESPACE::AsyncIO io{pool, rbuf};
        kinet::mpt::DbMetadataContext const ctx{io};
...
// A pool migrated KINET007 -> KINET008 must have its (empty) secondary ring
...
TEST(cli_tool, upgrade_kinet007_secondary_ring_uses_null_chunk_sentinel)
{
    using kinet::mpt::detail::db_metadata;
    char temppath[] = "cli_tool_tmp_test_XXXXXX";
    make_temp_pool(temppath);
    ...
    provision_kinet007_pool(temppath, {1, 2}, /*history_length=*/9999);
    {
        std::stringstream cout;
        std::stringstream cerr;
        std::string_view args[] = {
    ...
    kinet::mpt::DbMetadataContext const ctx{io_ctx.io};
    auto const *const m = ctx.main();
    ASSERT_EQ(
        0, memcmp(m->magic, db_metadata::MAGIC, db_metadata::MAGIC_STRING_LEN));

    auto const &sstore =
        kinet::mpt::test::DbMetadataTestAccess::storage(m->secondary_timeline);
...
// End-to-end regression for the metadata-wipe incident: a KINET007 pool that
...
// than via the CLI: the full UpdateAux::init that `kinet-mpt
// --activate-secondary` runs requires a populated fast/slow chunk list that
// this synthetic KINET007 pool doesn't have, whereas the wipe bug lives
...
TEST(cli_tool, activate_secondary_on_migrated_pool_preserves_metadata)
{
...

    {
        ...
        KINET_ASYNC_NAMESPACE::AsyncIO io{pool, buffers};
        kinet::mpt::DbMetadataContext ctx{io};

        ASSERT_FALSE(ctx.timeline_active(kinet::mpt::timeline_id::secondary));
        ...
        EXPECT_TRUE(ctx.timeline_active(kinet::mpt::timeline_id::secondary));

        auto const &sstore = kinet::mpt::test::DbMetadataTestAccess::storage(
...
// kinet-mpt --repair path: a pool whose inactive secondary ring was left with
...
TEST(cli_tool, repair_normalizes_zeroed_secondary_ring)
{
    ...
    ASSERT_EQ(
        kinet::mpt::test::DbMetadataTestAccess::storage(
...
TEST(cli_tool, repair_cli_noop_on_healthy_pool)
{
    ...
    }
    {
        ...
        std::stringstream cerr;
        std::string_view args[] = {
            "kinet-mpt", "--storage", temppath, "--repair"};
        ASSERT_EQ(0, main_impl(cout, cerr, args)) << cerr.str();
...
TEST(cli_tool, repair_refuses_active_secondary)
{
    ...
    ctx.activate_secondary_header();
    ASSERT_TRUE(ctx.timeline_active(kinet::mpt::timeline_id::secondary));
    auto const real_chunk = kinet::mpt::test::DbMetadataTestAccess::storage(
                                ctx.main()->secondary_timeline)
                                .cnv_chunks[0]
                                .cnv_chunk_id;
...
// Same shape as upgrade_migrates_kinet007_pool, but the first cnv-chunk-0
...
// resulting KINET008 pool reflects the clean half's history_length, not
// the corrupt one.
TEST(cli_tool, upgrade_heals_dirty_kinet007_copy)
{
    using kinet::mpt::detail::db_metadata;
    static constexpr size_t KINET007_DB_METADATA_SIZE = 528512;
    static constexpr size_t KINET007_LIST_TRIPLE_OFFSET = 528488;
...

    {
        ...
        auto const build_kinet007_buf = [&](uint64_t const history_length,
                                            bool const dirty) {
            std::vector<uint8_t> buf(
                KINET007_DB_METADATA_SIZE +
                    size_t(chunk_count) * sizeof(db_metadata::chunk_info_t),
            ...
            for (int i = 0; i < 6; i++) {
                memcpy(
                    buf.data() + KINET007_LIST_TRIPLE_OFFSET + i * 4,
                    &invalid,
                    4);
            }
            return buf;
        ...
        auto const dirty_buf =
            build_kinet007_buf(corrupt_history_length, /*dirty=*/true);
        // Copy 1: clean + intended history_length.
        auto const clean_buf =
            build_kinet007_buf(clean_history_length, /*dirty=*/false);
...
TEST(cli_tool, upgrade_requires_storage)
{
    std::stringstream cout;
    std::stringstream cerr;
    std::string_view args[] = {"kinet-mpt", "--upgrade"};
    int const retcode = main_impl(cout, cerr, args);
    ASSERT_NE(0, retcode);
    EXPECT_TRUE(cerr.str().starts_with("FATAL:"));
}
