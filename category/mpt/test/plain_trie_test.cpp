...
namespace updates
{
    std::vector<std::pair<kinet::byte_string, kinet::byte_string>> const
        var_len_kv{
    ...
    std::vector<std::pair<kinet::byte_string, kinet::byte_string>> const top_kv{
        {0x01111111_bytes, 0xdead_bytes}, // 0
        {0x11111111_bytes, 0xbeef_bytes}, // 1
...
TYPED_TEST(PlainTrieTest, leaf_nodes_persist)
{
    ...
    auto u1 = make_update(key1, kinet::byte_string_view{});
    auto u2 = make_update(key2, kinet::byte_string_view{});
...
TYPED_TEST(PlainTrieTest, large_values)
{
    ...
    auto const value1 = kinet::byte_string(100 * 1024 * 1024, 0xf); // 100 MB
    auto const value2 = kinet::byte_string(255 * 1024 * 1024, 0x3); // 255 MB
...
TYPED_TEST(PlainTrieTest, multi_level_find_blocking)
{
    ...
    auto upsert_and_find_with_prefix = [&](kinet::byte_string const prefix,
                                           kinet::byte_string const top_value) {
        KINET_ASSERT(NibblesView{prefix}.nibble_size() == prefix_len);
        UpdateList updates;
...
TYPED_TEST(PlainTrieTest, node_version)
{
    ...
    std::vector<kinet::byte_string> const keys = {
        0x000000_bytes,
        0x000001_bytes,
        0x000002_bytes,
        0x000010_bytes,
