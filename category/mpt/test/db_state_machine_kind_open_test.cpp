...
namespace
{
    using kinet::mpt::test::DbAccessor;
    ...
    TEST(db_state_machine_kind_open, primary_open_uses_registered_factory)
    {
        ...
        auto const unfile = kinet::make_scope_exit(
        ...
        // ring_a, then close. Mirrors what kinet-mpt --create
        ...
        EXPECT_EQ(res.value().node->value(), kinet::byte_string_view{k});
    }
    ...
    TEST(db_state_machine_kind_open, secondary_open_no_arg_reads_ring_b_kind)
    {
        ...
        // sees after kinet-mpt --create followed by kinet-mpt
        // --activate-secondary.
        {
            Db db{std::make_unique<StateMachineAlwaysMerkle>(), create_config};
            DbAccessor::aux(db).metadata_ctx().set_state_machine_kind(
        ...
        EXPECT_EQ(
            pres.value().node->value(), kinet::byte_string_view{k_primary});

        auto const sres =
            secondary->find(NodeCursor{sroot}, NibblesView{k_secondary}, 0);
        ASSERT_TRUE(sres.has_value());
        EXPECT_EQ(
            sres.value().node->value(), kinet::byte_string_view{k_secondary});
    }
