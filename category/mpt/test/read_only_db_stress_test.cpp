    ...
    try {
        ...
        try {
            ...
            cli.add_flag(
                "--overwrite-keys-mode",
                overwrite_keys_mode,
                "Enable mode to overwrite identical key sets, allowing faster "
                "disk "
                "chunk reuse");
            ...
            auto upsert_new_version_overwrite_keys =
                [&](uint64_t const version) {
                    ...
                    rw_root = rw_db.upsert(
                        std::move(rw_root), std::move(ul), version);
                };
            ...
            auto random_async_read = [&]() {
                ...
                struct receiver_t
                {
                    ...
                    {
                        if (res) {
                            KINET_ASSERT(res.value() == version_bytes);
                            ++(*nsuccess);
                        }
                        else {
                ...
                while (!g_done) {
                    ...
                    auto const res = ro_db.find(
                        concat(
                            NibblesView{prefix},
                            NibblesView{
                                to_key(version * num_nodes_per_version)}),
                        version);
                    if (res.has_value()) {
                        ++nsuccess;
                        KINET_ASSERT(res.value().node->value() == value);
                    }
                    else {
                        ++nfailed;
                    }
                    ...
                    pool.submit(
                        ...
                        [ro_db = &ro_db,
                         ...
                         version = version] {
                            ...
                            if (res.has_value()) {
                                KINET_ASSERT(
                                    res.value().node->value() == value);
                            }
                            else {
                                KINET_ASSERT_PRINTF(
                                    ro_db->get_earliest_version() > version,
                                    "db earliest version %lu, find to_key(%lu) "
                                    "at "
                                    "version %lu",
                                    ro_db->get_earliest_version(),
                                    key_index,
