...
namespace kinet::test
{
            ...
            uint8_t length_max = 255)
        {
            ...
            if (!input_.empty()) {
                ...
                for (size_t n = 0; n < count.second; n++) {
                    ...
                    key_type key{};
                    if constexpr (std::
                                      is_same_v<key_type, kinet::byte_string>) {
                        key.resize(32);
                        for (auto &i : key) {
                            fill_(i);
                        }
    ...
    struct trie_fuzzer_fixture : public TFixture
    {
        ...
        void Process(
            ...
            std::map<size_t, std::optional<kinet::byte_string>> const &mods)
        {
            KINET_ASSERT(groups.size() == kv.size());
            ...
            for (size_t i = 0; i < kv.size(); ++i) {
                ...
                KINET_ASSERT(it != inputs.end());

                auto const next = std::next(it);
                KINET_ASSERT(next != inputs.end());
        ...
        void OneHundredUpdates(
            ...
            std::map<size_t, std::optional<kinet::byte_string>> const &mods)
        {
            KINET_ASSERT(this->root.get() == nullptr);
        ...
        void GeneratedKv(
            std::map<kinet::byte_string, kinet::byte_string> const &kv,
            std::vector<size_t> const &groups,
            std::map<size_t, std::optional<kinet::byte_string>> const &mods)
        {
            KINET_ASSERT(this->root.get() == nullptr);
            std::vector<std::pair<kinet::byte_string, kinet::byte_string>>
                transformed;
            transformed.reserve(kv.size());
            for (auto const &p : kv) {
                transformed.emplace_back(
                    [&]() {
                        KINET_ASSERT(p.first.size() == 32);
                        kinet::byte_string b;
                        b.resize(32);
            ...
            KINET_ASSERT(root_hash() == NULL_ROOT);
        }
    };
