...
namespace kinet::test
{
    ...
    {
        KINET_ASSERT(!s1.empty());
        KINET_ASSERT(!s2.empty());
        ...
        KINET_ASSERT(s1.size() == (1u + s1_size / 2u + odd));
        KINET_ASSERT(s2.size() == (1u + s1_size / 2u + odd));
    ...
    struct InMemoryPrefixPathComparator
    {
        ...
        operator()(byte_string_view element, byte_string_view value) const
        {
            KINET_ASSERT(element.size() > 20);
            KINET_ASSERT(value.size() > 20);
