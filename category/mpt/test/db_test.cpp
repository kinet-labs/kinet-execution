...
namespace
{
    ...
    struct OnDiskDbWithFileAsyncFixture : public OnDiskDbWithFileFixture
    {
        ...
        void async_get(auto &&sender, std::function<void(result_t<T>)> callback)
        {
            ...
            struct receiver_t
            {
                ...
                void set_value(
                    kinet::async::erased_connected_operation *const state,
...

    {
        ...
        kinet::byte_string const prefix{0x00_bytes};
    ...
    kinet::byte_string keccak_int_to_string(size_t const n)
    {
        kinet::byte_string ret(KECCAK256_SIZE, 0);
        kinet_keccak256((unsigned char const *)&n, 8, ret.data());
        return ret;
    }

    std::pair<std::deque<kinet::byte_string>, std::deque<Update>>
...
TEST_F(OnDiskDbWithFileFixture, rwdb_access_multi_version)
{
    ...
    kinet::fiber::PriorityPool pool{1, 8};
...
TEST(DbTest, history_length_adjustment_never_under_min)
{
    ...
    kinet::byte_string const large_value(16 * 1024, 0xf);
...
TEST(DbTest, history_length_adjustment_trims_both_timelines)
{
    ...
    kinet::byte_string const value(2 * 1024, 0xf);
...
TEST_F(OnDiskDbWithFileAsyncFixture, async_get_node_then_async_traverse)
{
    ...
    struct TraverseReceiver
    {
        ...
        void set_value(
            kinet::async::erased_connected_operation *const traverse_state,
            kinet::async::result<bool> const res)
        {
            ASSERT_TRUE(res);
    ...
    {
        using ResultType = kinet::async::result<std::shared_ptr<Node>>;
            ...
            ResultType const res)
        {
            if (!res) {
                ...
                auto *traverse_state = new auto(kinet::async::connect(
...
TEST(DbTest, out_of_order_upserts_to_nonexist_earlier_version)
{
    ...
    auto const undb = kinet::make_scope_exit(
...
TEST(DbTest, out_of_order_upserts_with_compaction)
{
    ...
    auto get_release_offsets = [](kinet::byte_string_view const bytes)
        -> std::pair<uint32_t, uint32_t> {
        KINET_ASSERT(bytes.size() == 8);
        return {
            // copy size is implicitly part of the `unaligned_load` call
            // NOLINTNEXTLINE(bugprone-suspicious-stringview-data-usage)
            kinet::unaligned_load<uint32_t>(bytes.data()),
            kinet::unaligned_load<uint32_t>(bytes.data() + sizeof(uint32_t))};
    };
    ...
    for (block_id = 0; block_id < 1000; ++block_id) {
        std::deque<kinet::byte_string> kv_alloc;
    ...
    struct SimpleTraverse : public TraverseMachine
    {
        ...
        virtual bool down(unsigned char const branch, Node const &node) override
        {
            ...
            if (branch == INVALID_BRANCH) {
                ...
                EXPECT_TRUE(node.has_value());
                EXPECT_EQ(node.value(), kinet::byte_string_view{});
...
TEST(RangedGetTest, path_exceeds_min_prefix)
{
    ...
    RangedGetMachine range_machine{
        min,
        max,
        [&num_results](NibblesView const, kinet::byte_string_view const) {
            ++num_results;
        }};
...
TEST_F(OnDiskDbFixture, rw_query_old_version)
{
    ...
    auto write = [&](kinet::byte_string_view k,
                     kinet::byte_string_view v,
...
TEST(DbTest, auto_expire_large_set)
{
    ...
    auto const prefix = 0x00_bytes;
    kinet::byte_string const value(256 * 1024, 0);
    std::vector<kinet::byte_string> keys;
    std::vector<kinet::byte_string> const values;
    ...
    for (uint64_t block_id = 0; block_id < blocks; ++block_id) {
        for (unsigned i = 0; i < keys_per_block; ++i) {
            ...
            kinet_keccak256((unsigned char const *)&raw, 8, key.data());
        }
...
TEST_F(OnDiskDbFixture, metadata)
{
    ...
    kinet::bytes32_t const voted_block_id{10};
    ...
    kinet::bytes32_t const proposed_block_id{12};
...
TEST_F(OnDiskDbFixture, copy_trie_from_to_same_version)
{
    ...
    auto verify_dest_state = [&](Db &db, kinet::byte_string const prefix) {
...
TEST_F(OnDiskDbFixture, copy_trie_to_dest_with_ancestor_prefix)
{
    ...
    kinet::byte_string const kv = keccak_int_to_string(0);
    root = upsert_updates_flat_list(
...
TEST(DbTest, move_trie_version_forward_history_ring_wrap_around)
{
    ...
    auto const prefix = 0x0012_bytes;
    kinet::byte_string const kv = keccak_int_to_string(10);
...
TYPED_TEST(DbTest, scalability)
{
    ...
    std::vector<kinet::byte_string> keys;
    {
        std::vector<kinet::mpt::Update> updates;
        keys.reserve(COUNT);
        updates.reserve(COUNT);
        kinet::small_prng rand;
        UpdateList ul;
        for (size_t n = 0; n < COUNT; n++) {
            kinet::byte_string key(16, 0);
    ...
    for (size_t n = 1; n <= MAX_CONCURRENCY; n <<= 1) {
        ...
        for (size_t i = 0; i < n; i++) {
            threads.emplace_back(
                [&](size_t myid) {
                    kinet::small_prng rand{uint32_t(myid)};
                    latch++;
                    while (latch != 0) {
...
TEST_F(OnDiskDbWithFileFixture, move_trie_version_forward_updates_auto_expire)
{
    ...
    EXPECT_EQ(
        test::DbAccessor::aux(db)
            .metadata_ctx()
            .get_auto_expire_version_metadata(kinet::mpt::timeline_id::primary),
        0);
