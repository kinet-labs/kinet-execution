...
using kinet::mpt::test::DbMetadataTestAccess;

TEST(db_metadata, DISABLED_copy)
{
#if KINET_CONTEXT_HAVE_TSAN
    return; // This test explicitly relies on racy memory copying
#endif
    kinet::mpt::detail::db_metadata *metadata[3];
    metadata[0] = (kinet::mpt::detail::db_metadata *)calloc(
        1, sizeof(kinet::mpt::detail::db_metadata));
    metadata[1] = (kinet::mpt::detail::db_metadata *)calloc(
        1, sizeof(kinet::mpt::detail::db_metadata));
    metadata[2] = (kinet::mpt::detail::db_metadata *)calloc(
        1, sizeof(kinet::mpt::detail::db_metadata));
    auto const unmetadata = kinet::make_scope_exit([&]() noexcept {
        free(metadata[0]);
        free(metadata[1]);
        free(metadata[2]);
    });
...
TEST(db_metadata, total_size)
{
    ...
    EXPECT_EQ(sizeof(kinet::mpt::detail::db_metadata), 4480u);
}

TEST(db_metadata, ring_b_layout_matches_ring_a)
{
    using md = kinet::mpt::detail::db_metadata;
...
TEST(db_metadata, role_bytes_zero_initialized)
{
    ...
    auto *m = (kinet::mpt::detail::db_metadata *)calloc(
        1, sizeof(kinet::mpt::detail::db_metadata));
    auto const cleanup = kinet::make_scope_exit([&]() noexcept { free(m); });
...
TEST(db_metadata, secondary_timeline_header_survives_metadata_copy)
{
    ...
    auto const cleanup_src =
        kinet::make_scope_exit([&]() noexcept { free(src); });
    ...
    auto const cleanup_dst =
        kinet::make_scope_exit([&]() noexcept { free(dst); });
...
TEST(db_metadata, db_copy_preserves_secondary_next_version)
{
    ...
    kinet::mpt::detail::db_copy(dst, src, sizeof(md));
