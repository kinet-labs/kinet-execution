...
TEST(serialize_to_big_endian, test)
{
    ...
    EXPECT_EQ(serialize_as_big_endian<8>(n), 0x1122334455667788_bytes);
    EXPECT_EQ(serialize_as_big_endian<6>(n), 0x334455667788_bytes);
    EXPECT_EQ(serialize_as_big_endian<2>(n), 0x7788_bytes);

    uint32_t const n2 = 0x11223344;
    EXPECT_EQ(serialize_as_big_endian<4>(n2), 0x11223344_bytes);
    EXPECT_EQ(serialize_as_big_endian<2>(n2), 0x3344_bytes);
}

TEST(deserialize_from_big_endian_nibbles, test)
{
    Nibbles const a{0x00112233_bytes};
    EXPECT_EQ(deserialize_from_big_endian<uint32_t>(a), 0x112233);

    Nibbles const b{0x112233_bytes};
    EXPECT_EQ(deserialize_from_big_endian<uint32_t>(b), 0x112233);

    Nibbles const c{0xaabbccdd00112233_bytes};
