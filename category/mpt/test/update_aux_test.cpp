...
namespace
{
    ...
    void with_rw_aux(F &&fn)
    {
        ...
        std::jthread const rw_asyncio([&](std::stop_token) {
            ...
            kinet::async::AsyncIO testio(pool, testbuf);
...
{
    kinet::async::storage_pool pool(kinet::async::use_anonymous_inode_tag{});

    // All this threading nonsense is because we can't have two AsyncIO
    // instances on the same thread.

    std::unique_ptr<kinet::mpt::UpdateAux> aux_writer;
    std::atomic<bool> io_set = false;
    std::jthread const rw_asyncio([&](std::stop_token token) {
        ...
        kinet::io::Buffers testbuf =
            kinet::io::make_buffers_for_segregated_read_write(
                ...
                kinet::async::AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE);
        kinet::async::AsyncIO testio(pool, testbuf);
        aux_writer = std::make_unique<kinet::mpt::UpdateAux>(
    ...
    aux_writer->metadata_ctx().modify_metadata(
        [](kinet::mpt::detail::db_metadata *m) {
            m->is_dirty().store(1, std::memory_order_release);
        });

    ASSERT_TRUE(const_cast<kinet::mpt::detail::db_metadata *>(
                    aux_writer->metadata_ctx().main())
                    ->is_dirty());

    kinet::io::Ring ring;
    kinet::io::Buffers testrobuf = kinet::io::make_buffers_for_read_only(
        ring, 2, kinet::async::AsyncIO::KINET_IO_BUFFERS_READ_SIZE);
    auto pool_ro = pool.clone_as_read_only();
    kinet::async::AsyncIO testio(pool_ro, testrobuf);

    // RO open should abort when dirty bits are set and never clear.
    ASSERT_DEATH(
        ({ kinet::mpt::DbMetadataContext{testio}; }),
    ...
    EXPECT_NO_THROW(({ kinet::mpt::DbMetadataContext{testio}; }));
}
...
TEST(update_aux_test, root_offsets_fast_slow)
{
    ...
    kinet::async::AsyncIO testio(pool, testbuf);
    {
        kinet::mpt::UpdateAux aux_writer{testio, AUX_TEST_HISTORY_LENGTH};

        // Root offset at 0, fast list offset at 50. This is correct
        auto const start_offset =
            aux_writer.node_writer_fast->sender().offset();
        (void)pool
            .chunk(kinet::async::storage_pool::chunk_type::seq, start_offset.id)
            .write_fd(50);
    ...
    { // Fail to reopen upon calling rewind_to_match_offsets()
        EXPECT_EXIT(
            ({ kinet::mpt::UpdateAux{testio, AUX_TEST_HISTORY_LENGTH}; }),
...
TEST(update_aux_test, configurable_root_offset_chunks)
{
    std::filesystem::path const filename{
        ...
        "kinet_update_aux_test_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30)); // 8GB
    ...
    // is derived from it.
    kinet::async::storage_pool::creation_flags flags;
    flags.num_cnv_chunks = 5;
    {
        ...
        kinet::async::storage_pool pool(
            std::span{&filename, 1},
            kinet::async::storage_pool::mode::truncate,
            flags);
        EXPECT_EQ(pool.chunks(kinet::async::storage_pool::cnv), 5);

        kinet::async::AsyncIO testio(pool, testbuf);
        kinet::mpt::UpdateAux const aux(testio);
    ...
    }
    {
        ...
        kinet::async::storage_pool pool(
            std::span{&filename, 1},
            kinet::async::storage_pool::mode::open_existing,
            flags);
        EXPECT_EQ(pool.chunks(kinet::async::storage_pool::cnv), 5);
        kinet::async::AsyncIO testio(pool, testbuf);
        kinet::mpt::UpdateAux const aux(testio);
...
{
    auto path_template = (KINET_ASYNC_NAMESPACE::working_temporary_directory() /
                          "kinet_update_aux_test_XXXXXX")
                             .native();
    ...
    flags.num_cnv_chunks = 0; // legacy footer
    {
        kinet::async::storage_pool pool(
            ...
            kinet::async::storage_pool::mode::truncate,
            flags);
        EXPECT_EQ(pool.chunks(kinet::async::storage_pool::cnv), 3);
...
TEST(update_aux_test, state_machine_kind_default_ethereum)
{
    ...
    // stamped (e.g. via kinet-mpt --state-machine).
    with_rw_aux([](UpdateAux &aux) {
        EXPECT_EQ(
...
TEST(update_aux_test, state_machine_kind_persists_across_reopen)
{
    ...
    auto const unfilename = kinet::make_scope_exit(
        [&]() noexcept { std::filesystem::remove(filename); });

    kinet::io::Ring ring1;
    kinet::io::Ring ring2;
    auto testbuf = kinet::io::make_buffers_for_segregated_read_write(
        ring1,
        ring2,
        2,
        4,
        ...
        kinet::async::AsyncIO::KINET_IO_BUFFERS_WRITE_SIZE);
    {
        kinet::async::storage_pool pool(
            ...
            flags);
        kinet::async::AsyncIO testio(pool, testbuf);
        kinet::mpt::UpdateAux aux(testio);
...
// Simulates opening a DB created by pre-dual-timeline code (KINET007), whose
...
// Byte-level test for the KINET007 -> KINET008 layout-shift migration.
// KINET008 shrank root_offsets_ring_t::SIZE_ from 65536 to 32, moving
// sizeof(db_metadata) from 528512 to 4480. Any real KINET007 pool has
// chunk_info[], the list triple, and the db_offsets + consensus fields
// at byte offsets that the KINET008 layout no longer uses. The ctor's
// migration must relocate those blocks before downstream code reads
// through the new offsets.
//
// This test writes a real KINET007 layout directly to cnv chunk 0 (both
...
// entries) landed at their KINET008 offsets with the right values and
// that new-in-KINET008 fields initialised to their idle state.
TEST(update_aux_test, migrates_kinet007_layout_to_kinet008)
{
    using kinet::mpt::detail::db_metadata;
    static constexpr size_t KINET007_DB_METADATA_SIZE = 528512;
    static constexpr size_t KINET007_DB_OFFSETS_OFFSET = 524328;
    static constexpr size_t KINET007_LIST_TRIPLE_OFFSET = 528488;

    kinet::async::storage_pool::creation_flags flags;
    flags.chunk_capacity = 24;
    flags.allow_migration = true;
    kinet::async::storage_pool pool(
        kinet::async::use_anonymous_inode_tag{}, flags);
    ...
    uint32_t const test_chunk_count = static_cast<uint32_t>(
        pool.chunks(kinet::async::storage_pool::seq) +
        pool.chunks(kinet::async::storage_pool::cnv));

    auto &cnv_chunk = pool.chunk(kinet::async::storage_pool::cnv, 0);
    auto const [write_fd, base_offset] = cnv_chunk.write_fd(0);
    auto const half_capacity = cnv_chunk.capacity() / 2;

    auto const build_kinet007_buffer = [&] {
        std::vector<uint8_t> buf(
            KINET007_DB_METADATA_SIZE +
        ...
        // KINET007 cnv_chunks[] array filled with 0xff as KINET007 did.
        ...
        memset(buf.data() + 64, 0xff, KINET007_DB_OFFSETS_OFFSET - 64);
        ...
        // auto_expire_version at 524384 — KINET008 promotes this global
        ...
        // 524456..528488: 0xff (KINET007 convention).
        memset(buf.data() + 524456, 0xff, KINET007_LIST_TRIPLE_OFFSET - 524456);
        // free_list (begin, end) at KINET007_LIST_TRIPLE_OFFSET.
        memcpy(
            buf.data() + KINET007_LIST_TRIPLE_OFFSET, &test_free_list_begin, 4);
        memcpy(
            buf.data() + KINET007_LIST_TRIPLE_OFFSET + 4,
            &test_free_list_end,
            4);
        // fast_list and slow_list (begin=NULL_CHUNK, end=NULL_CHUNK —
        // empty).
        uint32_t const invalid = db_metadata::NULL_CHUNK;
        memcpy(buf.data() + KINET007_LIST_TRIPLE_OFFSET + 8, &invalid, 4);
        memcpy(buf.data() + KINET007_LIST_TRIPLE_OFFSET + 12, &invalid, 4);
        memcpy(buf.data() + KINET007_LIST_TRIPLE_OFFSET + 16, &invalid, 4);
        memcpy(buf.data() + KINET007_LIST_TRIPLE_OFFSET + 20, &invalid, 4);
    ...
    auto const buffer = build_kinet007_buffer();
        ...
        for (unsigned i = 0; i < 2; i++) {
            ...
            EXPECT_EQ(
                ...
                memcmp(
                    ...
                    db_metadata::MAGIC,
                    db_metadata::MAGIC_STRING_LEN))
                << "copy " << i << " magic should be migrated to KINET008";
            ...
            // KINET007's global auto_expire_version was migrated into the
            ...
            // state_machine_kind_ overlapped KINET007's cnv_chunks[] (0xff
            ...
            EXPECT_EQ(
                ...
                static_cast<uint8_t>(state_machine_kind::ethereum));
            // reserved_sm_ must be zeroed (was 0xff in KINET007 cnv_chunks[]).
            ...
            // New-in-KINET008 fields must start at idle values
            // regardless of what was in the overlapping KINET007 bytes
...
// Simulates a crash mid-migration: copy 0 finished migrating to KINET008 in
// a prior run; copy 1 still tagged KINET007 with 0xff padding. The magic-
...
TEST(update_aux_test, migrates_kinet007_resumes_after_partial_migration)
{
    kinet::async::storage_pool::creation_flags flags;
    flags.chunk_capacity = 24;
    flags.allow_migration = true;
...

    {
        ...
        // Corrupt copy 1 only. Copy 0 stays KINET008, so on reopen the
        ...
        memcpy(
            ...
            db_metadata::PREVIOUS_MAGIC,
            db_metadata::MAGIC_STRING_LEN);
        // Raw 0xff fill to simulate a KINET007-padded ring; the private
        ...
        for (unsigned i = 0; i < 2; i++) {
            ...
            EXPECT_EQ(
                ...
                memcmp(
                    ...
                    db_metadata::MAGIC,
                    db_metadata::MAGIC_STRING_LEN))
                << "copy " << i << " magic should be KINET008 after migration";
            EXPECT_EQ(m->secondary_timeline_active_, 0u);
            EXPECT_EQ(m->primary_ring_idx, 0u);
            EXPECT_EQ(
                test::DbMetadataTestAccess::next_version(m->secondary_timeline),
...
// A binary that opens a KINET007 pool without setting allow_migration must
// abort with a message directing operators to kinet-mpt --upgrade. This is
...
// layout.
TEST(update_aux_death_test, aborts_on_kinet007_without_allow_migration)
{
    using kinet::mpt::detail::db_metadata;
    static constexpr size_t KINET007_DB_METADATA_SIZE = 528512;
    ...
    // Write minimal KINET007 magic to cnv chunk 0 (both copies).
    auto &cnv_chunk = pool.chunk(kinet::async::storage_pool::cnv, 0);
    auto const [write_fd, base_offset] = cnv_chunk.write_fd(0);
    auto const half_capacity = cnv_chunk.capacity() / 2;
    ...
    uint32_t const chunk_count =
        static_cast<uint32_t>(pool.chunks(kinet::async::storage_pool::seq));
    std::vector<uint8_t> buf(
        KINET007_DB_METADATA_SIZE +
    ...
    ASSERT_DEATH(
        { DbMetadataContext const ctx{testio}; }, "kinet-mpt --upgrade");
}

// Regression for the KINET007 -> KINET008 migration on a pool large enough
// that step 2's chunk_info[] memmove destination overruns the KINET007
...
TEST(
    update_aux_test, migrate_kinet007_preserves_db_offsets_at_large_chunk_count)
{
    using kinet::mpt::detail::db_metadata;
    static constexpr size_t KINET007_DB_METADATA_SIZE = 528512;
    static constexpr size_t KINET007_DB_OFFSETS_OFFSET = 524328;
    ...
    static_assert(
        4480 + 8 * size_t(chunk_count) > KINET007_DB_OFFSETS_OFFSET,
    ...
    memset(
        ...
        0xff,
        KINET007_DB_OFFSETS_OFFSET - (48 + size_t(test_cnv_chunks_len) * 8));

    // The 128-byte db_offsets..block_ids block. Byte indices are relative to
    // KINET007_DB_OFFSETS_OFFSET; a per-byte pattern makes any clobber
    ...
    memcpy(buf + KINET007_DB_OFFSETS_OFFSET, db_block, sizeof(db_block));

    memset(buf + 524456, 0xff, KINET007_LIST_TRIPLE_OFFSET - 524456);
    uint32_t const fl_begin = 5;
    uint32_t const fl_end = 7;
    uint32_t const invalid = db_metadata::NULL_CHUNK;
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET, &fl_begin, 4);
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET + 4, &fl_end, 4);
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET + 8, &invalid, 4);
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET + 12, &invalid, 4);
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET + 16, &invalid, 4);
    memcpy(buf + KINET007_LIST_TRIPLE_OFFSET + 20, &invalid, 4);
    ...
    for (size_t s = 0; s < 3; s++) {
        memcpy(
            buf + KINET007_DB_METADATA_SIZE +
                size_t(ci_slots[s]) * sizeof(db_metadata::chunk_info_t),
            &ci_patterns[s],
            sizeof(uint64_t));
    }

    auto *const m = kinet::start_lifetime_as<db_metadata>(buf);
    kinet::mpt::detail::migrate_kinet007_to_kinet008(m, chunk_count);
    ...
    EXPECT_EQ(m->fast_list.begin, db_metadata::NULL_CHUNK);
    EXPECT_EQ(m->slow_list.begin, db_metadata::NULL_CHUNK);
    // new-in-KINET008 fields at idle defaults.
    EXPECT_EQ(m->secondary_timeline.cnv_chunks_len(), 0u);
    EXPECT_EQ(m->primary_ring_idx, 0u);
    EXPECT_EQ(m->secondary_timeline_active_, 0u);
    EXPECT_EQ(
...
{
    std::string templ = (KINET_ASYNC_NAMESPACE::working_temporary_directory() /
                         "kinet_update_aux_ring_cap_test_XXXXXX")
                            .string();
    ...
    KINET_ASSERT(-1 != ::ftruncate(fd, 2UL << 30)); // 2GB
...
TEST(update_aux_test, reactivate_after_deactivate)
{
    with_rw_aux([](UpdateAux &aux) {
        ...
        EXPECT_EQ(sro2[0], kinet::async::INVALID_OFFSET)
...
TEST(update_aux_test, secondary_ring_initialized_to_invalid_offset_on_activate)
{
    with_rw_aux([](UpdateAux &aux) {
        ...
        EXPECT_EQ(ro[1], kinet::async::INVALID_OFFSET);
        EXPECT_EQ(ro[100], kinet::async::INVALID_OFFSET);
        aux.deactivate_secondary_timeline();
    });
}
...
TEST(update_aux_test, promote_persists_across_reopen)
{
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_update_aux_promote_persist_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30)); // 8GB
...
TEST(update_aux_test, replay_completes_pending_promote_after_crash)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_pending_promote_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_completes_pending_activate_after_crash)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_pending_activate_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_is_noop_when_activate_already_committed)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_already_committed_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_completes_pending_deactivate_after_crash)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_pending_deactivate_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_completes_partial_cnv_chunks_move)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_partial_cnv_chunks_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_preserves_primary_low_positions)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_preserves_primary_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
TEST(update_aux_test, replay_handles_single_copy_pending_stamp)
{
    ...
    std::filesystem::path const filename{
        KINET_ASYNC_NAMESPACE::working_temporary_directory() /
        "kinet_replay_single_copy_pending_XXXXXX"};
    int const fd = ::mkstemp((char *)filename.native().data());
    KINET_ASSERT(fd != -1);
    KINET_ASSERT(-1 != ::ftruncate(fd, 8UL << 30));
...
namespace
{
    ...
    chunk_offset_t
    fake_root_write(kinet::async::storage_pool &pool, UpdateAux &aux)
    {
        chunk_offset_t const off_before =
            aux.node_writer_fast->sender().offset();
        (void)pool.chunk(kinet::async::storage_pool::seq, off_before.id)
    ...
    void primary_write(
        kinet::async::storage_pool &pool, UpdateAux &aux, uint64_t version)
    {
        ...
        if (max_ver == INVALID_BLOCK_NUM) {
            ...
            KINET_ASSERT(version == max_ver + 1);
            mctx.append_root_offset(off, timeline_id::primary);
        }
    }
