...
inline kinet::byte_string const &to_byte_string(std::string_view const s)
{
    static std::map<std::string, kinet::byte_string> storage;
    std::string key(s);
    auto it = storage.find(key);
    if (it == storage.end()) {
        auto const res = kinet::from_hex(s);
