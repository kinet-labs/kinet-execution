...
public:
    ...
    // kinds at process start (e.g. kinet::register_ethereum_state_machines()).
    ...
    // (kinet-mpt --activate-secondary --state-machine <kind>).
    ...
    struct DbGetSender
    {
            ...
            , block_id(block_id_)
        {
            if constexpr (std::same_as<T, std::shared_ptr<Node>>) {
                KINET_ASSERT(op_type == op_t::op_get_node1);
            }
        }
