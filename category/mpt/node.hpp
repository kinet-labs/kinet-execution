...
struct node_disk_pages_spare_15
{
        ...
        : value{0}
    {
        ...
        KINET_ASSERT(count <= max_count);
        KINET_ASSERT(shift <= max_shift);
        value.spare.count = count & max_count;
        value.spare.shift = shift & max_shift;
        KINET_ASSERT(to_pages() >= pages);
    }
...
#pragma GCC diagnostic pop
    ...
    */
    ...
    {
        KINET_ASSERT(bytes <= Node::max_size);
    ...
    unsigned char const *read_pos, size_t const max_bytes)
{
    ...
    KINET_ASSERT(alloc_size == node->get_mem_size());
    return node;
}
