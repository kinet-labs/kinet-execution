...
std::string print_bytes(KINET_ASYNC_NAMESPACE::file_offset_t const bytes_)
{
...
static char const *
state_machine_kind_name(KINET_MPT_NAMESPACE::state_machine_kind const kind)
{
    switch (kind) {
    case KINET_MPT_NAMESPACE::state_machine_kind::ethereum:
        return "ethereum";
    case KINET_MPT_NAMESPACE::state_machine_kind::kinet:
        return "kinet";
    }
    return "unknown";
}
...
{
    kinet::async::storage_pool::chunk_type const type;
    uint32_t const chunk_id;
    kinet::mpt::detail::db_metadata::chunk_info_t const metadata;
    std::span<std::byte const> const compressed;

    kinet::async::storage_pool::chunk_t *chunk_ptr;
    ...
    chunk_info_restore_t(
        kinet::async::storage_pool::chunk_type const type_,
        uint32_t const chunk_id_,
        kinet::mpt::detail::db_metadata::chunk_info_t const metadata_,
    ...
    size_t run()
    {
        if (!is_uncompressed) {
            ...
            auto undecompressed = kinet::make_scope_exit([&]() noexcept {
            ...
            if (nonchunkstorage.empty()) {
                ...
                if (use_faster_memory_mode) {
                    ...
                    int const fd = kinet::async::make_temporary_inode();
                    if (fd == -1) {
                        throw std::system_error(errno, std::system_category());
                    }
...
{
    kinet::async::storage_pool::chunk_t const *chunk_ptr;
    ...
    chunk_info_archive_t(
        kinet::async::storage_pool::chunk_t *chunk_ptr_,
...
struct impl_t
{
    ...
    KINET_MPT_NAMESPACE::state_machine_kind state_machine =
        KINET_MPT_NAMESPACE::state_machine_kind::ethereum;
    ...
    std::optional<KINET_ASYNC_NAMESPACE::storage_pool> pool;

    std::vector<chunk_info_archive_t> fast, slow;
    KINET_ASYNC_NAMESPACE::file_offset_t total_used = 0;
...
public:
    ...
    KINET_ASYNC_NAMESPACE::file_offset_t print_list_info(
        KINET_MPT_NAMESPACE::UpdateAux &aux,
        KINET_MPT_NAMESPACE::detail::db_metadata::chunk_info_t const
            *const item_,
        char const *name, T *list = nullptr)
    {
        ...
        KINET_ASYNC_NAMESPACE::file_offset_t total_capacity = 0;
        KINET_ASYNC_NAMESPACE::file_offset_t total_used = 0;
        uint32_t count = 0;
        ...
        do {
            ...
            KINET_ASSERT(chunk.zone_id().second == chunkid);
    ...
    void print_timeline_info(
        KINET_MPT_NAMESPACE::UpdateAux &aux, kinet::mpt::timeline_id const tid,
        char const *const name)
    {
        KINET_ASSERT(aux.metadata_ctx().timeline_active(tid));
        cout << "     " << name << ":\n        State machine kind: "
        ...
        if (max_v == kinet::mpt::INVALID_BLOCK_NUM) {
    ...
    void print_db_history_summary(KINET_MPT_NAMESPACE::UpdateAux &aux)
    {
        cout << "MPT database has been configured to retain no more than "
             ...
             << kinet::to_hex(kinet::byte_string_view(
                    aux.metadata_ctx().get_latest_proposed_block_id().bytes,
                    sizeof(kinet::bytes32_t)))
             << ").\n     Latest voted is ("
        ...
        print_timeline_info(aux, kinet::mpt::timeline_id::primary, "Primary");
        if (aux.metadata_ctx().timeline_active(
                kinet::mpt::timeline_id::secondary)) {
            print_timeline_info(
                aux, kinet::mpt::timeline_id::secondary, "Secondary");
        }
    }
    ...
    void do_restore_database()
    {
        ...
        auto unfd = kinet::make_scope_exit([&]() noexcept { ::close(fd); });
        ...
        auto const unin =
            kinet::make_scope_exit([&]() noexcept { archive_read_free(in); });
        ...
        for (archive_entry *entry = nullptr;
            ...
            kinet::async::storage_pool::chunk_type type;
            std::string_view const pathname_sv(pathname.native());
            auto const type_sv = pathname_sv.substr(0, 4);
            if (type_sv == "cnv/") {
                type = kinet::async::storage_pool::cnv;
            }
            else if (type_sv == "seq/") {
                type = kinet::async::storage_pool::seq;
            }
            else {
                continue;
            }
            ...
            kinet::mpt::detail::db_metadata::chunk_info_t metadata;
            ...
            for (;;) {
                ...
                if (0 == strcmp(xattr_name, "kinet.triedb.metadata")) {
                    memcpy(&metadata, xattr_value, sizeof(metadata));
                }
            }
            if (type == kinet::async::storage_pool::seq) {
                if (!metadata.in_fast_list && !metadata.in_slow_list) {
                    ...
                    ss << "Sequential type chunk in archive has neither "
                          ...
                          "this archive was generated by kinet-mpt?";
                    throw std::runtime_error(ss.str());
                }
            }
        ...
        for (auto &i : todecompress) {
            ...
            if (i.type == kinet::async::storage_pool::cnv && i.chunk_id == 0) {
        ...
        chunks.reserve(1024);
        {
            kinet::io::Ring ring(kinet::io::RingConfig{1});
            kinet::io::Buffers rwbuf =
                kinet::io::make_buffers_for_mixed_read_write(
                    ring,
                    2,
                    2,
            ...
            KINET_MPT_NAMESPACE::UpdateAux aux(io);
            for (;;) {
                ...
                auto const chunkid = item->index(aux.metadata_ctx().main());
                KINET_ASSERT(
                    chunkid != kinet::mpt::detail::db_metadata::NULL_CHUNK);
        ...
        cout << std::endl;
        kinet::async::file_offset_t total_bytes_decompressed = 0;
        for (;;) {
            size_t done = 0;
        ...
        kinet::mpt::detail::unsigned_20 fast_list_base_insertion_count(
            UINT32_MAX);
        kinet::mpt::detail::unsigned_20 slow_list_base_insertion_count(
            UINT32_MAX);
        uint32_t fast_list_begin_index{
            kinet::mpt::detail::db_metadata::NULL_CHUNK};
        ...
        uint32_t slow_list_end_index{
            kinet::mpt::detail::db_metadata::NULL_CHUNK};
        for (auto &i : todecompress) {
            if (i.type == kinet::async::storage_pool::cnv) {
                if (i.chunk_id == 0) {
                    auto const *old_metadata =
                        (kinet::mpt::detail::db_metadata const *)
                            i.nonchunkstorage.data();
                    if (0 != memcmp(
                                 old_metadata->magic,
                                 kinet::mpt::detail::db_metadata::MAGIC,
                                 kinet::mpt::detail::db_metadata::
                                     MAGIC_STRING_LEN)) {
                        std::stringstream ss;
                    ...
                    auto &cnv_chunk =
                        pool->chunk(kinet::async::storage_pool::cnv, 0);
                    ...
                    auto const un_new_metadata_map =
                        kinet::make_scope_exit([&]() noexcept {
                            ::munmap(new_metadata_map, cnv_chunk.capacity());
                        });
                    kinet::mpt::detail::db_metadata *db_metadata[2] = {
                        (kinet::mpt::detail::db_metadata *)new_metadata_map,
                        (kinet::mpt::detail::db_metadata
                    ...
                    do_([&](kinet::mpt::detail::db_metadata *metadata) {
                        KINET_ASSERT(
                            0 == memcmp(
                                     metadata->magic,
                                     kinet::mpt::detail::db_metadata::MAGIC,
        ...
        if (debug_printing) {
            ...
            for (; it != todecompress.end() &&
                   it->type == kinet::async::storage_pool::seq;
                 ++it) {
                std::cerr << " " << it->chunk_id;
            }
            std::cerr << std::endl;
        ...
        auto override_insertion_count =
            [](kinet::mpt::detail::db_metadata *db,
               kinet::mpt::UpdateAux::chunk_list type,
               kinet::mpt::detail::unsigned_20 initial_insertion_count) {
                KINET_ASSERT(type != kinet::mpt::UpdateAux::chunk_list::free);
                auto const g = db->hold_dirty();
                auto *i =
                    const_cast<kinet::mpt::detail::db_metadata::chunk_info_t *>(
                        type == kinet::mpt::UpdateAux::chunk_list::fast
                            ? db->fast_list_begin()
                            : db->slow_list_begin());
                i->insertion_count0_ =
        ...
        for (auto const &i : todecompress) {
            if (i.type == kinet::async::storage_pool::seq) {
                if (i.metadata.in_fast_list) {
                    aux.metadata_ctx().append(
                        kinet::mpt::UpdateAux::chunk_list::fast, i.chunk_id);
                    if (0 == fast_chunks_inserted++) {
                        aux.metadata_ctx().modify_metadata(
                            override_insertion_count,
                            kinet::mpt::UpdateAux::chunk_list::fast,
                            fast_list_base_insertion_count);
                    }
                }
                else if (i.metadata.in_slow_list) {
                    aux.metadata_ctx().append(
                        kinet::mpt::UpdateAux::chunk_list::slow, i.chunk_id);
                    if (0 == slow_chunks_inserted++) {
                        aux.metadata_ctx().modify_metadata(
                            override_insertion_count,
                            kinet::mpt::UpdateAux::chunk_list::slow,
                            slow_list_base_insertion_count);
                    }
                }
                if (i.metadata.in_fast_list || i.metadata.in_slow_list) {
                    ...
                    KINET_ASSERT(it != chunks.end());
                    *it = kinet::mpt::detail::db_metadata::NULL_CHUNK;
                }
            }
        }
        KINET_ASSERT(
        ...
        if (fast_chunks_inserted == 0) {
            aux.metadata_ctx().append(
                kinet::mpt::UpdateAux::chunk_list::fast, fast_list_begin_index);
            auto const it =
        ...
        if (slow_chunks_inserted == 0) {
            aux.metadata_ctx().append(
                kinet::mpt::UpdateAux::chunk_list::slow, slow_list_begin_index);
            auto const it =
        ...
        for (unsigned int const &chunk : chunks) {
            if (chunk != kinet::mpt::detail::db_metadata::NULL_CHUNK) {
                aux.metadata_ctx().append(
                    kinet::mpt::UpdateAux::chunk_list::free, chunk);
            }
        }
    ...
    void do_archive_database()
    {
        ...
        auto const unfd1 = kinet::make_scope_fail(
            [&]() noexcept { ::unlink(archive_database.c_str()); });
        auto const unfd2 =
            kinet::make_scope_exit([&]() noexcept { ::close(fd); });

        {
            ...
            if (total_used / kinet::async::file_offset_t(statfs.f_bsize) >
                kinet::async::file_offset_t(statfs.f_bavail)) {
                std::stringstream ss;
                ss << "WARNING: Destination filesystem for archive "
                   << archive_database << " has "
                   << print_bytes(
                          kinet::async::file_offset_t(statfs.f_bavail) *
                          kinet::async::file_offset_t(statfs.f_bsize))
            ...
            auto const total_used2 =
                ...
                * pool->chunk(kinet::async::storage_pool::seq, 0).capacity();
            int const tempfd = kinet::async::make_temporary_inode();
            if (tempfd == -1) {
                throw std::system_error(errno, std::system_category());
            }
            ...
            ::close(tempfd);
            if (total_used2 / kinet::async::file_offset_t(statfs.f_bsize) >
                kinet::async::file_offset_t(statfs.f_bavail)) {
                std::stringstream ss;
                ss << "WARNING: Temporary files filesystem "
                   << kinet::async::working_temporary_directory() << " has "
                   << print_bytes(
                          kinet::async::file_offset_t(statfs.f_bavail) *
                          kinet::async::file_offset_t(statfs.f_bsize))
        ...
        }
        {
            ...
            auto const unout = kinet::make_scope_exit([&]() noexcept {
            ...
            auto map_chunk_into_memory = [this](chunk_info_archive_t &i) {
                ...
                if (i.chunk_ptr->zone_id() == std::pair{pool->cnv, 0u}) {
                    ...
                    auto const db_metadata_size =
                        sizeof(kinet::mpt::detail::db_metadata) +
                        pool->chunks(kinet::async::storage_pool::seq) *
                            sizeof(
                                kinet::mpt::detail::db_metadata::chunk_info_t);
                    i.uncompressed =
            ...
            {
                auto const *m =
                    kinet::start_lifetime_as<kinet::mpt::detail::db_metadata>(
                ...
                auto add_ring = [&cnv_chunk_ids, this](auto const &ring) {
                    KINET_ASSERT(ring.cnv_chunks_len() <= ring.SIZE_ - 1);
                    for (uint32_t k = 0; k < ring.cnv_chunks_len(); k++) {
                        ...
                        if (id == kinet::mpt::detail::db_metadata::NULL_CHUNK) {
                            continue;
                        }
                        KINET_ASSERT(id < pool->chunks(pool->cnv));
                        if (std::find(
                                cnv_chunk_ids.begin(),
                                cnv_chunk_ids.end(),
            ...
            for (auto it = tocompress.begin(); it != tocompress.end();) {
                ...
                for (auto it2 = it;
                    ...
                    if (i.uncompressed_storage == nullptr) {
                        ...
                        auto const unentry = kinet::make_scope_exit(
        ...
        cout << "\nDatabase has been archived to " << archive_database << " "
             << print_bytes(kinet::async::file_offset_t(stat.st_size))
...
{
    CLI::App cli("Tool for managing MPT databases", "kinet-mpt");
    ...
    try {
        impl_t impl(cout, cerr);
        {
            ...
            cli_ops_group->add_flag(
                ...
                "KINET007->KINET008 migration zero-filled the inactive "
            ...
            cli_ops_group->add_flag(
                ...
                "format (KINET008) and ensure it is durable on disk "
                "before exiting. Run after upgrading the kinet apt "
                "package and before starting kinet services.");
            cli.add_option(
                "--archive",
            ...
            cli.add_option(
                ...
                ->check([](std::string const &s) -> std::string {
                    ...
                    constexpr auto max_ring_chunks =
                        kinet::mpt::detail::db_metadata::root_offsets_ring_t::
                            SIZE_ -
                        1;
            ...
            cli.add_option(
                ...
                ->transform(CLI::CheckedTransformer(
                    std::map<
                        std::string,
                        KINET_MPT_NAMESPACE::state_machine_kind>{
                        {"ethereum",
                         KINET_MPT_NAMESPACE::state_machine_kind::ethereum},
                        {"kinet",
                         KINET_MPT_NAMESPACE::state_machine_kind::kinet}},
                    CLI::ignore_case))
            ...
            impl.flags.num_cnv_chunks =
                impl.root_offsets_chunk_count +
                kinet::mpt::UpdateAux::cnv_chunks_for_db_metadata;
            ...
            if (impl.create_empty_database) {
                mode = KINET_ASYNC_NAMESPACE::storage_pool::mode::truncate;
                ...
                mode = KINET_ASYNC_NAMESPACE::storage_pool::mode::open_existing;
                impl.flags.open_read_only = false;
                impl.flags.open_read_only_allow_dirty = false;
            ...
            if (mode == KINET_ASYNC_NAMESPACE::storage_pool::mode::truncate) {
                KINET_ASYNC_NAMESPACE::storage_pool const pool{
        ...
        auto wr_ring(
            needs_write_ring
                ? std::optional<kinet::io::Ring>(kinet::io::RingConfig{4})
                : std::nullopt);
        kinet::io::Buffers rwbuf =
            needs_write_ring
                ? kinet::io::make_buffers_for_segregated_read_write(
                      ...
                      *wr_ring,
                      ...
                      KINET_ASYNC_NAMESPACE::AsyncIO::
                          KINET_IO_BUFFERS_WRITE_SIZE)
                : kinet::io::make_buffers_for_read_only(
                      ring,
                      2,
                      KINET_ASYNC_NAMESPACE::AsyncIO::
                          KINET_IO_BUFFERS_READ_SIZE);
        auto io = KINET_ASYNC_NAMESPACE::AsyncIO{*impl.pool, rwbuf};
        KINET_MPT_NAMESPACE::UpdateAux aux(io);
        ...
        if (aux.metadata_ctx().is_new_pool()) {
            aux.metadata_ctx().set_state_machine_kind(
                KINET_MPT_NAMESPACE::timeline_id::primary, impl.state_machine);
        ...
        // all; neither is a request to restamp the primary.
            ...
            auto const stamped = aux.metadata_ctx().get_state_machine_kind(
                KINET_MPT_NAMESPACE::timeline_id::primary);
        ...
        if (impl.activate_secondary) {
            ...
            if (aux.metadata_ctx().secondary_ring_needs_repair()) {
                cerr << "Secondary ring is corrupt: cnv_chunks[] were "
                        "zero-filled by a pre-fix KINET007->KINET008 migration "
                        "and would alias cnv chunk 0 (the db_metadata chunk). "
                        "Run 'kinet-mpt --repair' before "
                        "--activate-secondary.\n";
                return 1;
            }
            ...
            if (aux.metadata_ctx().repair_inactive_secondary_ring()) {
                cout << "Repaired secondary ring: reset a zero-filled "
                        "cnv_chunks[] (left by a buggy KINET007->KINET008 "
            ...
            if (impl.upgrade_database) {
                if (aux.metadata_ctx().is_new_pool()) {
                    cout << "\nWARNING: --upgrade found no existing DB "
                            "metadata; a fresh KINET008 pool was created. "
                            "Use --create for an explicit new-pool "
                            "workflow.\n";
                }
                else {
                    cout << "\nDB is on version KINET008; flushing "
                            "metadata...\n";
                }
