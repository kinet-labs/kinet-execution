...
struct write_operation_io_receiver
{
    ...
    void set_value(
        ...
        KINET_ASYNC_NAMESPACE::write_single_buffer_sender::result_type const
            res)
    {
        KINET_ASSERT(res);
        KINET_ASSERT(res.assume_value().get().size() == should_be_written);
        res.assume_value()
            .get()
...
using node_writer_unique_ptr_type =
    KINET_ASYNC_NAMESPACE::AsyncIO::connected_operation_unique_ptr_type<
        KINET_ASYNC_NAMESPACE::write_single_buffer_sender,
        write_operation_io_receiver>;

using KINET_ASYNC_NAMESPACE::receiver;
...
class read_long_update_sender
    : public KINET_ASYNC_NAMESPACE::read_multiple_buffer_sender
{
    KINET_ASYNC_NAMESPACE::read_multiple_buffer_sender::buffer_type buffer_;

public:
    template <receiver Receiver>
    explicit read_long_update_sender(Receiver const &receiver)
        : KINET_ASYNC_NAMESPACE::read_multiple_buffer_sender(
              ...
              receiver.bytes_to_read)
    {
        KINET_ASSERT(
            receiver.bytes_to_read >
            KINET_ASYNC_NAMESPACE::AsyncIO::READ_BUFFER_SIZE);
        KINET_ASSERT(buffer_.data() != nullptr);
    }

    read_long_update_sender(read_long_update_sender &&o) noexcept
        : KINET_ASYNC_NAMESPACE::read_multiple_buffer_sender(std::move(o))
...
public:
    ...
    // On disk stuff
    KINET_ASYNC_NAMESPACE::AsyncIO *io{nullptr};
    ...
    // on-disk
    explicit UpdateAux(
        KINET_ASYNC_NAMESPACE::AsyncIO &io_,
    ...
    {
        KINET_ASSERT(metadata_ctx_ != nullptr);
...
template <receiver Receiver>
    requires(
        KINET_ASYNC_NAMESPACE::compatible_sender_receiver<
...
using inflight_map_owning_t = ankerl::unordered_dense::segmented_map<
    ...
    std::vector<std::move_only_function<KINET_ASYNC_NAMESPACE::result<void>(
        NodeCursor const &)>>,
