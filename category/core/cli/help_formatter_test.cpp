...
{
    CLI::App app{"Test description.", "kinet-test"};
    HelpFormatter{"abc123"}.install(app);

    auto const out = render_help(app);
    EXPECT_NE(out.find("kinet-test abc123"), std::string::npos)
        << "expected 'kinet-test abc123' header in:\n"
        << out;
...
{
    CLI::App app{"desc", "kinet-test"};
    HelpFormatter{"v0"}.install(app);

    auto const out = render_help(app);
    EXPECT_NE(out.find("Usage: kinet-test [OPTIONS]"), std::string::npos)
        << out;
}
...
TEST(HelpFormatter, positional_arguments_render_in_arguments_section)
{
    ...
    auto const out = render_help(app);
    EXPECT_NE(
        out.find("Usage: kinet-test [OPTIONS] <ARGS>..."), std::string::npos)
        << out;
