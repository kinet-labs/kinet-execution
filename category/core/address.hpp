...
template <>
struct std::hash<kinet::Address>
{
    size_t operator()(kinet::Address const &x) const noexcept
    {
...
template <>
struct ankerl::unordered_dense::hash<kinet::Address>
{
    using is_avalanching = void;

    uint64_t operator()(kinet::Address const &x) const noexcept
    {
        return std::hash<kinet::Address>{}(x);
    }
};

namespace boost
{
    inline size_t hash_value(kinet::Address const &address) noexcept
    {
        return std::hash<kinet::Address>{}(address);
    }
}
