...
Buffers::Buffers(
    ...
    , wr_ring_(wr_ring)
    , read_bits_{[=] {
        KINET_ASSERT(std::has_single_bit(read_size));
        KINET_ASSERT(read_size >= (1UL << 12));
    ...
    , write_bits_{[=]() -> size_t {
        ...
        KINET_ASSERT(std::has_single_bit(write_size));
        KINET_ASSERT(write_size >= (1UL << 12));
...
{
    if (wr_ring_ != nullptr) {
        KINET_ASSERT(!io_uring_unregister_buffers(&wr_ring_->get_ring()));
    }
    KINET_ASSERT(!io_uring_unregister_buffers(&ring_.get_ring()));
}
