...
static void capture_log(kinet_log const *const input_log, uintptr_t const ptr)
{
    // The "logging" function makes a copy of the `kinet_log` object, to be
    ...
    kinet_log *const output_log = std::bit_cast<kinet_log *>(ptr);
...
TEST(LogFFI, Basic)
{
    ...
    kinet_log_handler *handler;
    kinet_log output = {};

    ASSERT_EQ(
        0,
        kinet_log_handler_create(
            &handler,
            "test_handler",
            capture_log,
            nullptr,
            std::bit_cast<uintptr_t>(&output)));
    ASSERT_EQ(0, kinet_log_init(&handler, 1, SYSLOG_WARN));
