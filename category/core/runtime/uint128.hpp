...
struct uint128_t
{
    ...
    [[nodiscard]] static constexpr uint128_t from_string(char const *const s)
    {
        ...
        if (p[0] == '0' && (p[1] == 'x' || p[1] == 'X')) {
            ...
            size_t num_digits = 0;
            if (*p == '\0') {
                KINET_THROW(std::invalid_argument, s);
            }
            ...
            while (*p != '\0') {
                ...
                if (++num_digits > max_hex_digits) {
                    KINET_THROW(std::out_of_range, s);
                }
                r = (r << 4) | d;
                ++p;
            }
...
template <>
struct std::numeric_limits<kinet::uint128_t>
{
    using type = kinet::uint128_t;
