...
// vectorizer can be turned off with the KINET_VM_NO_VECTORIZE pragma.  Since
// optimization pragmas are not applied to functions that are inlined at call
// sites, any function that is annotated as KINET_VM_NO_VECTORIZE should either
// also be marked as noinline, or only called from other KINET_VM_NO_VECTORIZE
// functions (thus inheriting the caller's attributes).
#if defined(__GNUC__) && !defined(__clang__)
    #define KINET_NO_VECTORIZE __attribute__((optimize("no-tree-vectorize")))
#else
    #define KINET_NO_VECTORIZE
#endif

KINET_NAMESPACE_BEGIN

// The constructor `explicit(true) uint256_t(__m256i x)` further below is
// only meaningful on AVX2 targets. We use a template-trick to disable it on
// non-AVX2 targets.
#if defined(__AVX2__)
// GCC complains about ignored attributes on `__m256i` when used
// directly as a parameter type. We can fix this using another layer of
// indirection by wrapping it in a trivial struct that can only be
// constructed from a `__m256i` value. This is a compile-time artifact,
// meaning it is never materialised at runtime.
struct m256i_wrapper
{
    __m256i const v;

    m256i_wrapper() = delete;

    explicit(false) constexpr m256i_wrapper(__m256i x)
        : v(x)
    {
    }
};
...
template <typename T>
concept m256i_native = std::convertible_to<T, m256i_wrapper>;
using m256i = __m256i;
#else
// On non-AVX2 targets we interpret `__m256i` as four words.
template <typename T>
concept m256i_native = false;
using m256i = words_t<4>;
#endif
...
{
    if consteval {
        return kinet::uint256::portable::addc(lhs, rhs, carry_in);
    }
    else {
        return kinet::uint256::intrinsics::addc(lhs, rhs, carry_in);
    }
}
...
{
    if consteval {
        return kinet::uint256::portable::subb(lhs, rhs, borrow_in);
    }
    else {
        return kinet::uint256::intrinsics::subb(lhs, rhs, borrow_in);
    }
}
...
{
    if consteval {
        return kinet::uint256::portable::shld(high, low, shift);
    }
    else {
        return kinet::uint256::intrinsics::shld(high, low, shift);
    }
}
...
{
    if consteval {
        return kinet::uint256::portable::shrd(high, low, shift);
    }
    else {
        return kinet::uint256::intrinsics::shrd(high, low, shift);
    }
}
...
{
    KINET_DEBUG_ASSERT(u_hi < v);
    if consteval {
        return kinet::uint256::portable::div(u_hi, u_lo, v);
    }
    else {
        return kinet::uint256::intrinsics::div(u_hi, u_lo, v);
    }
}
...
public:
    ...
    template <m256i_native M>
    [[gnu::always_inline]]
    explicit(true) uint256_t(M x) noexcept
    {
    ...
    {
        using namespace kinet::uint256::intrinsics;
...
#undef BITWISE_BINOP
    ...
    friend constexpr uint256_t operator<<(uint256_t const &x, T shift0) noexcept
        requires std::is_convertible_v<T, uint64_t> &&
                 (sizeof(T) <= sizeof(uint64_t))
    {
        if (KINET_UNLIKELY(static_cast<uint64_t>(shift0) >= 256)) {
            return 0;
        }
    ...
    {
        if (KINET_UNLIKELY(shift[3] | shift[2] | shift[1])) {
            return 0;
        }
        return x << shift[0];
    }
...
{
    if consteval {
        return kinet::uint256::portable::mulx(x, y, r_hi, r_lo);
    }
    else {
        return kinet::uint256::intrinsics::mulx(x, y, r_hi, r_lo);
    }
}
...
KINET_NO_VECTORIZE [[gnu::always_inline]]
constexpr words_t<R>
truncating_mul(words_t<M> const &x, words_t<N> const &y) noexcept
    requires(0 < R && 0 < M && 0 < N && R <= M + N)
{
    if consteval {
        return kinet::uint256::portable::truncating_mul<R, M, N>(x, y);
    }
    else {
        return kinet::uint256::intrinsics::truncating_mul<R, M, N>(x, y);
    }
}
...
{
    KINET_DEBUG_ASSERT(m);
    KINET_DEBUG_ASSERT(v);
...
knuth_div(size_t m, uint64_t *u, size_t n, uint64_t const *v, uint64_t *quot)
{
    ...
    KINET_DEBUG_ASSERT(m >= n);
    KINET_DEBUG_ASSERT(n > 1);
    KINET_DEBUG_ASSERT(v[n - 1] & (uint64_t{1} << 63));
    ...
    for (int i = static_cast<int>(m - n); i >= 0; i--) {
        ...
        KINET_DEBUG_ASSERT(u[ix + n] <= v[n - 1]);
        if (KINET_UNLIKELY(u[ix + n] == v[n - 1])) {
            q_hat = ~uint64_t{0};
...
{
    template <>
    struct numeric_limits<kinet::uint256_t>
    {
        using type = kinet::uint256_t;
...
constexpr uint8_t from_dec(char const chr)
{
    ...
    KINET_THROW(std::invalid_argument, "invalid digit");
}
...
template <>
struct std::formatter<kinet::uint256_t>
{
    ...
    auto format(kinet::uint256_t const &v, std::format_context &ctx) const
    {
        return std::format_to(ctx.out(), "0x{}", v.to_string(16));
    }
};

#undef KINET_NO_VECTORIZE
