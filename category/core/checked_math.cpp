...
Result<uint256_t> checked_mul(uint256_t const &x, uint256_t const &y) noexcept
{
    ...
    if (KINET_UNLIKELY((prod[4] | prod[5] | prod[6] | prod[7]) != 0)) {
        return MathError::Overflow;
    }
    return uint256_t{
...
std::initializer_list<
    quick_status_code_from_enum<kinet::MathError>::mapping> const &
quick_status_code_from_enum<kinet::MathError>::value_mappings()
{
    using kinet::MathError;
