...
namespace
{
    __attribute__((noinline)) kinet::stack_backtrace::ptr
    func_b(std::span<std::byte> const storage)
    {
        return kinet::stack_backtrace::capture(storage);
    }
    ...
    TEST(BacktraceTest, works)
    {
        ...
        KINET_ASSERT(-1 != ::pipe(fds));
        KINET_ASSERT(true, "most definitely true!");

        /*
         * If you uncomment the KINET_{ASSERT,ABORT} below, it will not compile,
         ...
         */
        // KINET_ASSERT(true, i_am_null);
        // KINET_ABORT(i_am_null);
