...
constexpr bytes32_t to_bytes(kinet_hash256 const &n) noexcept
{
    return std::bit_cast<bytes32_t>(n);
}
...
template <>
struct std::hash<kinet::bytes32_t>
{
    size_t operator()(kinet::bytes32_t const &x) const noexcept
    {
        return kinet::seeded_fast_hash(x.bytes, sizeof(x.bytes));
    }
};

template <>
struct ankerl::unordered_dense::hash<kinet::bytes32_t>
{
    using is_avalanching = void;

    uint64_t operator()(kinet::bytes32_t const &x) const noexcept
    {
        return std::hash<kinet::bytes32_t>{}(x);
    }
};

namespace boost
{
    inline size_t hash_value(kinet::bytes32_t const &bytes) noexcept
    {
        return std::hash<kinet::bytes32_t>{}(bytes);
    }
}
