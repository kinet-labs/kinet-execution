...
int kinet_hugetlbfs_open_dir_fd(
    struct kinet_hugetlbfs_resolve_params const *const params, int *const dirfd,
    char *pathbuf, size_t pathbuf_size)
{
    ...
    if (params->page_size == 0) {
        rc = kinet_get_default_hugepage_size(&resolve_size);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    rc = kinet_path_open_subdir(
        mountfd,
        params->path_suffix,
        params->create_dirs ? params->dir_create_mode : KINET_PATH_NO_CREATE,
        dirfd,
    ...
    if (rc != 0) {
        return FORMAT_ERRC(
            rc,
            "kinet_path_open_subdir of `%s` underneath `%.*s` failed at last "
            "path component of `%s`",
            params->path_suffix,
            (int)mount_path_len,
            pathbuf,
...
int kinet_get_default_hugepage_size(size_t *const pagesize)
{
    char info_buf[256];
...
int kinet_hugetlbfs_check_path(char const *const path, bool *const is_hugetlbfs)
{
    char *parent_path;
...
char const *kinet_hugetlbfs_get_last_error()
{
    return g_error_buf;
}
