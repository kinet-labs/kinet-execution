...
/// Arguments to kinet_hugetlbfs_open_dir_fd
struct kinet_hugetlbfs_resolve_params
{
...
int kinet_hugetlbfs_open_dir_fd(
    struct kinet_hugetlbfs_resolve_params const *, int *dirfd, char *pathbuf,
    size_t pathbuf_size);
...
int kinet_hugetlbfs_check_path(char const *path, bool *is_hugetlbfs);

/// Return the default hugepage size of the system; if hugepages are not
/// enabled at the kernel level, return EOPNOTSUPP
int kinet_get_default_hugepage_size(size_t *pagesize);
