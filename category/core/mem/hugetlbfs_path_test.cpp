...
TEST(HugetlbfsPath, Basic)
{
    ...
    rc = kinet_get_default_hugepage_size(&default_hpage_size);
    if (rc == EOPNOTSUPP) {
    ...
    kinet_hugetlbfs_resolve_params params = {
    ...
    rc = kinet_hugetlbfs_open_dir_fd(
    ...
    ASSERT_EQ(kinet_hugetlbfs_check_path(hugetlbfs_path, &is_hugetlbfs), 0);
