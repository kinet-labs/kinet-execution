 ...
 * `kinet_event_` API because it is shared as an external library, but this
 ...
 *     calling `kinet_event_get_last_error` will give the string explanation
 ...
 *
 *     #define FORMAT_ERR(...)                                           \
 *     kinet_format_err(g_my_api_err, sizeof(g_my_api_err),              \
 *                      &KINET_SOURCE_LOCATION_CURRENT(), __VA_ARGS__)
 *
 *  Example usage:
 *
...
int kinet_vformat_err(
    char *err_buf, size_t err_buf_size, kinet_source_location_t const *,
    int err, char const *format, va_list ap);

__attribute__((format(printf, 5, 6))) static inline int kinet_format_err(
    char *const err_buf, size_t const err_buf_size,
    kinet_source_location_t const *const src, int const err,
    char const *const format, ...)
{
    va_list ap;
    int rc;
    va_start(ap, format);
    rc = kinet_vformat_err(err_buf, err_buf_size, src, err, format, ap);
    va_end(ap);
    return rc;
}
