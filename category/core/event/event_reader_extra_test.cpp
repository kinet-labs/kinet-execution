...
TEST(EventSnapshotTest, IsSnapshot)
{
    ...
    ASSERT_EQ(
        0,
        kinet_event_is_snapshot_file(fd, snapshot_file.c_str(), &is_snapshot));
    EXPECT_TRUE(is_snapshot);
    close(fd);

    fd = open("/etc/passwd", O_RDONLY);
    ASSERT_NE(fd, -1);
    ASSERT_EQ(0, kinet_event_is_snapshot_file(fd, "/etc/passwd", &is_snapshot));
...
TEST(EventSnapshotTest, Decompress)
{
    ...
    ASSERT_EQ(
        EPROTO,
        kinet_event_decompress_snapshot_fd(
            fd_in, KINET_EVENT_NO_MAX_SIZE, "/etc/passwd", &fd_out));
    ASSERT_EQ(-1, fd_out);
    close(fd_in);
}
