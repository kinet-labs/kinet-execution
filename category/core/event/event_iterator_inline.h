...
#ifndef KINET_EVENT_ITERATOR_INTERNAL
...
kinet_event_iterator_sync_wait(struct kinet_event_iterator *const iter)
{
    ...
    while (write_last_seqno > 0) {
        ...
        struct kinet_event_descriptor const *event = &iter->descriptors[index];
...
inline enum kinet_event_iter_result kinet_event_iterator_try_copy(
    struct kinet_event_iterator const *const iter,
    struct kinet_event_descriptor *const event)
{
    ...
    if (KINET_LIKELY(seqno == iter->read_last_seqno + 1)) {
        ...
        *event = *ring_event;
        ...
        if (KINET_LIKELY(event->seqno == seqno)) {
            return KINET_EVENT_SUCCESS;
        }
        return KINET_EVENT_GAP;
    }
    if (KINET_LIKELY(seqno < iter->read_last_seqno)) {
        return KINET_EVENT_NOT_READY;
    }
    return seqno == iter->read_last_seqno && seqno == 0 ? KINET_EVENT_NOT_READY
                                                        : KINET_EVENT_GAP;
}

inline enum kinet_event_iter_result kinet_event_iterator_try_next(
    struct kinet_event_iterator *const iter,
    struct kinet_event_descriptor *const event)
{
    enum kinet_event_iter_result const r =
        kinet_event_iterator_try_copy(iter, event);
    if (KINET_LIKELY(r == KINET_EVENT_SUCCESS)) {
        ++iter->read_last_seqno;
    }
    return r;
}

inline void kinet_event_iterator_set_seqno(
    struct kinet_event_iterator *const iter, uint64_t const seqno)
{
    iter->read_last_seqno = seqno - 1;
}

inline uint64_t
kinet_event_iterator_reset(struct kinet_event_iterator *const iter)
{
    uint64_t const last_available_seqno = kinet_event_iterator_sync_wait(iter);
