...
#ifndef KINET_EVENT_RECORDER_INTERNAL
...
inline uint64_t kinet_event_get_epoch_nanos()
{
    struct timespec now;
...
static inline struct kinet_event_descriptor *kinet_event_recorder_reserve(
    struct kinet_event_recorder *const recorder, size_t const payload_size,
    uint64_t *const seqno, uint8_t **const payload)
{
    ...
    uint64_t const start_record_timestamp = kinet_event_get_epoch_nanos();
    uint64_t const payload_buf_size = recorder->payload_buf_mask + 1;
    uint64_t const sliding_window_width =
        payload_buf_size - KINET_EVENT_WINDOW_INCR;
    struct kinet_event_ring_control *const rctl = recorder->control;
    size_t const alloc_size =
        kinet_round_size_to_align(payload_size, KINET_EVENT_PAYLOAD_ALIGN);

    if (KINET_UNLIKELY(alloc_size > UINT32_MAX)) {
        *seqno = 0;
        *payload = nullptr;
        return nullptr;
    }
    ...
    if (KINET_UNLIKELY(
        ...
        __atomic_compare_exchange_n(
            ...
            &buffer_window_start,
            kinet_round_size_to_align(payload_end, KINET_EVENT_WINDOW_INCR) -
...
inline void kinet_event_recorder_commit(
    struct kinet_event_descriptor *const event, uint64_t const seqno)
{
