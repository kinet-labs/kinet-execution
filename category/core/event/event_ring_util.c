...
// Create KINET_EVENT_DEFAULT_RING_DIR or override subpaths with rwxrwxr-x
    ...
    *is_snapshot = false;
    ...
    while (zbuf_in.pos < zbuf_in.size) {
        ...
        if (n_blocks++ == 0) {
            ...
            if (zbuf_out.pos < sizeof KINET_EVENT_RING_HEADER_VERSION ||
                memcmp(
                    out_buf,
                    KINET_EVENT_RING_HEADER_VERSION,
                    sizeof KINET_EVENT_RING_HEADER_VERSION) != 0) {
                return FORMAT_ERRC(
                    EPROTO,
        ...
        if (max_size != KINET_EVENT_NO_MAX_SIZE && decomp_size > max_size) {
            return FORMAT_ERRC(
                ENOBUFS,
...
int kinet_event_ring_init_simple(
    struct kinet_event_ring_simple_config const *const ring_config,
    int const ring_fd, off_t const ring_offset, char const *const error_name)
{
    struct kinet_event_ring_size ring_size;
    int rc = kinet_event_ring_init_size(
    ...
    size_t const ring_bytes = kinet_event_ring_calc_storage(&ring_size);
    ...
    return kinet_event_ring_init_file(
        &ring_size,
...
int kinet_event_ring_query_excl_writer_pid(int const ring_fd, pid_t *const pid)
{
    int rc;
    struct kinet_event_flock_info fl_info;
    size_t lock_count = 1;

    *pid = 0;
    rc = kinet_event_ring_query_flocks(ring_fd, &fl_info, &lock_count);
    if (rc != 0) {
        return rc;
    }
...
{
    struct kinet_hugetlbfs_resolve_params const params = {
        .page_size = 1UL << 21,
        .path_suffix = KINET_EVENT_DEFAULT_RING_DIR,
        .create_dirs = true,
        .dir_create_mode = DIR_CREATE_MODE};
    int const rc =
        kinet_hugetlbfs_open_dir_fd(&params, dirfd, pathbuf, pathbuf_size);
    if (rc != 0) {
        // Copy the error message directly, since we added nothing interesting
        strlcpy(
            _g_kinet_event_ring_error_buf,
            kinet_hugetlbfs_get_last_error(),
            sizeof _g_kinet_event_ring_error_buf);
    }
    return rc;
}

int kinet_check_path_supports_map_hugetlb(
    char const *const path, bool *const supported)
{
    int const rc = kinet_hugetlbfs_check_path(path, supported);
    if (rc != 0) {
        strlcpy(
            _g_kinet_event_ring_error_buf,
            kinet_hugetlbfs_get_last_error(),
...
int kinet_event_open_hugetlbfs_dir_fd(int *, char *, size_t)
{
    return FORMAT_ERRC(ENOSYS, "system does not support hugetlbfs");
}

int kinet_check_path_supports_map_hugetlb(char const *, bool *const supported)
{
    *supported = false;
    return 0;
}
    ...
    size_t pathbuf_size)
{
    ...
    if (default_path == KINET_EVENT_DEFAULT_HUGETLBFS) {
        rc = kinet_event_open_hugetlbfs_dir_fd(nullptr, pathbuf, pathbuf_size);
        if (rc != 0) {
            return rc;
        }
    }
        ...
        if (rc != 0) {
            return FORMAT_ERRC(
                rc,
                "kinet_path_open_subdir of `%s` failed at `%s`",
                default_path,
                pathbuf);
        }
    }
    ...
    rc = kinet_path_append(&append, file, &pathbuf_size);
    if (rc != 0) {
        return FORMAT_ERRC(
            rc, "kinet_path_append of %s failed; partial: %s", file, pathbuf);
    }
    ...
    int const fd, char const *const error_name, bool *const is_snapshot)
{
    ...
    // Decompress the fd as if by kinet_event_decompress_snapshot_fd, but with
    ...
    *is_snapshot = false;
    int const rc = decompress_snap_fd_to_temp_file(
        fd, sizeof KINET_EVENT_RING_HEADER_VERSION, error_name, &out);
    (void)close(out.fd);
