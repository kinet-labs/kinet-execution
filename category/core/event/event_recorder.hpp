...
protected:
    alignas(64) kinet_event_recorder recorder_;
    ...
    setup_record_error_event(
        uint16_t event_type, kinet_event_record_error_type,
...
{
    static_assert(
        alignof(T) <= KINET_EVENT_PAYLOAD_ALIGN,
    ...
    static_assert(
        sizeof(T) + sizeof(kinet_event_record_error) <=
        RECORD_ERROR_TRUNCATED_SIZE);

    // In the event of a recording error, we'll record the `T` instance at
    // the next address after a `struct kinet_event_record_error` object;
    // this ensures we won't under-align the `T{}`
    static_assert(alignof(kinet_event_record_error) >= alignof(T));
    ...
    if (KINET_UNLIKELY(
        ...
        auto const [event, header_buf, seqno] = setup_record_error_event(
            ...
            KINET_EVENT_RECORD_ERROR_OVERFLOW_4GB,
            sizeof(T),
            trailing_bufs_array,
            payload_size);
    ...
    if (KINET_UNLIKELY(
            alloc_payload_size >=
            recorder_.payload_buf_mask + 1 - 2 * KINET_EVENT_WINDOW_INCR)) {
        ...
        auto const [event, header_buf, seqno] = setup_record_error_event(
            std::to_underlying(event_type),
            KINET_EVENT_RECORD_ERROR_OVERFLOW_EXPIRE,
            sizeof(T),
            trailing_bufs_array,
            payload_size);
        return {event, reinterpret_cast<T *>(header_buf), seqno};
...
{
    kinet_event_recorder_commit(r.event, r.seqno);
}
