...
1. **Reserve resources for recording** -- first, the caller allocates an
   ...
   are allocated by the C API `kinet_event_recorder_reserve`
...
3. **Commit the event** -- finally, the caller marks the event as "ready"
   by calling the `kinet_event_recorder_commit` function, passing in the
...
The template type `T` is an event payload type, e.g., `struct kinet_exec_txn_log`
...
[advanced topics](https://docs.kinet.xyz/execution-events/advanced)
section of the SDK. Then read the comments in the `kinet_event_recorder_reserve`
...
// the reserve step does it (thus why we passed `KINET_TEST_EVENT_VLT` above)
...
- `T`, the event payload type template argument, is explicitly specified when
  ...
  `reserve_event<kinet_test_event_vlt>(KINET_TEST_EVENT_VLT, ...)`
...
- We usually _don't_ have to specify the template argument in the return
  ...
  verbose `ReservedEvent<kinet_test_event_vlt> const vlt_event` because the
...
- `EventEnum` is the specific enumeration type used by event ring's content
   type, e.g., `enum kinet_test_event` for the test suite events; the actual
   type is deduced from the argument, which above is `KINET_TEST_EVENT_VLT`;
...
arrays. In the test event schema, `KINET_TEST_EVENT_VLT` is a dummy event that
...
`struct kinet_exec_txn_log`. An instance of it does _not_ already exist prior
...
returns a `kinet_exec_txn_log *` pointing to the payload buffer space for us to
...
It has payload type `struct kinet_event_record_error`, defined in
`event_ring.h`.
...
has recording error type `KINET_EVENT_RECORD_ERROR_OVERFLOW_EXPIRE`.
...
payload has expired, in the `kinet_event_ring_payload_check` function.
