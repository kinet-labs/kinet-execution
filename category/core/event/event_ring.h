...
struct kinet_event_ring_header;

enum kinet_event_content_type : uint16_t;
enum kinet_event_record_error_type : uint16_t;
...
struct kinet_event_ring
{
    ...
    struct kinet_event_ring_header *header;     ///< Event ring metadata
    struct kinet_event_descriptor *descriptors; ///< Event descriptor ring array
...
static_assert(sizeof(struct kinet_event_descriptor) == 64);

/// Describes the size of an event ring's primary data structures
struct kinet_event_ring_size
{
...
struct kinet_event_ring_header
{
    ...
    enum kinet_event_content_type
        content_type;                        ///< Kind of events in this ring
    uint8_t schema_hash[32];                 ///< Ensure event definitions match
    struct kinet_event_ring_size size;       ///< Size of following structures
    struct kinet_event_ring_control control; ///< Tracks ring's state/status
};
...
enum kinet_event_content_type : uint16_t
{
    KINET_EVENT_CONTENT_TYPE_NONE,  ///< An invalid value
    KINET_EVENT_CONTENT_TYPE_TEST,  ///< Used in simple automated tests
    KINET_EVENT_CONTENT_TYPE_EXEC,  ///< Core execution events
    KINET_EVENT_CONTENT_TYPE_COUNT  ///< Total number of content types
};
...
int kinet_event_ring_init_size(
    uint8_t descriptors_shift, uint8_t payload_buf_shift,
    uint16_t context_large_pages, struct kinet_event_ring_size *);
...
size_t kinet_event_ring_calc_storage(struct kinet_event_ring_size const *);
...
int kinet_event_ring_init_file(
    struct kinet_event_ring_size const *, enum kinet_event_content_type,
...
int kinet_event_ring_mmap(
    struct kinet_event_ring *, int mmap_prot, int mmap_extra_flags, int ring_fd,
    off_t ring_offset, char const *error_name);

/// Remove an event ring's shared memory mappings from our process' address
/// space
void kinet_event_ring_unmap(struct kinet_event_ring *);
...
static bool kinet_event_ring_try_copy(
    struct kinet_event_ring const *, uint64_t seqno,
    struct kinet_event_descriptor *);

/// Obtain a pointer to the event's payload in shared memory in a zero-copy
/// fashion; to check for expiration, call kinet_event_payload_check
static void const *kinet_event_ring_payload_peek(
    struct kinet_event_ring const *, struct kinet_event_descriptor const *);

/// Return true if the zero-copy buffer returned by kinet_event_payload_peek
/// still contains the event payload for the given descriptor; returns false if
/// the event payload has been overwritten
static bool kinet_event_ring_payload_check(
    struct kinet_event_ring const *, struct kinet_event_descriptor const *);
...
static void *kinet_event_ring_payload_memcpy(
    struct kinet_event_ring const *, struct kinet_event_descriptor const *,
    void *dst, size_t n);

/// Initialize an iterator to point to the most recently produced event in the
/// event ring
int kinet_event_ring_init_iterator(
    struct kinet_event_ring const *, struct kinet_event_iterator *);

/// Initialize a recorder to write into an event ring
int kinet_event_ring_init_recorder(
    struct kinet_event_ring const *, struct kinet_event_recorder *);

/// Return a description of the last error that occurred on this thread
char const *kinet_event_ring_get_last_error();
...
/// resident, e.g., `enum kinet_event_content_type`)
constexpr uint8_t KINET_EVENT_RING_HEADER_VERSION[] = {
    'R', 'I', 'N', 'G', '0', '1'};

/// Array of human-readable names for the event ring content types
extern char const
    *g_kinet_event_content_type_names[KINET_EVENT_CONTENT_TYPE_COUNT];

/*
 * Event ring size limits
 */

constexpr uint8_t KINET_EVENT_MIN_DESCRIPTORS_SHIFT = 16;
constexpr uint8_t KINET_EVENT_MAX_DESCRIPTORS_SHIFT = 32;

constexpr uint8_t KINET_EVENT_MIN_PAYLOAD_BUF_SHIFT = 27;
constexpr uint8_t KINET_EVENT_MAX_PAYLOAD_BUF_SHIFT = 40;

/// Sliding window increment, see `event_recorder.md` documentation
constexpr uint64_t KINET_EVENT_WINDOW_INCR = 1UL << 24;

/// Allocations from an event ring payload buffer have this alignment
constexpr size_t KINET_EVENT_PAYLOAD_ALIGN = 16;
...
// `RECORD_ERROR` which indicates a payload of `struct kinet_event_record_error`
constexpr uint16_t KINET_EVENT_RECORD_ERROR_EVENT_TYPE = 1;

struct kinet_event_record_error
{
    alignas(16) enum kinet_event_record_error_type
...
enum kinet_event_record_error_type : uint16_t
{
    KINET_EVENT_RECORD_ERROR_NONE,            ///< No error
    KINET_EVENT_RECORD_ERROR_OVERFLOW_4GB,    ///< Payload overflows UINT32_MAX
    KINET_EVENT_RECORD_ERROR_OVERFLOW_EXPIRE, ///< Payload expired on creation
    KINET_EVENT_RECORD_ERROR_MISSING_EVENT,   ///< Missing expected from peer
};
...
inline bool kinet_event_ring_try_copy(
    struct kinet_event_ring const *const event_ring, uint64_t const seqno,
    struct kinet_event_descriptor *const event)
{
    if (KINET_UNLIKELY(seqno == 0)) {
        return false;
    }
    struct kinet_event_descriptor const *const ring_event =
    ...
    *event = *ring_event;
    ...
    if (KINET_UNLIKELY(ring_seqno != seqno)) {
        return false;
    }
    return true;
}

inline void const *kinet_event_ring_payload_peek(
    struct kinet_event_ring const *const event_ring,
    struct kinet_event_descriptor const *const event)
{
    return event_ring->payload_buf +
           (event->payload_buf_offset & event_ring->payload_buf_mask);
}

inline bool kinet_event_ring_payload_check(
    struct kinet_event_ring const *const event_ring,
    struct kinet_event_descriptor const *const event)
{
...
inline void *kinet_event_ring_payload_memcpy(
    struct kinet_event_ring const *const event_ring,
    struct kinet_event_descriptor const *const event, void *const dst,
    size_t const n)
{
    if (KINET_UNLIKELY(!kinet_event_ring_payload_check(event_ring, event))) {
        return nullptr;
    }
    void const *const src = kinet_event_ring_payload_peek(event_ring, event);
    memcpy(dst, src, n);
    if (KINET_UNLIKELY(!kinet_event_ring_payload_check(event_ring, event))) {
