...
thread_local char _g_kinet_event_ring_error_buf[1024];
...
int kinet_event_ring_init_size(
    ...
    struct kinet_event_ring_size *const size)
{
    ...
    if (descriptors_shift < KINET_EVENT_MIN_DESCRIPTORS_SHIFT ||
        descriptors_shift > KINET_EVENT_MAX_DESCRIPTORS_SHIFT) {
        return FORMAT_ERRC(
            ERANGE,
            "descriptors_shift %hhu outside allowed range [%hhu, %hhu]: "
            "(ring sizes: [%lu, %lu])",
            descriptors_shift,
            KINET_EVENT_MIN_DESCRIPTORS_SHIFT,
            KINET_EVENT_MAX_DESCRIPTORS_SHIFT,
            (1UL << KINET_EVENT_MIN_DESCRIPTORS_SHIFT),
            (1UL << KINET_EVENT_MAX_DESCRIPTORS_SHIFT));
    }
    if (payload_buf_shift < KINET_EVENT_MIN_PAYLOAD_BUF_SHIFT ||
        payload_buf_shift > KINET_EVENT_MAX_PAYLOAD_BUF_SHIFT) {
        return FORMAT_ERRC(
            ERANGE,
            "payload_buf_shift %hhu outside allowed range [%hhu, %hhu]: "
            "(buffer sizes: [%lu, %lu])",
            payload_buf_shift,
            KINET_EVENT_MIN_PAYLOAD_BUF_SHIFT,
            KINET_EVENT_MAX_PAYLOAD_BUF_SHIFT,
            (1UL << KINET_EVENT_MIN_PAYLOAD_BUF_SHIFT),
            (1UL << KINET_EVENT_MAX_PAYLOAD_BUF_SHIFT));
    }
...
size_t kinet_event_ring_calc_storage(
    struct kinet_event_ring_size const *const ring_size)
{
    return PAGE_2MB +
           ring_size->descriptor_capacity *
               sizeof(struct kinet_event_descriptor) +
...
int kinet_event_ring_init_file(
    struct kinet_event_ring_size const *const ring_size,
    enum kinet_event_content_type const content_type,
    ...
    off_t const ring_offset, char const *error_name)
{
    ...
    struct kinet_event_ring_header header;
    char namebuf[64];
    ...
    // via a call to kinet_event_ring_init_size
    ...
    if (content_type == KINET_EVENT_CONTENT_TYPE_NONE ||
        content_type >= KINET_EVENT_CONTENT_TYPE_COUNT) {
        return FORMAT_ERRC(
            EINVAL,
    ...
    memcpy(header.magic, KINET_EVENT_RING_HEADER_VERSION, sizeof header.magic);
    memcpy(header.schema_hash, schema_hash, sizeof header.schema_hash);
    header.content_type = content_type;
    header.size = *ring_size;
    ring_bytes = kinet_event_ring_calc_storage(ring_size);
...
int kinet_event_ring_mmap(
    struct kinet_event_ring *const event_ring, int const mmap_prot,
    int const mmap_extra_flags, int const ring_fd, off_t const ring_offset,
    char const *error_name)
{
    int rc;
    char namebuf[64];
    struct kinet_event_ring_header const *header;
    ...
    if (memcmp(
            header->magic,
            KINET_EVENT_RING_HEADER_VERSION,
...
Error:
    kinet_event_ring_unmap(event_ring);
    return rc;
}

void kinet_event_ring_unmap(struct kinet_event_ring *const event_ring)
{
    struct kinet_event_ring_header const *const header = event_ring->header;
    if (header != nullptr) {
...
int kinet_event_ring_init_iterator(
    ...
    struct kinet_event_iterator *const iter)
{
    memset(iter, 0, sizeof *iter);
    struct kinet_event_ring_header const *header = event_ring->header;
    if (header == nullptr) {
    ...
    (void)kinet_event_iterator_reset(iter);
    return 0;
}

int kinet_event_ring_init_recorder(
    struct kinet_event_ring const *const event_ring,
    struct kinet_event_recorder *const recorder)
{
    memset(recorder, 0, sizeof *recorder);
    struct kinet_event_ring_header *header = event_ring->header;
    if (header == nullptr) {
        return FORMAT_ERRC(EINVAL, "event_ring has been unmapped");
    }
...
{
    return _g_kinet_event_ring_error_buf;
}

char const *g_kinet_event_content_type_names[KINET_EVENT_CONTENT_TYPE_COUNT] = {
    [KINET_EVENT_CONTENT_TYPE_NONE] = "none",
    [KINET_EVENT_CONTENT_TYPE_TEST] = "test",
    [KINET_EVENT_CONTENT_TYPE_EXEC] = "exec",
};
