...
#if defined(KINET_COMPILER_TESTING) || defined(KINET_COMPILER_BENCHMARKS)
...
namespace kinet::vm
{
    ...
    {
        if (KINET_UNLIKELY(mode == CompilerOnly)) {
        ...
        SharedVarcode const &vcode)
    {
        ...
        if (KINET_LIKELY(ncode != nullptr)) {
            // The bytecode is compiled.
            if (KINET_UNLIKELY(ncode->chain_id() != traits::id())) {
            ...
            if (KINET_UNLIKELY(entry == nullptr)) {
        ...
        KINET_DEBUG_ASSERT(result.gas_left >= 0);
        KINET_DEBUG_ASSERT(msg_gas >= result.gas_left);
    ...
    void VM::debug_set_execute_override(ExecuteOverride f [[maybe_unused]])
    {
        if constexpr (enable_execute_override) {
            ...
            KINET_ABORT("debug_set_execute_override is not available in this "
                        "build configuration (requires KINET_COMPILER_TESTING "
                        "or KINET_COMPILER_BENCHMARKS)");
        }
    }
