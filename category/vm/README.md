...
Virtual Machine. The native backend uses asmjit to compile EVM bytecode
directly to x86 machine code for faster execution. The work in this project is
a C++ reimplementation of the [`kinet-jit`][jit] project, with the eventual
aim of merging the compiler into the core Kinet monorepo.
...
```
$ build/test/ethereum_test/kinet-ethereum-test
```
...
```
$ build/test/ethereum_test/kinet-ethereum-test --help
```
...
By setting the `KINET_COMPILER_ASM_DIR` environment
variable to a valid directory, the compiler will print contract
assembly code in files to this directory.

The `KINET_COMPILER_DEBUG_TRACE=1` environment variable can be
...
```
$ export KINET_COMPILER_ASM_DIR=/tmp/debug
$ export KINET_COMPILER_DEBUG_TRACE=1
...
### The `KINET_COMPILER_TESTING` configuration

If the project is configured with `KINET_COMPILER_TESTING` enabled, e.g.
```
$ cmake -S . -B build -DKINET_COMPILER_TESTING=ON
```
...
`kinet-compiler`, one for each basic block entered.
...
by default. To enable them, set `KINET_COMPILER_BENCHMARKS=ON` via CMake. This
option is orthogonal to `KINET_COMPILER_TESTING`; it's possible to build the
...
$ CC=gcc-14 CXX=g++-14 cmake                                \
    ...
    -DKINET_COMPILER_TESTING=Off                            \
    -DKINET_COMPILER_BENCHMARKS=On
$ cmake --build build
...
$ CC=gcc-14 CXX=g++-14 cmake                                \
    ...
    -DKINET_COMPILER_TESTING=Off                            \
    -DKINET_COMPILER_BENCHMARKS=On                          \
    -DKINET_VM_INTERPRETER_STATS=On
...
- A compiler from EVM bytecode to native x86 machine code (using asmjit); the
  programs generated by this compiler rely on being linked against a runtime
  support library, and on being run in the context of an EVMC-compatible host.
- A runtime support library for EVM programs that handles interaction with the
  host context (gas calculations etc.).
- An EVMC-compatible VM layer that handles compiling bytecode to native code,
...
[jit]: https://github.com/kinet-crypto/kinet-jit
...
Passing `-DKINET_COMPILER_DUMP_ASM=On` to CMake, will dump all `.s` assembly files for the kinet code into `build/asm`. One can then use the following vscode extension: https://github.com/dseight/vscode-disasexpl to view the assembly. In order for this to work, add the following setting to `.vscode/settings.json` file inside the `kinet-compiler` repo (if your CMake build folder is called something else, modify appropriately):
