...
namespace kinet::vm::compiler::basic_blocks
{
    /**
    ...
    public:
         ...
         */
        template <Traits traits = EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>
        BasicBlocksIR(
        ...
        {
            KINET_ASSERT(bytes.size() <= *interpreter::code_size_t::max());
    ...
    private:
        ...
        void add_jump_dest()
        {
            KINET_DEBUG_ASSERT(blocks_.back().instrs.empty());
    ...
    {
        KINET_DEBUG_ASSERT(current_offset < bytes.size());
        ...
        : codesize(byte_count)
    {
        ...
        while (current_offset < *byte_count) {
            ...
            if (st == St::OUTSIDE_BLOCK) {
                ...
                KINET_ASSERT(st == St::INSIDE_BLOCK);

                auto handle_terminator = [&](Terminator t) {
                    using enum Terminator;
                    KINET_ASSERT(t != FallThrough);
...
template <>
struct std::formatter<kinet::vm::compiler::basic_blocks::Terminator>
{
    ...
    auto format(
        kinet::vm::compiler::basic_blocks::Terminator const &t,
        std::format_context &ctx) const
    {
        auto const *v = [t] {
            using enum kinet::vm::compiler::basic_blocks::Terminator;
...
template <>
struct std::formatter<kinet::vm::compiler::basic_blocks::Block>
{
    constexpr auto parse(std::format_parse_context &ctx)
    {
        return ctx.begin();
    ...
    auto format(
        kinet::vm::compiler::basic_blocks::Block const &blk,
        std::format_context &ctx) const
    {
        ...
        if (blk.fallthrough_dest != kinet::vm::compiler::INVALID_BLOCK_ID) {
...
template <>
struct std::formatter<kinet::vm::compiler::basic_blocks::BasicBlocksIR>
{
    constexpr auto parse(std::format_parse_context &ctx)
    {
        return ctx.begin();
