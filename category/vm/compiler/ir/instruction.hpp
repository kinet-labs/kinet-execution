...
namespace kinet::vm::compiler
{
    ...
    {
        KINET_DEBUG_ASSERT(immediate_value == 0 || opcode() == OpCode::Push);
    }

    constexpr uint256_t const &Instruction::immediate_value() const noexcept
    {
        KINET_ASSERT(opcode() == OpCode::Push);
    ...
    {
        using enum kinet::vm::compiler::OpCode;
...
template <>
struct std::formatter<kinet::vm::compiler::OpCode>
{
    ...
    auto format(
        kinet::vm::compiler::OpCode const &op, std::format_context &ctx) const
    {
...
template <>
struct std::formatter<kinet::vm::compiler::Instruction>
{
    constexpr auto parse(std::format_parse_context &ctx)
    {
        return ctx.begin();
    ...
    auto format(
        kinet::vm::compiler::Instruction const &inst,
