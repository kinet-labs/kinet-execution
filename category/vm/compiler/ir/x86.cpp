...
using namespace kinet::vm::compiler::basic_blocks;
...
namespace
{
        ...
        Emitter &emit, native_code_size_t const max_native_size)
    {
        ...
        if (KINET_UNLIKELY(size_estimate > *max_native_size)) {
        ...
        native_code_size_t const max_native_size, CompilerConfig const &config)
    {
        ...
        for (auto const &instr : block.instrs) {
            KINET_DEBUG_ASSERT(remaining_base_gas >= instr.static_gas_cost());
    ...
    emit_terminator(Emitter &emit, BasicBlocksIR const &ir, Block const &block)
    {
        ...
        case FallThrough:
            ...
            break;
        case JumpI:
            KINET_DEBUG_ASSERT(block.fallthrough_dest != INVALID_BLOCK_ID);
...
namespace kinet::vm::compiler::native
{
        ...
        CompilerConfig const &config)
    {
        ...
        KINET_DEBUG_ASSERT(size_estimate <= *max_native_size);
        return std::make_shared<Nativecode>(
            rt,
            traits::id(),
            entry,
