...
namespace runtime = kinet::vm::runtime;
...
namespace
{
    using namespace kinet::vm::compiler::native;
    using kinet::uint256_t;
...
namespace kinet::vm::compiler::native
{
    ...
    asmjit::x86::Mem Emitter::RoData::add32(uint256_t const &x)
    {
        ...
        if (KINET_UNLIKELY(data_.size() >= (1 << 26))) {
    ...
    void Emitter::RuntimeImpl::call_impl()
    {
        ...
        KINET_DEBUG_ASSERT(arg_count_ <= MAX_RUNTIME_ARGS);
        KINET_DEBUG_ASSERT(
        ...
        for (size_t a = 0, i = 0; i < arg_count_; ++i) {
            ...
            if (elem->stack_offset()) {
                ...
                KINET_DEBUG_ASSERT(elem->literal().has_value());
    ...
    {
        KINET_DEBUG_ASSERT(arg_count_ >= implicit_arg_count());
    ...
    entrypoint_t Emitter::finish_contract(asmjit::JitRuntime &rt)
    {
        ...
        if (jump_table_label_) {
            ...
            for (size_t bid = 0; bid < *bytecode_size_; ++bid) {
                auto const lbl = jump_dests_.find(bid);
    ...
    void Emitter::runtime_store_input_stack(uint64_t const base_offset)
    {
        ...
        auto const skip_lbl = as_.newLabel();
    ...
    void Emitter::runtime_print_top2(std::string const &msg)
    {
        ...
        auto const e1 = stack_.get(stack_.top_index());
        ...
        auto const e2 = stack_.get(stack_.top_index() - 1);
    ...
    {
        KINET_ASSERT(x.general_reg().has_value());
        KINET_ASSERT(y.general_reg().has_value());
    ...
    void Emitter::add_jump_dest(byte_offset const d)
    {
        ...
        KINET_DEBUG_ASSERT(size < sizeof(name));
        ...
        StackElemRef elem, std::tuple<LiveSet...> const &live)
    {
        ...
        auto const r = stack_.release_general_reg(std::move(elem));
        ...
        StackElemRef elem, std::tuple<LiveSet...> const &live)
    {
        ...
        auto const r = stack_.release_avx_reg(std::move(elem));
    ...
    void Emitter::gas_decrement_no_check(int64_t const gas)
    {
        ...
        if (KINET_UNLIKELY(gas > std::numeric_limits<int32_t>::max())) {
    ...
    bool Emitter::block_prologue(basic_blocks::Block const &b)
    {
        ...
        auto const it = jump_dests_.find(b.offset);
        if (it != jump_dests_.end()) {
            as_.bind(it->second);
        }

        if (KINET_UNLIKELY(runtime_debug_trace_) && !keep_stack) {
    ...
    #define KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT() ++final_write_count
    #define KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT_IF(b)                     \
        do {                                                                   \
            if (b) {                                                           \
                KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT();                    \
            }                                                                  \
        }                                                                      \
        while (0)
    #define KINET_COMPILER_X86_FINAL_WRITE_COUNT final_write_count
#else
    #define KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT()
    #define KINET_COMPILER_X86_FINAL_WRITE_COUNT 0
    #define KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT_IF(b) (void)sizeof(b)
#endif
        ...
        KINET_DEBUG_ASSERT(!init1_spill);
        auto const init_yx1 = avx_reg_to_ymm(*init1->avx_reg());
        auto yx1 = init_yx1;
        if (spill_elem_has_new_mem_location) {
            KINET_DEBUG_ASSERT(spill_elem->stack_offset().has_value());
            as_.vmovaps(
            ...
            KINET_COMPILER_X86_INC_FINAL_WRITE_COUNT_IF(
        ...
        while (!non_dep.empty()) {
            ...
            if (!d->avx_reg()) {
                ...
                if (d->stack_offset()) {
                    ...
                    KINET_DEBUG_ASSERT(d->general_reg().has_value());
                    auto const m =
            ...
            if (!d->avx_reg() && d->stack_offset()) {
                ...
                KINET_DEBUG_ASSERT(dep_counts[e] > 0);
                if (--dep_counts[e] == 0) {
                    non_dep.push_back(e);
                }
            }
        ...
        for (auto const [e, ec] : dep_counts) {
            ...
            StackElem *d = e;
            do {
                KINET_DEBUG_ASSERT(dep_counts[d] == 1);
                KINET_DEBUG_ASSERT(!d->avx_reg().has_value());
                KINET_DEBUG_ASSERT(d->stack_offset().has_value());
                dep_counts[d] = 0;
                cycle.push_back(d);
                KINET_DEBUG_ASSERT(
            ...
            KINET_DEBUG_ASSERT(cycle.size() >= 2);
            as_.vmovaps(
...
#ifdef KINET_COMPILER_TESTING
        KINET_ASSERT(
            KINET_COMPILER_X86_FINAL_WRITE_COUNT ==
    ...
    void Emitter::discharge_deferred_comparison()
    {
        ...
        if (dc.negated_stack_elem) {
            auto const comp = negate_comparison(dc.comparison());
    ...
    {
        KINET_ASSERT(debug_logger_.file());
        ...
        StackElemRef e, std::tuple<LiveSet...> const &live)
    {
        ...
        KINET_DEBUG_ASSERT(e->avx_reg().has_value());
        return 8;
    }
    ...
    uint8_t Emitter::volatile_gpq_index()
    {
        static_assert(
            ...
            gpq == x86::rdx);
        KINET_DEBUG_ASSERT(volatile_general_reg == rdi_general_reg);
        KINET_DEBUG_ASSERT(volatile_general_reg == rsi_general_reg);
        KINET_DEBUG_ASSERT(volatile_general_reg == rcx_general_reg);
        KINET_DEBUG_ASSERT(volatile_general_reg == rdx_general_reg);
    ...
    {
        KINET_ASSERT(elem->literal().has_value());
        ...
        StackElemRef elem, asmjit::x86::Mem const &mem)
    {
        if (elem->avx_reg()) {
            ...
            KINET_DEBUG_ASSERT(elem->stack_offset().has_value());
    ...
    void Emitter::mov_stack_elem_to_gpq256(StackElemRef elem, Gpq256 const &gpq)
    {
        if (elem->general_reg()) {
            ...
            KINET_ASSERT(elem->avx_reg().has_value());
            if constexpr (remember_intermediate) {
    ...
    void Emitter::mov_stack_elem_to_avx_reg(StackElemRef elem)
    {
        ...
        if (elem->literal()) {
            ...
            KINET_ASSERT(elem->general_reg().has_value());
    ...
    {
        KINET_DEBUG_ASSERT(elem->general_reg().has_value());
    ...
    {
        KINET_DEBUG_ASSERT(elem->avx_reg().has_value());
    ...
    Emitter::revertible_mov_stack_offset_to_general_reg(StackElemRef const elem)
    {
        ...
        auto reg_elem = [this] {
            ...
            KINET_DEBUG_ASSERT(!spill.has_value());
            return x;
        };
    ...
    {
        KINET_DEBUG_ASSERT(e->general_reg().has_value());
        ...
        asmjit::x86::Ymm const dst)
    {
        ...
        if (auto const *y = std::get_if<x86::Ymm>(&src)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(src));
    ...
    void Emitter::mov_stack_elem_to_mem_be(StackElemRef const e, x86::Mem m)
    {
        if (e->literal()) {
            ...
            if (e->avx_reg()) {
                ...
                KINET_DEBUG_ASSERT(e->stack_offset().has_value());
    ...
    void Emitter::byte()
    {
        ...
        if (src->general_reg()) {
            ...
            if (ix->literal()) {
                KINET_DEBUG_ASSERT(src->stack_offset().has_value());
        ...
        std::tuple<LiveSet...> const &live)
    {
        if (shift->literal() && value->literal()) {
            auto const &i = shift->literal()->value;
            auto const &x = value->literal()->value;
            return stack_.alloc_literal({kinet::sar(i, x)});
        }
    ...
    void Emitter::eq()
    {
        ...
        if (left_loc == LocationType::AvxReg) {
            ...
            KINET_DEBUG_ASSERT(left_loc == LocationType::GeneralReg);
            Gpq256 const &gpq = general_reg_to_gpq256(*dst->general_reg());
            as_.or_(gpq[0], gpq[1]);
            as_.or_(gpq[2], gpq[3]);
            as_.or_(gpq[0], gpq[2]);
    ...
    Emitter::iszero(StackElemRef const elem)
    {
        ...
        KINET_DEBUG_ASSERT(left == right);
        if (loc == LocationType::AvxReg) {
            x86::Ymm const y = avx_reg_to_ymm(*left->avx_reg());
            as_.vptest(y, y);
        }
        else {
            KINET_DEBUG_ASSERT(loc == LocationType::GeneralReg);
    ...
    void Emitter::push_iszero(StackElemRef elem)
    {
        ...
        if (std::holds_alternative<Comparison>(res)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<StackElemRef>(res));
    ...
    std::optional<Comparison> Emitter::issigned(StackElemRef const e)
    {
        ...
        KINET_DEBUG_ASSERT(!e->literal().has_value());
    ...
    {
        KINET_DEBUG_ASSERT(remaining_base_gas >= 0);
        discharge_deferred_comparison();
        auto [dst, _] = alloc_general_reg();
        Gpq256 const &gpq = general_reg_to_gpq256(*dst->general_reg());
        as_.mov(
    ...
    void Emitter::calldataload()
    {
        ...
        // kinet_vm_runtime_load_bounded_le_raw expects context to be passed
        // in rbx.
        ...
        KINET_DEBUG_ASSERT(rdi_general_reg == volatile_general_reg);
        KINET_DEBUG_ASSERT(rsi_general_reg == volatile_general_reg);
        ...
        if (auto const *lit = std::get_if<uint64_t>(&offset_op)) {
            ...
            KINET_DEBUG_ASSERT(r != x86::rdi && r != x86::rsi);
            as_.mov(x86::rdi, x86::qword_ptr(reg_context, data_offset));
            as_.mov(x86::esi, x86::dword_ptr(reg_context, size_offset));
            as_.add(x86::rdi, r);
            as_.sub(x86::rsi, r);
        ...
        auto const load_bounded_fn =
            rodata_.add_external_function(kinet_vm_runtime_load_bounded_le_raw);
    ...
    void Emitter::mstore8()
    {
        ...
        if (value->general_reg()) {
            ...
            KINET_DEBUG_ASSERT(value->stack_offset().has_value());
            KINET_DEBUG_ASSERT(volatile_general_reg == rcx_general_reg);
            KINET_DEBUG_ASSERT(
    ...
    {
        KINET_DEBUG_ASSERT(ft.offset <= *bytecode_size_);
    ...
    {
        if (dest->literal()) {
            auto const lit = literal_jump_dest_operand(std::move(dest));
            write_to_final_stack_offsets();
            adjust_by_stack_delta<false>();
    ...
    asmjit::Label const &Emitter::jump_dest_label(uint256_t const &dest)
    {
        if (dest >= *bytecode_size_) {
            ...
            auto const it = jump_dests_.find(dest[0]);
            if (it == jump_dests_.end()) {
                return error_label_;
            }
            else {
        ...
        StackElemRef const &dest, std::tuple<LiveSet...> const &live)
    {
        ...
        if (dest->general_reg()) {
            ...
            KINET_DEBUG_ASSERT(dest->avx_reg().has_value());
        ...
        std::optional<StackElem *> const spill_elem)
    {
        if (spill_elem.has_value()) {
            KINET_DEBUG_ASSERT(dest->general_reg().has_value());
            // Restore `stack_` back to the state before calling
            // `non_literal_jump_dest_operand`.
            auto *e = *spill_elem;
            if (e != nullptr) {
                KINET_DEBUG_ASSERT(e->is_on_stack());
        ...
        if (std::holds_alternative<Gpq256>(dest_op)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(dest_op));
    ...
    Emitter::jumpi_comparison(StackElemRef const cond, StackElemRef const dest)
    {
        ...
        if (cond.get() == dc.stack_elem) {
            ...
            if (cond->avx_reg()) {
                ...
                KINET_DEBUG_ASSERT(cond->stack_offset().has_value());
                RegReserv const dest_reserv{dest};
    ...
    void Emitter::jumpi_spill_fallthrough_stack()
    {
        ...
        if (dest->literal()) {
            ...
            auto const fallthrough_lbl = as_.newLabel();
        ...
        StackElemRef e, uint16_t bound, std::tuple<LiveSet...> const &live)
    {
        ...
        if (e->general_reg()) {
            ...
            KINET_ASSERT(e->stack_offset().has_value());
    ...
    {
        KINET_DEBUG_ASSERT(ix->literal().has_value());
        KINET_DEBUG_ASSERT(src->stack_offset().has_value());

        auto const i = ix->literal()->value;
        ix.reset(); // Potentially release locations
        KINET_DEBUG_ASSERT(i < 32);
    ...
    {
        KINET_DEBUG_ASSERT(ix->literal().has_value());
        KINET_DEBUG_ASSERT(src->general_reg().has_value());
        ...
        StackElemRef ix, StackElemRef src, std::tuple<LiveSet...> const &live)
    {
        ...
        if (ix->general_reg()) {
            ...
            KINET_DEBUG_ASSERT(ix->avx_reg().has_value());
            auto const ix_ymm = avx_reg_to_ymm(*ix->avx_reg());
            as_.vmovq(x86::rax, ix_ymm.xmm());
            ix0 = x86::rax;
        }
    ...
    {
        KINET_DEBUG_ASSERT(ix->literal().has_value());
        KINET_DEBUG_ASSERT(src->avx_reg().has_value());
        ...
        auto const dst_ymm = avx_reg_to_ymm(*dst->avx_reg());
        ...
        StackElemRef ix, StackElemRef src, std::tuple<LiveSet...> const &live)
    {
        ...
        auto const scratch_ymm = avx_reg_to_ymm(*scratch->avx_reg());
        ...
        int8_t ix, StackElemRef src, std::tuple<LiveSet...> const &live)
    {
        ...
        if (src->general_reg()) {
            ...
            KINET_DEBUG_ASSERT(src->stack_offset());
    ...
    void Emitter::signextend_by_literal_ix(
        ...
        std::tuple<LiveSet...> const &live)
    {
        KINET_DEBUG_ASSERT(!src->literal().has_value());
        if (pre_ix >= 31) {
    ...
    void Emitter::signextend_by_non_literal(
        StackElemRef ix, StackElemRef src, std::tuple<LiveSet...> const &live)
    {
        KINET_DEBUG_ASSERT(!stack_.has_deferred_comparison());
        KINET_DEBUG_ASSERT(!ix->literal().has_value());
    ...
    {
        if (shift->literal()) {
            auto const shift_value = shift->literal()->value;
        ...
        unsigned shift, StackElemRef value, std::tuple<LiveSet...> const &live)
    {
        ...
        if (shift == 256) {
            KINET_DEBUG_ASSERT(shift_type == ShiftType::SAR);
    ...
    Emitter::shift_avx_reg_by_literal(unsigned shift, StackElemRef value)
    {
        ...
        KINET_DEBUG_ASSERT(!stack_.has_deferred_comparison());
        KINET_DEBUG_ASSERT(shift <= 256);
        KINET_DEBUG_ASSERT(value->avx_reg().has_value());
    ...
    StackElemRef Emitter::shift_by_literal(
        ...
        std::tuple<LiveSet...> const &live)
    {
        KINET_DEBUG_ASSERT(!value->literal().has_value());
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        if (value->literal()) {
            KINET_DEBUG_ASSERT(!shift->literal().has_value());
    ...
    StackElemRef Emitter::shift_general_reg_by_non_literal(
        ...
        std::tuple<LiveSet...> const &live)
    {
        KINET_DEBUG_ASSERT(value->general_reg().has_value());
        ...
        KINET_DEBUG_ASSERT(rcx_gpq[rcx_gpq_ix] == x86::rax);
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        if (commutative) {
            auto const src_ord = get_stack_elem_general_order_index(src, live);
            auto const dst_ord = get_stack_elem_general_order_index(dst, live);
            if (src_ord < dst_ord) {
                std::swap(dst, src);
            }
        }
        ...
        if (!dst->general_reg()) {
            if (dst->literal()) {
                ...
                KINET_DEBUG_ASSERT(dst->avx_reg().has_value());
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        KINET_DEBUG_ASSERT(dst_loc == LocationType::GeneralReg);
        ...
        std::function<bool(size_t, uint64_t)> is_no_operation)
    {
        ...
        KINET_DEBUG_ASSERT(!std::holds_alternative<x86::Ymm>(src_op));
        ...
        if (std::holds_alternative<Gpq256>(dst_op)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(dst_op));
            KINET_DEBUG_ASSERT(!std::holds_alternative<x86::Mem>(src_op));
    ...
    Emitter::get_una_arguments(
        ...
        std::tuple<LiveSet...> const &live)
    {
        KINET_DEBUG_ASSERT(!dst->literal());
        RegReserv const dst_reserv{dst};
        ...
        if (!dst->avx_reg()) {
            ...
            KINET_DEBUG_ASSERT(dst->stack_offset().has_value());
        ...
        if (!is_live(dst, live)) {
            auto const n = stack_.release_avx_reg(std::move(dst));
        ...
        StackElemRef dst, StackElemRef src, std::tuple<LiveSet...> const &live)
    {
        ...
        //  (general, avx)
        //  (avx, general)
        KINET_DEBUG_ASSERT(dst->general_reg() || src->general_reg());
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        if (dst_loc == LocationType::GeneralReg) {
            auto const new_dst = release_general_reg(dst, live);
            if (dst == src) {
            ...
            KINET_DEBUG_ASSERT(dst_loc == LocationType::AvxReg);
            if (is_live(dst, live)) {
            ...
            auto const n = stack_.release_avx_reg(dst);
            if (dst == src) {
        ...
        std::function<bool(size_t, uint64_t)> is_no_operation)
    {
        ...
        KINET_DEBUG_ASSERT(dst->avx_reg().has_value());
        KINET_DEBUG_ASSERT(std::holds_alternative<x86::Ymm>(left_op));
        if (std::holds_alternative<x86::Ymm>(right_op)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(right_op));
            (as_.*VM)(
                avx_reg_to_ymm(*dst->avx_reg()),
                std::get<x86::Ymm>(left_op),
    ...
    {
        KINET_DEBUG_ASSERT(!stack_.has_deferred_comparison());
        KINET_DEBUG_ASSERT(!elem->literal());
        if (elem->general_reg()) {
            auto const &gpq = general_reg_to_gpq256(*elem->general_reg());
            if (is_live(elem, live)) {
                as_.mov(x86::rax, gpq[1]);
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        KINET_DEBUG_ASSERT(read_size <= 32);
        ...
        // kinet_vm_runtime_increase_memory_raw_* expect context to be passed
        // in rbx.
        static_assert(reg_context == x86::rbx);
        ...
        if (std::holds_alternative<uint64_t>(offset_op)) {
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Gpq>(offset_op));
            auto const r = std::get<x86::Gpq>(offset_op);
            if (r != x86::rax) {
                as_.mov(x86::rax, r);
            }
        ...
        auto const increase_memory_fn =
            memory_version == runtime::Memory::Version::V1
                ? rodata_.add_external_function(
                      kinet_vm_runtime_increase_memory_raw_v1)
                : rodata_.add_external_function(
                      kinet_vm_runtime_increase_memory_raw_mip3);
    ...
    {
        KINET_DEBUG_ASSERT(right != 0 && right != 1);
        if constexpr (Is32Bit) {
            if constexpr (std::is_same_v<LeftOpType, x86::Gpq>) {
    ...
    {
        KINET_DEBUG_ASSERT(std::popcount(right) == 1);
        if constexpr (Is32Bit) {
            KINET_DEBUG_ASSERT(right <= std::numeric_limits<uint32_t>::max());
            if constexpr (std::is_same_v<LeftOpType, x86::Gpq>) {
        ...
        x86::Gpq dst, LeftOpType left, uint64_t pre_right)
    {
        ...
        if (right == 0) {
            ...
            KINET_DEBUG_ASSERT(Is32Bit || is_uint64_bounded(right));
            int32_t r;
    ...
    {
        KINET_DEBUG_ASSERT(bit_size > 0 && bit_size <= 256);
        ...
        auto next_dst_pair = [&](size_t i) -> std::pair<x86::Gpq, x86::Gpq> {
            auto const dst1 = i == last_ix - 1 ? x86::rax : dst[i + 1];
            auto const dst2 = i == 0 ? dst[0] : x86::rdx;
            return {dst1, dst2};
        };
        ...
        if (std::holds_alternative<Gpq256>(left)) {
            ...
            KINET_ASSERT(std::holds_alternative<x86::Mem>(left));
    ...
    Emitter::MulEmitter::mul_sequence(size_t sub_size, x86::Gpq const *mul_dst)
    {
        ...
        if (std::holds_alternative<uint256_t>(right_) &&
            ...
            if (std::holds_alternative<Gpq256>(left_)) {
                ...
                KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(left_));
            ...
            auto const known_value = std::visit(
                Cases{
                    [&](uint256_t const &r) {
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        KINET_DEBUG_ASSERT(!left->literal().has_value());
        ...
        if (required_reg_count > dst_word_count) {
            ...
            if (!left->general_reg() && !left->stack_offset()) {
                KINET_DEBUG_ASSERT(left->avx_reg().has_value());
        ...
        if (needs_mulx) {
            ...
            if (dst_gpq[rdx_general_reg_index] == x86::rdx) {
                KINET_DEBUG_ASSERT(*dst->general_reg() == rdx_general_reg);
            ...
            if (preserve_dst_rdx) {
                ...
                if (!preserve_left_rdx && !preserve_right_rdx &&
                    ...
                    KINET_DEBUG_ASSERT(q[rdx_general_reg_index] == x86::rdx);
        ...
        KINET_DEBUG_ASSERT(dst_word_count <= 4);
        x86::Gpq emit_tmp[3];
        if (tmp != dst) {
            auto &tmp_gpq = general_reg_to_gpq256(*tmp->general_reg());
        ...
        if (bit_size & 31) {
            auto const mask = (uint64_t{1} << (bit_size & 63)) - 1;
    ...
    {
        KINET_DEBUG_ASSERT(!elem->literal().has_value());
        KINET_DEBUG_ASSERT(shift_in <= 255);
        ...
        auto const mask = (static_cast<uint64_t>(1) << (shift & 63)) - 1;
        ...
        KINET_DEBUG_ASSERT(!sh->literal().has_value());
        ...
        if (is_live(sh, live)) {
            ...
            KINET_DEBUG_ASSERT(sh->avx_reg().has_value());
            mov_avx_reg_to_stack_offset(sh);
            dst = stack_.release_stack_offset(sh);
        }
        ...
        std::tuple<LiveSet...> const &live)
    {
        ...
        auto const non_negative_lbl = as_.newLabel();
        auto const after_lbl = as_.newLabel();
    ...
    {
        KINET_DEBUG_ASSERT(exp > 0);
        KINET_DEBUG_ASSERT(elem->stack_offset().has_value());

        x86::Mem mem{stack_offset_to_mem(*elem->stack_offset())};
        auto const reserv = insert_general_reg(elem);
        KINET_DEBUG_ASSERT(elem->general_reg());
        auto const &gpq = general_reg_to_gpq256(*elem->general_reg());
    ...
    {
        return modop_optimized<kinet::addmod, 0, 0, &Emitter::add_mod2>();
    }
        ...
        StackElemRef const a_elem, StackElemRef const b_elem, size_t const exp)
    {
        ...
        KINET_DEBUG_ASSERT(!std::holds_alternative<x86::Ymm>(right_op));
        ...
        if (std::holds_alternative<Gpq256>(left_op)) {
                        ...
                        for (size_t i = start_offset; i < numQwords; i++) {
                            ...
                            if (i == start_offset) {
                                if (bits_occupied <= 32) {
                                    ...
                                    KINET_DEBUG_ASSERT(bits_occupied <= 64);
            ...
            KINET_DEBUG_ASSERT(std::holds_alternative<x86::Mem>(left_op));
            KINET_DEBUG_ASSERT(!std::holds_alternative<x86::Mem>(right_op));
    ...
    {
        return modop_optimized<kinet::mulmod, 1, 0, &Emitter::mul_mod2>();
    }
        ...
        StackElemRef a_elem, StackElemRef b_elem, size_t const exp)
    {
        ...
        KINET_DEBUG_ASSERT(exp >= 1 && exp < 256);
        if (a_elem->literal()) {
            std::swap(a_elem, b_elem);
        }
        KINET_DEBUG_ASSERT(!a_elem->literal().has_value());
        ...
        if (exp <= 192) {
            call_runtime_mul(
                Runtime<uint256_t *, uint256_t const *, uint256_t const *>(
                    this, false, kinet_vm_runtime_mul_192));
        }
        else {
            call_runtime_mul(
                Runtime<uint256_t *, uint256_t const *, uint256_t const *>(
    ...
    void Emitter::stack_elem_byte_width(StackElemRef const elem)
    {
        if (elem->general_reg()) {
            ...
            auto const avx_reg = avx_reg_to_ymm(*elem->avx_reg());
            auto [avx_tmp_elem, _] = alloc_avx_reg();
            auto const avx_tmp = avx_reg_to_ymm(*avx_tmp_elem->avx_reg());
    ...
    {
        KINET_ASSERT(!exponent_elem->literal().has_value());
