...
namespace kinet::vm::compiler::native
{
    ...
    {
        KINET_ASSERT(stack_.deferred_comparison_.stack_elem == nullptr);
        KINET_DEBUG_ASSERT(
    ...
    {
        KINET_ASSERT(!stack_.deferred_comparison_.stack_elem);
        KINET_ASSERT(stack_.deferred_comparison_.negated_stack_elem);
        KINET_DEBUG_ASSERT(
    ...
    void StackElem::negated_deferred_comparison()
    {
        KINET_ASSERT(!stack_.deferred_comparison_.negated_stack_elem);
        KINET_ASSERT(stack_.deferred_comparison_.stack_elem);
        KINET_DEBUG_ASSERT(stack_.deferred_comparison_.stack_elem != this);
        KINET_DEBUG_ASSERT(
            !stack_offset_ && !avx_reg_ && !general_reg_ && !literal_);
        stack_.deferred_comparison_.negated_stack_elem = this;
    }
    ...
    void StackElem::insert_general_reg()
    {
        ...
        KINET_ASSERT(!general_reg_.has_value());
        KINET_ASSERT(stack_.general_reg_stack_elems_[reg.reg] == nullptr);
        general_reg_ = reg;
    ...
    void StackElem::free_avx_reg()
    {
        ...
        KINET_ASSERT(stack_.avx_reg_stack_elems_[reg] == this);
    ...
    void StackElem::free_general_reg()
    {
        ...
        KINET_ASSERT(stack_.general_reg_stack_elems_[reg] == this);
    ...
    StackElemRef &Stack::at(int32_t const index)
    {
        if (index < 0) {
            ...
            KINET_ASSERT(i < negative_elems_.size());
            return negative_elems_[i];
        }
        else {
            auto const i = static_cast<size_t>(index);
            KINET_ASSERT(i < positive_elems_.size());
    ...
    StackElemRef Stack::negate_if_deferred_comparison(StackElemRef const e)
    {
        ...
        if (dc.stack_elem == e.get()) {
            ...
            if (dc.stack_elem) {
                ...
                KINET_DEBUG_ASSERT(i != dc.stack_elem->stack_indices_.end());
                return at(*i);
            }
            else {
                auto d = new_stack_elem();
        ...
        int32_t const preferred, PrevLoc const moved_from) const
    {
        ...
        KINET_ASSERT(!available_stack_offsets_.empty());
        return {*available_stack_offsets_.begin(), moved_from};
    }

    StackElem *Stack::find_stack_elem_for_avx_reg_spill()
    {
        KINET_ASSERT(free_avx_regs_.empty());
    ...
    {
        KINET_ASSERT(free_general_regs_.empty());
    ...
    Stack::spill_all_caller_save_general_regs()
    {
        ...
        for (uint8_t i = 1; i < GENERAL_REG_COUNT; ++i) {
            ...
            KINET_ASSERT(e->reserve_general_reg_count_ == 0);
    ...
    Stack::spill_avx_reg_range(uint8_t const first)
    {
        ...
        for (uint8_t i = first; i < 16; ++i) {
            ...
            KINET_ASSERT(e->reserve_avx_reg_count_ == 0);
        ...
        StackElem &e, int32_t const preferred, PrevLoc const moved_from)
    {
        ...
        auto const offset = find_available_stack_offset(preferred, moved_from);
        e.insert_stack_offset(offset);
    }
    ...
    void Stack::move_avx_reg(StackElem &src, StackElem &dst)
    {
        ...
        KINET_ASSERT(avx_reg_stack_elems_[reg.reg] == &src);
        dst.avx_reg_ = reg;
    ...
    void Stack::move_general_reg(StackElem &src, StackElem &dst)
    {
        ...
        KINET_ASSERT(general_reg_stack_elems_[reg.reg] == &src);
    ...
    {
        KINET_ASSERT(e1.general_reg_.has_value());
        KINET_ASSERT(e2.general_reg_.has_value());

        GeneralReg const reg1 = *e1.general_reg_;
        GeneralReg const reg2 = *e2.general_reg_;
        KINET_ASSERT(general_reg_stack_elems_[reg1.reg] == &e1);
        KINET_ASSERT(general_reg_stack_elems_[reg2.reg] == &e2);
    ...
    {
        KINET_ASSERT(e.general_reg_.has_value());
        e.remove_general_reg();
    }
