...
namespace kinet::vm::utils
{
    ...
    {
        auto const &tbl = kinet::vm::compiler::make_opcode_table<
    ...
    std::vector<uint8_t> compile_tokens(
        ...
        evm_as::EvmBuilder<EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>> const
            &eb)
    {
            ...
            if (p != input) {
                ...
                if (op == "PUSH0") {
                    ...
                    if (!opcode.has_value()) {
                        ...
                        eb.ins(static_cast<kinet::vm::compiler::EvmOpCode>(
                            opcode.value()));
                    }
                }
