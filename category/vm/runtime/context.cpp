...
{
    KINET_DEBUG_ASSERT((*new_size & 31) == 0);
    KINET_DEBUG_ASSERT(old_size < *new_size);
...
namespace kinet::vm::runtime
{
    ...
    {
        kinet_vm_runtime_increase_capacity(this, old_size, new_size);
    }
    ...
    Context::copy_result_data()
    {
        ...
        if (*memory_end <= memory.size) {
            ...
            if (KINET_UNLIKELY(!is_memory_size_in_bound<traits>(memory_end))) {
    ...
    evmc::Result Context::copy_to_evmc_result()
    {
        ...
        if (KINET_UNLIKELY(result.status == Error)) {
            return evmc_error_result(EVMC_FAILURE);
        }
        if (KINET_UNLIKELY(result.status == OutOfGas)) {
            return evmc_error_result(EVMC_OUT_OF_GAS);
        }

        KINET_DEBUG_ASSERT(result.status == Success || result.status == Revert);
