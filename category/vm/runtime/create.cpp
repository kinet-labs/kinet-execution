...
{
    consteval Bin<2> create_code_word_cost(kinet_eth_revision const rev)
    {
        return (rev >= KINET_ETH_SHANGHAI) ? bin<2> : bin<0>;
    }

    consteval Bin<4> create2_code_word_cost(kinet_eth_revision const rev)
    {
        return (rev >= KINET_ETH_SHANGHAI) ? bin<8> : bin<6>;
    }
        ...
        evmc_call_kind const kind, int64_t const remaining_block_base_gas)
    {
        ...
        if constexpr (
            traits::evm_rev() >= KINET_ETH_PRAGUE &&
        ...
        if constexpr (traits::evm_rev() >= KINET_ETH_SHANGHAI) {
            if (KINET_UNLIKELY(*size > traits::max_initcode_size())) {
        ...
        if (KINET_UNLIKELY(ctx->env.depth >= 1024)) {
            return 0;
        }
