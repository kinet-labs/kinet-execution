...
namespace kinet::vm::runtime
{
    ...
    struct Environment
    {
        ...
        {
            KINET_DEBUG_ASSERT(return_data_size == 0);
    ...
    struct Memory
    {
        ...
        explicit Memory(
            ...
            , parent_handle{han}
        {
            KINET_DEBUG_ASSERT(han != nullptr);
            KINET_DEBUG_ASSERT(dat != nullptr);
        }
        ...
        ~Memory()
        {
            ...
            if (KINET_UNLIKELY(data_handle)) {
                KINET_ASSERT(size <= capacity);
                KINET_ASSERT(data == data_handle);
                clear();
            }
        }
        ...
        {
            KINET_DEBUG_ASSERT(data >= data_handle);

            auto const x = static_cast<uintptr_t>(data - data_handle);

            KINET_DEBUG_ASSERT((x & 31) == 0);
        ...
        void clear()
        {
            if (KINET_LIKELY(parent_handle == data_handle)) {
        ...
        void release()
        {
            if (KINET_UNLIKELY(data_handle != parent_handle)) {
    ...
    struct Context
    {
        ...
        {
            gas_remaining -= gas;
            if (KINET_UNLIKELY(gas_remaining < 0)) {
        ...
        void expand_memory(Bin<29> const min_size)
        {
            if (memory.size < *min_size) {
                ...
                KINET_DEBUG_ASSERT(new_cost >= memory.cost);
                ...
                if (KINET_UNLIKELY(memory.capacity < *new_size)) {
        ...
        void return_to(Context *parent)
        {
            if (parent != nullptr) {
                ...
                KINET_DEBUG_ASSERT(memory.parent_handle == p.data_handle);
                if (KINET_UNLIKELY(
        ...
        {
            if (KINET_UNLIKELY(is_stack_unwinding_active)) {
                stack_unwind();
            }
        }
...
extern "C" void kinet_vm_runtime_increase_capacity(
    kinet::vm::runtime::Context *, uint32_t old_size,
    kinet::vm::runtime::Bin<30> new_size);

extern "C" void kinet_vm_runtime_increase_memory_v1(
    kinet::vm::runtime::Bin<29> min_size, kinet::vm::runtime::Context *);
extern "C" void kinet_vm_runtime_increase_memory_mip3(
    kinet::vm::runtime::Bin<29> min_size, kinet::vm::runtime::Context *);

// Note: kinet_vm_runtime_increase_memory_raw_* uses non-standard
// calling convention. Context is passed in rbx and new min
// memory size if passed in rdi. See context.S. Use the
// kinet_vm_runtime_increase_memory_* function for a version
// using standard calling convention
extern "C" void kinet_vm_runtime_increase_memory_raw_v1();
extern "C" void kinet_vm_runtime_increase_memory_raw_mip3();
