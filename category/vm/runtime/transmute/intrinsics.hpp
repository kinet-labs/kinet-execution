...
extern "C" __m256i
kinet_vm_runtime_load_bounded_le(uint8_t const *src_buffer, int64_t load_size);

// Note: kinet_vm_runtime_load_bounded_le_raw uses non-standard
// calling convention. See transmute.S. Use the
// kinet_vm_runtime_load_bounded_le function for a version
// using standard calling convention.
extern "C" __m256i kinet_vm_runtime_load_bounded_le_raw();
...
namespace kinet::vm::runtime
{
    ...
    {
        if (KINET_LIKELY(max_len >= 32)) {
            return load_le_unsafe<uint256_t>(bytes);
        }
        return uint256_t{kinet_vm_runtime_load_bounded_le(bytes, max_len)};
    }
}
