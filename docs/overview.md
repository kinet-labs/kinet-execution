...
- The consensus repository is called [kinet-bft](https://github.com/category-labs/kinet-bft),
  because its distributed consensus algorithm belongs to the Byzantine Fault
  Tolerant family; the name of the consensus daemon binary is `kinet-node`

- The execution repository is just called [kinet](https://github.com/category-labs/kinet),
  and the name of the execution daemon binary is also just `kinet`

## How is execution used?

Most of the source code in this repository is compiled into a single library,
called `libkinet_execution.a` (or `libkinet_execution.so`, if you compile
using shared libraries).

This library is used in a few different ways:

1. **Stand-alone/daemon mode**: the execution daemon (source code in the
   `cmd/kinet` directory) is run as a systemd service; it is small
   wrapper around the `libkinet_execution` functionality that looks for
...
2. **Hosted mode**: the consensus daemon and the RPC server also use some
   ...
   as a shared object (`libkinet_execution.so`), and loading it into their
   processes; they can then call execution functions directly

3. **Interactive mode**: `cmd/kinet` can also be run interactively from the
...
```
  ...
  Writes new blocks    ║         The "ledger":          ║       written by the
   ...
   ││       Kinet consensus       ││        │ ├────────────────────────────┤ │
   ││          algorithm          ││        │ │                            │ │
   ││                             ││        │ │    libkinet_execution.a    │ │
   │├─────────────────────────────┤│        │ │  static library functions  │ │
   ││                             ││        │ │ (including EVM and triedb) │ │
   ││    libkinet_execution.so    ││        │ │                            │ │
...
`kinet-node`: even though a "Kinet node" is really both daemons operating
...
_appears_ to be "the node." In reality, during normal operation of the Kinet
blockchain protocol, neither daemon can do much of anything without the other.

## Organization of the source code

The source code for `libkinet_execution` is in the `category` subdirectory.
...
| vm        | Ethereum Virtual Machine implementation (interpreter and native code compiler, with Kinet extensions)        |
...
produces a separate library called `libkinet_event`, which is only meant for
third parties.
...
`libkinet_execution` library. `libkinet_event` is a separate CMake target in
