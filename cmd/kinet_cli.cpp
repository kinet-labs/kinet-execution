...
{
    using res = std::invoke_result_t<kinet_hash256 (*)(T), T>;
...
struct DbStateMachine
{
    ...
    {
        KINET_ASSERT(version != INVALID_BLOCK_NUM);
        ...
        KINET_ASSERT(curr_version == INVALID_BLOCK_NUM);
        KINET_ASSERT(curr_section_prefix.nibble_size() == 0);
    ...
    Result<NodeCursor> lookup(NibblesView const key) const
    {
        ...
        KINET_ASSERT(!curr_section_prefix.empty());
        KINET_ASSERT(curr_table_id != INVALID_NIBBLE);
        fmt::println(
            "Looking up key {} \nat version {} on table {} ... ",
            key,
            curr_version,
...
int main(int const argc, char *argv[])
{
    ...
    CLI::App cli{
        "Inspection and snapshot tooling for a Kinet execution database.",
        "kinet-cli"};
    ...
    cli_group
        ->add_option(
            ...
            "(default: 1)")
        ->check(CLI::Range(1u, KINET_SNAPSHOT_SHARDS))
    ...
    cli.add_flag(
        ...
        "active with its state_machine_kind stamped (via kinet-mpt "
    ...
    if (dump_binary_snapshot.has_value()) {
        ...
        // KINET_SNAPSHOT_SHARDS) with s % total_shards == shard_number (see
        ...
        uint64_t const shards_this_run =
            shard_number < KINET_SNAPSHOT_SHARDS
                ? (KINET_SNAPSHOT_SHARDS - shard_number + total_shards - 1) /
                      total_shards
                : 0;
        ...
        uint64_t const required_fds =
            shards_this_run * KINET_SNAPSHOT_FILES_PER_SHARD + fd_headroom;
        struct rlimit limit{};
        if (getrlimit(RLIMIT_NOFILE, &limit) != 0) {
            ...
            LOG_ERROR(
                ...
                required_fds,
                shards_this_run,
                KINET_SNAPSHOT_FILES_PER_SHARD);
            return 1;
        }
    }
    ...
    if (dump_binary_snapshot.has_value()) {
        ...
        bool const success = kinet_db_dump_snapshot(
            c_dbname_paths.data(),
            c_dbname_paths.size(),
