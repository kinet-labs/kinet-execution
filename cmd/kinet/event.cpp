...
int claim_event_ring_file(fs::path const &ring_path)
{
    ...
    if (flock(ring_fd, LOCK_EX | LOCK_NB) == -1) {
        ...
        if (saved_errno == EWOULDBLOCK) {
            ...
            (void)kinet_event_ring_query_excl_writer_pid(ring_fd, &owner_pid);
            if (owner_pid == 0) {
                LOG_ERROR(
...
int allocate_event_ring_file(
    kinet_event_ring_simple_config const &simple_cfg, fs::path const &init_path,
    fs::path const &ring_path, int *const init_ring_fd)
{
    ...
    *init_ring_fd =
    ...
    if (int const rc = kinet_event_ring_init_simple(
...
// function kinet_event_ring_query_excl_writer_pid might assume the file is
...
int create_owned_event_ring(
    ...
    kinet_event_ring_simple_config const &simple_cfg, int *const ring_fd)
{
...
int init_owned_event_ring(
    EventRingConfig ring_config, kinet_event_content_type const content_type,
    ...
    std::unique_ptr<OwnedEventRing> &owned_event_ring)
{
    ...
    if (int const rc = kinet_event_resolve_ring_file(
    ...
    if (int const rc = kinet_check_path_supports_map_hugetlb(
    ...
    kinet_event_ring event_ring;
    if (int const rc = kinet_event_ring_mmap(
            &event_ring,
    ...
    LOG_INFO(
        "{} event ring created: {}",
        g_kinet_event_content_type_names[std::to_underlying(content_type)],
        ring_path);
    return 0;
}
    ...
    EventRingConfig ring_config, std::unique_ptr<OwnedEventRing> &ring)
{
    return init_owned_event_ring(
        ...
        g_kinet_exec_event_schema_hash,
