...
{
    auto app =
        CLI::App("Instrumentable standalone kinet compiler execution engine");
    auto args = arguments{};
...
static void dump_result(arguments const &args, evmc::Result const &result)
{
    ...
    if (result.status_code == EVMC_SUCCESS) {
        object["result"] = json(kinet::to_hex(
            kinet::byte_string_view{result.output_data, result.output_size}));
    }
    else {
...
int main(int argc, char **argv)
{
    ...
    if (rev == "BERLIN") {
        return mce_main<EvmTraits<KINET_ETH_BERLIN>>(args);
    }
    else if (rev == "LONDON") {
        return mce_main<EvmTraits<KINET_ETH_LONDON>>(args);
    }
    else if (rev == "PARIS") {
        return mce_main<EvmTraits<KINET_ETH_PARIS>>(args);
    }
    else if (rev == "SHANGHAI") {
        return mce_main<EvmTraits<KINET_ETH_SHANGHAI>>(args);
    }
    else if (rev == "CANCUN") {
        return mce_main<EvmTraits<KINET_ETH_CANCUN>>(args);
    }
    else if (rev == "PRAGUE") {
        return mce_main<EvmTraits<KINET_ETH_PRAGUE>>(args);
    }
    else if (rev == "OSAKA") {
        return mce_main<EvmTraits<KINET_ETH_OSAKA>>(args);
    }
    else if (rev == "AMSTERDAM") {
        return mce_main<EvmTraits<KINET_ETH_AMSTERDAM>>(args);
    }
    else if (rev == "LATEST") {
        return mce_main<EvmTraits<KINET_ETH_LATEST_STABLE_REVISION>>(args);
    }
    else {
