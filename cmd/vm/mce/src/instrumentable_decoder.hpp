...
public:
    ...
    std::vector<uint8_t> decode(fs::path const &filename)
    {
        ...
        if (filename.extension() == ".mevm") {
            ...
            kinet::vm::utils::parser_config config{false, false};
            if constexpr (instrument) {
                if constexpr (device == InstrumentationDevice::Cachegrind) {
                    CACHEGRIND_START_INSTRUMENTATION;
                    std::vector<uint8_t> const code =
                        kinet::vm::utils::parse_opcodes(config, contents);
                ...
                return kinet::vm::utils::parse_opcodes(config, contents);
            }
        }
        ...
        if constexpr (instrument) {
            if constexpr (device == InstrumentationDevice::Cachegrind) {
                ...
                CACHEGRIND_START_INSTRUMENTATION;
                std::vector<uint8_t> const code =
                    kinet::vm::utils::parse_hex_program(bytes);
                CACHEGRIND_STOP_INSTRUMENTATION;
            ...
            return kinet::vm::utils::parse_hex_program(bytes);
        }
    }
