...
{
    kinet::vm::runtime::EvmStackAllocator stack_allocator;
    kinet::vm::MemoryPool memory_pool_;
    kinet::test::TestState<false> test_state_;
    kinet::vm::VM vm_;

public:
    InstrumentableVM(asmjit::JitRuntime &rt)
        : evmc_vm{EVMC_ABI_VERSION, "kinet-compiler-x86-microbenchmark-engine", "0.0.0", abi_compat::destroy, abi_compat::execute, abi_compat::get_capabilities, nullptr}
    ...
    evmc::Result execute(Binary &entry)
    {
        ...
        kinet::test::TestBlockHashBuffer const block_hash_buffer{};
        ...
        kinet::test::TestHost<traits> test_host{
            block_hash_buffer,
            state,
            tx,
            msg->sender,
    ...
    void dispatch_execute(
        Binary &entry, kinet::vm::runtime::Context *ctx, uint8_t *stck)
    {
