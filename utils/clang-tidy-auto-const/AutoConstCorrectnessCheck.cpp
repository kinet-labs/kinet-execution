//===--- AutoConstCorrectnessCheck.cpp - clang-tidy -----------------*- C++
//-*-===//
//
...
namespace clang::tidy::misc
{
    ...
    AutoConstCorrectnessCheck::AutoConstCorrectnessCheck(
    ...
    void
    AutoConstCorrectnessCheck::storeOptions(ClangTidyOptions::OptionMap &Opts)
    {
        ...
        Options.store(
            Opts, "TransformPointersAsValues", TransformPointersAsValues);
    }

    void AutoConstCorrectnessCheck::registerMatchers(MatchFinder *Finder)
    {
        ...
        auto const LocalValDecl = varDecl(
            isLocal(),
            ...
            functionDecl(
              hasBody(stmt(forEachDescendant(
                               declStmt(
                                   ...
                                   hasAncestor(functionDecl().bind("function-decl")))
                               .bind("decl-stmt")))));
    ...
    void
    AutoConstCorrectnessCheck::check(MatchFinder::MatchResult const &Result)
    {
        auto const *LocalScope = Result.Nodes.getNodeAs<Stmt>("scope");
        auto const *Variable = Result.Nodes.getNodeAs<VarDecl>("local-value");
        auto const *Function =
        ...
        if (Variable->getType()->isArrayType()) {
            if (auto const *ArrayT = dyn_cast<ArrayType>(Variable->getType())) {
        ...
        auto const *VarDeclStmt = Result.Nodes.getNodeAs<DeclStmt>("decl-stmt");
    ...
    void AutoConstCorrectnessCheck::registerScope(
