...
bool kinet_bn_add(uint8_t out[64], const uint8_t *input, size_t len);

// EIP-196: Precompiled contract for multiplication on the elliptic curve alt_bn128
bool kinet_bn_mul(uint8_t out[64], const uint8_t *input, size_t len);

// EIP-197: Precompiled contracts for optimal ate pairing check on the elliptic curve alt_bn128
bool kinet_snarkv(uint8_t out[32], const uint8_t *input, size_t len);
